...
<?php 
 $sizes=new Sizes();
  ...
  while($rw=mysql_fetch_object($sizes->result)){
     $cols=" case when sum(case when sizeid=$rw->id then case when status='checkedin' or status='stocktake' then quantity else quantity*-1 end end) is null then '' else sum(case when sizeid=$rw->id then case when status='checkedin' or status='stocktake' then quantity else quantity*-1 end end) end '$rw->name'";
  ...
  //$cols=substr($cols,-1);
	...
	if(!empty($obj->shquantity)){
		...
		if(!empty($rptgroup)){
			array_push($aColumns, "sum(case when status='checkedin' or status='stocktake' then post_graded.quantity else post_graded.quantity*-1 end) quantity");
		}else{
		array_push($aColumns, "case when status='checkedin' then post_graded.quantity else post_graded.quantity*-1 end");
		}
		...
		<table class="tgrid gridd" border="0" align="right">
			...
			<tr>
			  ...
			  <td><select name="status">
				...
				<option value="stocktake" <?php if($obj->status=="stocktake"){echo"selected";}?>>Stocktake</option>
