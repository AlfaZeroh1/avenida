...
$fields=" concat(hos_patients.surname,' ', hos_patients.othernames) as name, hos_patientlaboratorytests.testedon,hos_patientlaboratorytests.patienttreatmentid,hos_patients.dob,sys_genders.name as genderid,hos_patients.dob  ";
$join=" left join hos_patients on hos_patients.id=hos_patientlaboratorytests.patientid left join sys_genders on sys_genders.id=hos_patients.genderid";
...
<span style="display:block; padding:0px 0px 1px;">Tel: <?php echo $arr['companytel'];?></span> </div>
 ...
 <span style="display:block; padding:3px 10px; font-size:16px; text-align:l; font-weight:bold; color:#fff; background-color:#999">Laboratory Results</span></div>
 <div class="hfields" align="left" style="float:left; width:100%; padding-left:5px;">
 <hr/>
 <table align="left">
 <tr>
 <td align="left"><strong>Patient:</strong>&nbsp;&nbsp;<?php echo initialCap($patient->name); ?></td>
 </tr>
 <tr>
 <td align="left"><strong>Tested On:</strong>&nbsp;&nbsp;<?php echo formatDate($patient->testedon); ?></td>
 </tr>
  <tr>
 <td align="left"><strong>Gender:</strong>&nbsp;&nbsp;<?php echo $patient->genderid; ?></td>
 </tr>
  <tr>
 <td align="left"><strong>Age:</strong>&nbsp;&nbsp;<?php echo $patient->dob; ?></td>
 </tr>
 <tr>
 <td align="left"><strong>Lab No:</strong>&nbsp;&nbsp;<?php echo $testno; ?></td>
 </tr>
 <?php if(!empty($patient->patienttreatmentid)){?>
 <tr>
 <td align="left"><strong>Treatment No:</strong>&nbsp;&nbsp;<?php echo $patient->patienttreatmentid; ?></td>
 </tr>
 <?php } ?>
 <tr>
<!--  <td align="left"><strong>Age:</strong>&nbsp;&nbsp;<?php echo $patient->dob; ?></td> -->
 ...
 <?php
 $emp=mysql_fetch_object(mysql_query("select h.firstname from hrm_employees h left join auth_users u on h.id=u.employeeid left join hos_patienttreatments p on p.createdby=u.id left join hos_patientlaboratorytests t on t.patienttreatmentid=p.id where t.testno='$testno'"));
 if(!empty($emp->firstname)){
 ?>
 <tr>
 <td align="left"><strong>Doctor:</strong>&nbsp;&nbsp;<?php echo $emp->firstname; ?></td>
 </tr>
 <?php } 
 $query="select h.firstname from hrm_employees h left join auth_users u on h.id=u.employeeid left join hos_patientlaboratorytests t on t.lasteditedby=u.id where t.testno='$testno'";
 $emp=mysql_fetch_object(mysql_query($query));
 ?>
 <tr>
 <td align="left"><strong>Technologist:</strong>&nbsp;&nbsp;<?php echo $_SESSION['username']; ?></td>
 </tr>
 <tr>
 </table>
 <hr/></div>
