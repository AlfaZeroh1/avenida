...
if($obj->action=="Add Laboratorytest"){
	...
	$patienttreatments = new Patienttreatments();
	$fields="hos_patienttreatments.patientid,hos_patientappointments.departmentid, hos_patienttreatments.patientappointmentid  ";
	...
	if($patientlaboratorytests->add($patientlaboratorytests)){
		...
		$obj->treatmentno=$pat->patientappointmentid;
...
if($obj->action=="Add Item"){
	if(empty($obj->itemsitemid)){
	   $error="Item Must be provided! ";
	}elseif(empty($obj->itemsquantity)){
	...
	if($patientprescriptions->add($patientprescriptions)){
		...
		$patienttreatments = new Patienttreatments();
		$fields="hos_patienttreatments.patientid,hos_patienttreatments.patientappointmentid,hos_patientappointments.departmentid ";
		$join=" left join hos_patientappointments on hos_patientappointments.id=hos_patienttreatments.patientappointmentid";
		$having="";
		$groupby="";
		$orderby="";
		...
		$obj->patientid=$pat->patientid;
		$obj->departmentid=$pat->departmentid;
		$obj->treatmentno=$obj->patientappointmentid;
		$obj->consult=1;
		$obj->paid="No";
