...
if(!empty($treatmentid)){
	...
	$patienttreatmentss = new Patienttreatments();
	$fields="*";
	$join="";
	$having="";
	$groupby="";
	...
	if(!empty($appointmentid))
	    $where="where patientappointmentid='$ob->id'";
	else   
	    $where="where id='$treatmentid'";
	$patienttreatmentss->retrieve($fields, $join, $where, $having, $groupby, $orderby);//echo $patienttreatmentss->sql;
	$patienttreatmentss=$patienttreatmentss->fetchObject;
	...
	$treat=$patienttreatments->fetchObject;
	if(empty($treatmentid) and empty($patienttreatmentss->id))
		$obj->treatmentid=$treat->id;
        elseif(empty($treatmentid) and !empty($patienttreatmentss->id))
                $obj->treatmentid=$patienttreatmentss->id;
	else
	...
	if(!empty($_GET['appointmentid'])){
		...
		if(empty($patienttreatmentss->id)){
...
<table style="clear:both;"  class="tgrid display" id="laboratorylist" border="0" cellspacing="0" cellpadding="2" align="center">
	...
	?>
		<tr>
			...
			<td><a href="../patienttreatments/addpatienttreatments_proc.php?treatmentid=<?php echo $row->patienttreatmentid; ?>#tabs-3">View</a></td>
		</tr>
	<?php 
	}
	?>
...
<?php if(!empty($obj->diagnosiid) and $obj->diagnosiid!="NULL" and $obj->diagnosiid!=NULL){ ?>
			<tr>
...
<?php }else{ ?>
                                ...
                                <tr>
                                        ...
                                        <strong>You can Not Add Other Services Without Diagnosis</strong>
                                        </td>
                                </tr>
<?php } ?>
			</table>
...
<div id="tabs-6" style="min-height:420px;">
	...
	<?php if(!empty($obj->diagnosiid) and $obj->diagnosiid!="NULL" and $obj->diagnosiid!=NULL){ ?>
				...
				<tr>
					...
					<td><input type="text" name="frequency" id="frequency" size="4" tabindex="4" value="<?php echo $obj->frequency; ?>"></td>
					<td><input type="text" name="duration" id="duration" size="4" tabindex="5" value="<?php echo $obj->duration; ?>"></td>
					<td><input class="btn" type="submit" value="Add Item" name="action" tabindex="6"/></td>
...
<?php }else{ ?>
                                ...
                                <tr>
                                        <td colspan="9" align="center" style="color:red;">
                                        <strong>You can Not Prescribe Without Diagnosis</strong>
                                        </td>
                                </tr>
<?php } ?>
</table>
	...
	<tfoot>
		<tr>
			...
			<th>&nbsp;</th>
			<th>&nbsp;</th>
			<th><?php echo formatNumber($ttotal); ?></th>
			<th>&nbsp;</th>
			<th>&nbsp;</th>
		</tr>
	</tfoot>
