...
require_once("../taskstatuss/Taskstatuss_class.php");
...
<form action="" method="post">
  ...
  <?php
    ...
    while($row=mysql_fetch_object($statuss->result)){
      ?>
	<option value="<?php echo $row->id; ?>" <?php if($row->id==$obj->statusid){echo"selected";}?>><?php echo $row->name; ?></option>
      <?php
    }
  ?>
  </select>
...
<?php } ?>
	...
	<?php
		...
		if(!empty($obj->statusid)){
		  $where=" where pm_tasks.statusid in('$obj->statusid') and pm_tasks.employeeid in(select employeeid from auth_users where id='".$_SESSION['userid']."')";
		}
