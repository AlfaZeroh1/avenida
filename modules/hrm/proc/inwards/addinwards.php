...
function calculateQuantity(id,value,pack){
 var quantity=value*pack;
 var total = $("#quantity"+id).val(quantity);
...
function enableQuantity(id){
  ...
  if($("#id"+id).is(':checked')){
    ...
    $("#pack"+id).prop('readonly', false);
  }
  else{
    $("#quantity"+id).prop('readonly', true);
    $("#pack"+id).prop('readonly', true);
  }
 ...
 <?php if(!empty($obj->retrieve)){?>
	...
	<td><input type="submit" name="action2" value="Add"/></td>
		...
		<table align='center'>
			<tr>
			<td>
		<?php if($obj->type=="in"){echo "Inward";}else{echo "Return";}?> Note No:<input type="text" name="documentno" id="documentno" readonly size="8"  value="<?php echo $obj->documentno; ?>">
		<?php if($obj->type=="in"){echo "Inward";}else{echo "Return";}?> Date:<input type="date" name="inwarddate" id="inwarddate" readonly class="date_input" size="12" readonly  value="<?php echo $obj->inwarddate; ?>">
		Remarks :<textarea name="remarks" ><?php echo $obj->remarks; ?></textarea>
		LPO No:<textarea name="lpono" ><?php echo $obj->lpono; ?></textarea>
		<?php if($obj->type=="out"){?>
		GRN No:<textarea name="inwardno" readonly ><?php echo $obj->inwardno; ?></textarea>
		<?php } ?>
...
<table style="clear:both" id="tbl" cellpadding="0" align="center" width="98%" cellspacing="0">
	...
	<tr style="font-size:18px; vertical-align:text-top; ">
		...
		<th align="left">Package <?php if($obj->type=="in"){?>Received<?php }else{?>Returned<?php } ?>  </th>
		<th align="left">Qnt <?php if($obj->type=="in"){?>Received<?php }else{?>Returned<?php } ?>  </th>
		<th align="left">Package <?php if($obj->type=="in"){?>Receivable<?php }else{?>Returnable<?php } ?>  </th>
	...
	if($_SESSION['shpinwards']){
		...
		<tr style="font-size:12px; vertical-align:text-top; ">
			...
			<td><?php echo ($shpinwards[$i]['quantitys']-$shpinwards[$i]['quantity'])/$shpinwards[$i]['package']; ?> </td>
			<td><?php echo $shpinwards[$i]['quantitys']-$shpinwards[$i]['quantity']; ?> </td>
			<td><input type="text" id="pack<?php echo $i; ?>" size="4" value="<?php echo $shpinwards[$i]['quantity']/$shpinwards[$i]['package']; ?>" readonly onChange="calculateQuantity('<?php echo $i; ?>', this.value,<?php echo $shpinwards[$i]['package']; ?>);"/> </td>
