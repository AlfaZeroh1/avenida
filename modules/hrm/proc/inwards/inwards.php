...
<table style="clear:both;"  class="tgrid display" id="example" width="98%" border="0" cellspacing="0" cellpadding="2" align="center" >
	<thead>
		<tr>
			<th>#</th>
			<th>&nbsp;</th>
			<th><?php if($ob->type=="in"){?>Inward<?php }else{ ?>Return<?php } ?> Note No </th>
			<th>LPO No </th>
			<?php if($ob->type=="out"){?>
			<th>GRN No</th>
			<?php } ?>
			<th>Supplier </th>
			<th><?php if($ob->type=="in"){?>Inward<?php }else{ ?>Return<?php } ?> Date </th>
			<th>Remarks </th>
			<th>Total </th>
			<th>&nbsp;</th>
			<?php if($ob->type=="in"){ ?>
	...
	<?php
		...
		$where=" where proc_inwarddetails.status!=1 and proc_inwards.type='$ob->type' ";
	...
	?>
		<tr style="color:<?php echo $color; ?>">
			...
			<td><a href="addinwards_proc.php?retrieve=1&documentno=<?php echo $row->documentno; ?>&type=<?php echo $ob->type; ?>">View</a></td>
			<?php if($ob->type=="in"){ ?>
			<td><a href="addinwards_proc.php?retrieve=1&documentno=<?php echo $row->documentno; ?>&type=out&return=1">Return</a></td>
			<?php } ?>
		</tr>
	<?php 
	}
