...
$().ready(function() {
 ...
 $("#suppliername").result(function(event, data, formatted) {
   if (data)
   {
     ...
     document.getElementById("rate").value=data[19];
     document.getElementById("eurorate").value=data[20];
   }
 });
...
<?php
	...
	?>
		...
		<td>
		<input type='text' name='costprice' id='costprice'  onchange="calculateVATAmount();calculateTotal();" onblur="calculateTotal();"  size='8'  value='<?php echo $obj->costprice; ?>'/>
		</td>
		<td>
...
<table style="clear:both" class="tgrid display" id="example" cellpadding="0" align="center" width="98%" cellspacing="0">
	...
	if($_SESSION['shppurchaseorders']){
		...
		$receiv=mysql_fetch_object(mysql_query("select sum(quantity) quantity from proc_inwards left join proc_inwarddetails on proc_inwards.id=proc_inwarddetails.inwardid where proc_inwarddetails.itemid='".$shppurchaseorders[$i]['itemid']."' and proc_inwards.deliverynoteno='$obj->invoiceno'"));
