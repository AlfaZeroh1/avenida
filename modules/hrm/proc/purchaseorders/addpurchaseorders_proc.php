...
if($obj->action=="Save"){
	...
	elseif(empty($shppurchaseorders)){
		...
		if($purchaseorders->add($purchaseorders,$shppurchaseorders)){		  
		        ...
		        unset($_SESSION['itid']);
			unset($_SESSION['cost']);
			unset($_SESSION['shppurchaseorders']);
			$saved="Yes";
...
if($obj->action2=="Add"){
	...
	elseif(empty($obj->itemid) and empty($obj->categoryid)){
	...
	if(empty($obj->edit)){
	  ...
	  $shppurchaseorders[$obj->i]=array('quantity'=>"$obj->quantity", 'itemid'=>"$obj->itemid", 'itemname'=>"$items->name", 'categoryid'=>"$obj->categoryid", 'categoryname'=>"$obj->categoryname",  'unitofmeasureid'=>"$obj->unitofmeasureid", 'unitofmeasurename'=>"$unitofmeasures->name", 'code'=>"$obj->code", 'vatclasseid'=>"$obj->vatclasseid", 'taxamount'=>"$obj->taxamount", 'tax'=>"$obj->tax", 'costprice'=>"$obj->costprice",'discount'=>"$obj->discount",'discountamount'=>"$obj->discountamount", 'tradeprice'=>"$obj->tradeprice", 'remarks'=>"$obj->remarks", 'total'=>"$obj->total");
	  //edit costprice
	  $edititem= mysql_query("UPDATE `inv_items` SET costprice ='$obj->costprice',lasteditedby ='".$_SESSION['userid']."',lasteditedon =now() WHERE id ='$obj->itemid'");
...
if($obj->action=="Filter"){
	if(!empty($obj->invoiceno)){
		$purchaseorders = new Purchaseorders();
		$fields="proc_purchaseorders.status,proc_purchaseorderdetails.id, proc_purchaseorders.currencyid, proc_purchaseorders.type, proc_purchaseorders.rate, proc_purchaseorders.eurorate, proc_purchaseorderdetails.unitofmeasureid, proc_purchaseorderdetails.total, proc_purchaseorderdetails.vatclasseid, proc_purchaseorderdetails.tax, proc_purchaseorderdetails.taxamount, proc_purchaseorderdetails.discount,  proc_purchaseorderdetails.discountamount,  inv_unitofmeasures.name unitofmeasurename, proc_purchaseorders.documentno, inv_items.id as itemid,inv_items.name itemname, assets_categorys.id as categoryid, assets_categorys.name categoryname,  proc_purchaseorderdetails.quantity, proc_purchaseorderdetails.costprice, proc_purchaseorderdetails.total, proc_purchaseorderdetails.memo, proc_purchaseorders.requisitionno, proc_suppliers.id as supplierid, proc_purchaseorders.remarks, proc_purchaseorders.orderedon, proc_purchaseorders.file, proc_purchaseorders.createdby, proc_purchaseorders.createdon, proc_purchaseorders.lasteditedby, proc_purchaseorders.lasteditedon, proc_purchaseorders.ipaddress, proc_purchaseorderdetails.expenseid, fn_expenses.name expensename";
		$join=" left join proc_purchaseorderdetails on proc_purchaseorderdetails.purchaseorderid=proc_purchaseorders.id left join inv_items on inv_items.id=proc_purchaseorderdetails.itemid left join proc_suppliers on proc_purchaseorders.supplierid=proc_suppliers.id left join inv_unitofmeasures on inv_unitofmeasures.id=proc_purchaseorderdetails.unitofmeasureid left join assets_categorys on assets_categorys.id=proc_purchaseorderdetails.categoryid left join fn_expenses on fn_expenses.id=proc_purchaseorderdetails.expenseid ";
		$having="";
		$groupby="";
		$orderby="";
		...
		while($row=mysql_fetch_object($res)){
			...
			$shppurchaseorders[$it]=array('id'=>"$row->id",'quantity'=>"$row->quantity", 'received'=>"$row->received", 'itemid'=>"$row->itemid", 'itemname'=>"$items->name", 'categoryid'=>"$row->categoryid", 'categoryname'=>"$row->categoryname", 'expenseid'=>"$row->expenseid", 'expensename'=>"$row->expensename", 'unitofmeasureid'=>"$row->unitofmeasureid", 'unitofmeasurename'=>"$row->unitofmeasurename", 'code'=>"$row->code", 'vatclasseid'=>"$row->vatclasseid", 'taxamount'=>"$row->taxamount", 'tax'=>"$row->tax", 'costprice'=>"$row->costprice", 'tradeprice'=>"$row->tradeprice", 'remarks'=>"$row->remarks", 'memo'=>"$row->memo", 'discount'=>"$row->discount", 'discountamount'=>"$row->discountamount", 'total'=>"$row->total",'createdby'=>"$obj->createdby",'createdon'=>"$obj->createdon");
...
if(!empty($id)){
	$purchaseorders=new Purchaseorders();
	$where=" where id=$id ";
	$fields="proc_purchaseorders.id, proc_purchaseorders.documentno, proc_purchaseorders.requisitionno, proc_purchaseorders.supplierid, proc_purchaseorders.remarks, proc_purchaseorders.orderedon, proc_purchaseorders.file, proc_purchaseorders.createdby, proc_purchaseorders.createdon, proc_purchaseorders.lasteditedby, proc_purchaseorders.lasteditedon, proc_purchaseorders.ipaddress";
	$join="";
	$having="";
	$groupby="";
	$orderby="";
