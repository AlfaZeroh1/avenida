...
<?php } ?>
	...
	<?php  
		...
		$groupby="";
		$orderby="";
		$where=" where hrm_employeepaiddeductions.deductionid='$obj->deductionid' and hrm_employeepaiddeductions.month='$obj->month' and hrm_employeepaiddeductions.year='$obj->year' and hrm_employeepaiddeductions.reducing='Yes'";
		if(!empty($obj->loanid))
		  $where.=" and hrm_employeepaiddeductions.loanid='$obj->loanid' ";
		$employeepaiddeductionss->retrieve($fields,$join,$where,$having,$groupby,$orderby); 
...
// 		if($i==0)echo $employeepaiddeductionss->sql;
		$i++;
		$date=date("Y-m-t",mktime(0,0,0,$obj->month,1,$obj->year));
		...
		?>
	      ...
	      // }
	       ...
	       ?>
			...
			<?php
			if(!empty($obj->loanid)){
			$date=date("Y-m-t",mktime(0,0,0,($obj->month+1),1,$obj->year));
			$today=date("Y-m-t",mktime(0,0,0,($obj->month),1,$obj->year));
			...
			$groupby="";
			$orderby="";
			$where=" where hrm_employeepaiddeductions.employeeid='$row->employeeid' and deductionid='$obj->deductionid' and loanid='$obj->loanid' and concat(year,'-',concat(LPAD(month,2,'0'),'-','28'))>='$date' and reducing='Yes'";
			...
			$employeepaiddeductionx = new Employeepaiddeductions();
			$fields="sum(hrm_employeepaiddeductions.amount) amount";
			$join="";
			$having="";
			$groupby="";
			$orderby="";
			$where=" where hrm_employeepaiddeductions.employeeid='$row->employeeid' and deductionid='$obj->deductionid' and loanid='$obj->loanid' and concat(year,'-',concat(LPAD(month,2,'0'),'-','28'))<='$today' and reducing='No'";
			$employeepaiddeductionx->retrieve($fields,$join,$where,$having,$groupby,$orderby);//if($row->employeeid=345 and $obj->loanid==16)echo $employeepaiddeductionx->sql;
			$employeepaiddeductionx=$employeepaiddeductionx->fetchObject;
			...
			<td align="center"><?php echo formatNumber($employeepaiddeductionx->amount); ?></td>
			<?php
			if($obj->breakdown==1){
			$x=1;
			$ln=0;
