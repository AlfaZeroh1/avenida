...
<th colspan="3"><hr /></th>
	  ...
	  <td >Basic Salary:</td>
	  <?php  
	  $employeepayment = new Employeepayments();
	  $fields="hrm_employeepayments.*,sum(hrm_employeepayments.basic) as basic";
	  $join="";
	  $having="";
	  $groupby=" group by hrm_employeepayments.employeeid";
	  $orderby="";
	  $where=" where employeeid='$emp->id' and year='$year'";
	  $employeepayment->retrieve($fields,$join,$where,$having,$groupby,$orderby);
	  $employeepayment=$employeepayment->fetchObject;
	  ?>
          <td align="right"><?php echo formatNumber($employeepayment->basic); ?></td>
	  <td>&nbsp;</td>
	  </tr>
	  <tr valign="top" style="font-size:12px; ">
	  <?php 
	$employeepaidarrears = new Employeepaidarrears();
	$fields="hrm_employeepaidarrears.*, sum(hrm_employeepaidarrears.amount) as amount,hrm_arrears.name";
	$join=" left join hrm_arrears on hrm_arrears.id=hrm_employeepaidarrears.arrearid ";
	$having="";
	$groupby=" group by hrm_employeepaidarrears.arrearid";
	$orderby="";
	$where=" where hrm_employeepaidarrears.employeeid='$emp->id'  and hrm_employeepaidarrears.year='$year'";
	...
	$fields="hrm_employeepaidallowances.*,sum(hrm_employeepaidallowances.amount) as amount, hrm_allowances.name allowanceid, hrm_allowances.noncashbenefit, hrm_allowances.id allowance";
	$join=" left join hrm_allowances on hrm_allowances.id=hrm_employeepaidallowances.allowanceid ";
	$having="";
	$groupby=" group by hrm_employeepaidallowances.allowanceid";
	$orderby="";
	$where=" where hrm_employeepaidallowances.employeeid='$emp->id' and hrm_employeepaidallowances.year='$year'";
	...
	while($rw=mysql_fetch_object($rs))
	  ...
	  if($rw->noncashbenefit=="Yes"){
	     $totalnoncashbenefit+=$rw->amount;
	  }
	  ?>
	  </tr>
	  <tr valign="top" style="font-size:12px; ">
	  <td ><?php echo substr(initialCap($rw->allowanceid),0,30); ?>:</td>
	  <td align="right"><?php echo formatNumber($rw->amount); ?></td>
	  <td>&nbsp;</td>
	  <?
	      }
	  ...
	  <td ></td>
	  <td>&nbsp;</td>
      <td align="right"><?php echo formatNumber($employeepayment->basic+$t+$s) ?></td>
	  </tr>
	...
	$fields="hrm_employeepaiddeductions.*,sum(hrm_employeepaiddeductions.amount) as amount,hrm_deductions.name deductionid, hrm_deductions.id ded";
	$join=" left join hrm_deductions on hrm_deductions.id=hrm_employeepaiddeductions.deductionid ";
	$having="";
	$groupby=" group by hrm_employeepaiddeductions.deductionid ";
	$orderby="";
	$where=" where hrm_employeepaiddeductions.employeeid='$emp->id' and hrm_employeepaiddeductions.year='$year'";
	...
	$res = $employeepaiddeductions->result;
	  ...
	  while($row=mysql_fetch_object($res)){
		  ...
		  ?>
		  <tr valign="top" style="font-size:12px; ">
			  <td><?php echo substr(initialCap($row->deductionid),0,30); ?>:</td>
	  ...
	  $groupby="";
	  $orderby="";
	  $where=" where hrm_employeepaidsurchages.employeeid='$emp->id'  and hrm_employeepaidsurchages.year='$year'";
	 ...
	 // $res=retrievePaidEmployeeSurchages($pay->employeeid,$pay->month,$pay->year,$pay->paydate);
		  while($row=mysql_fetch_object($res)){
		  	...
		  	?>
		  	 <tr valign="top" style="font-size:12px; ">
		  	<td >Surchage - <?php echo  substr(initialCap($row->surchageid),0,19); ?>:</td>
		  	<td align="right"><?php echo formatNumber($row->amount); ?></td>
		  	<td>&nbsp;</td>
		  	</tr>
	  ...
	  <td >Net Pay:</td>
	  <td>&nbsp;</td>
      <td align="right"><?php echo formatNumber($employeepayment->basic+$t+$s-($total+$totalnoncashbenefit)); ?></td>
	  </tr>
	...
	<?php
	$date=date("Y-m-d",mktime(0,0,0,1,$year));
	...
	$groupby="";
	$orderby="";
	$where=" where id in(select deductionid from hrm_employeepaiddeductions where employeeid='$emp->id'and year='$year') and statement='yes'";
	...
	if($row->id==4){
	  ...
	  $fields=" hrm_loans.name, hrm_employeeloans.principal,hrm_employeeloans.loanid ";
	  $join=" left join hrm_loans on hrm_loans.id=hrm_employeeloans.loanid ";
	  $having="";
	  $groupby="";
	  $orderby="";
	  $where=" where employeeid='$emp->id' and loanid in (select loanid from hrm_employeepaiddeductions where employeeid='$emp->id'and year='$year' and year not in(2014))";
...
// 	  }
	...
	?>	
	  ...
	  <td><?php echo formatNumber($employeepaiddeductions->amount); ?></td>
	  <td align="right">&nbsp;</td>
	  </tr>
	  <tr valign="top" style="font-size:12px; ">	
	  <td ><?php echo substr($row->name,0,30); ?> (Employer Contribution):</td>
	  <td><?php echo formatNumber($employeepaiddeductions->employeramount); ?></td>
	  <td align="right">&nbsp;</td>
	  </tr>
	  <tr valign="top" style="font-size:12px; ">	
	  <td ><?php echo substr($row->name,0,30); ?> (Total Contribution):</td>
	  <td>&nbsp;</td>
	  <td align="right"><?php echo formatNumber($employeepaiddeductions->amount+$employeepaiddeductions->employeramount); ?></td>
	  </tr>	  
	<?php }else{ ?>
	<tr valign="top" style="font-size:12px; ">	
