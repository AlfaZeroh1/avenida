		...
		if(!empty($_POST[$row->id])){
			...
			while($rw=mysql_fetch_object($allowances->result)){
				...
				//check employee specific allowances
					...
					if($rw->id==3){
					  ...
					  if(!empty($employeeallowances->tomonth) and !empty($employeeallowances->toyear)  and $rw->allowancetypeid==2){
			...
			while($rw=mysql_fetch_object($deductions->result)){
				...
				if($rw->id==1){
					...
					//get NSSF
					$nssfs = new Nssfs();
					$taxable=$payes->getTaxable($grosspay, $row->id, $obj);
								...
								if($ob->amount!=0){
								  ...
								  if(strtolower($rwf->interesttype)=="amount"){
								    ...
								    if($rwf->method=="straight-line")
									  $deduction=$rwf->interest*$rwf->initialvalue*$rwf->duration/100;
