<style>
/* td{width:10% !important;text-align:left !important;} */
th{width:300px !important;text-align:left !important;}
/*table.dataTable,
table.dataTable td,
table.dataTable th {
...
<table style="clear:both;"  class="display" id="tbl" >
	<thead>
		<tr>
			<th>#</th>
	...
	<?php
		...
		$where=" where hrm_employees.statusid=1 and case when hrm_employees.type=1 then hrm_employees.id not in(select employeeid from hrm_employeepayments where month='$obj->month' and year='$obj->year') else 1=1 end ".$wh;
	...
	?>
		<tr>
			...
			<td><a href="../employees/addemployees_proc.php?sys=true&id=<?php echo $row->id; ?>" target="_blank"><?php echo $row->pfnum; ?>&nbsp;<?php echo $row->employeeid; ?></a></td>
			...
			if($row->type==2){
			  $basic=$row->basic*12*$employeeclockings->days/(365);
			}else{
			  $basic=$row->basic;
			}
				...
				elseif($rw->overall=="All" and ($rw->id!=1 and $rw->id!=2 and $rw->id!=3)){ 
					...
					if(!empty($rw->toyear) and !empty($rw->tomonth)){
					  if($now>=$fromdate and $now<=$todate){
							  ...
							  $where=" where hrm_employeepaiddeductions.employeeid=$row->id and hrm_employeepaiddeductions.deductionid=$rw->id and hrm_employeepaiddeductions.reducing='Yes' ";
							  $having="";
							  $groupby="";
							  ...
							  if($obj->deductions==1){
								  ...
								  if(strtolower($rww->interesttype)=="amount"){
								    ...
								    if($rww->method=="straight-line")
									    $deduction=$rww->interest*$rww->initialvalue/100;
			...
			<?php }
			$netpay=($grosspay-$totalnoncashbenefit)-$totaldeductions+$totalarrears1;
			?>
...
<tr>
	...
	&nbsp;<input type="submit" name="action" class="btn btn-success" value="Make Payment"/>&nbsp;<input type="button" name="action" value="Print Preview" onclick="Clickheretoprint('print');"/></td>
</tr>
</table>
<hr>
</form>
