...
// 		jsPrintSetup.setOption('scaling',true);
...
<?php if(PRINTTITLE==1){
 ...
 Printed on: <span style="font-size:10px;"><?php echo date("Y-m-d H:i:s")."   ".$x;?></span></div>
...
<th colspan="3"><hr /></th>
	  ...
	  $groupby="";
	  $orderby="";
	  $where=" where employeeid='$emp->id' and month='$month' and year='$year'";
	...
	$fields="hrm_employeepaidallowances.*, hrm_allowances.name allowanceid, hrm_allowances.noncashbenefit, hrm_allowances.id allowance";
	...
	$groupby="";
	$orderby="";
	$where=" where hrm_employeepaiddeductions.employeeid='$emp->id' and hrm_employeepaiddeductions.month='$month' and hrm_employeepaiddeductions.year='$year' and hrm_employeepaiddeductions.reducing='Yes' ";
	...
	$res = $employeepaiddeductions->result;
	 ...
	 // $res=retrievePaidEmployeeSurchages($pay->employeeid,$pay->month,$pay->year,$pay->paydate);
	  ...
	  <td >Net Pay:</td>
	  <td>&nbsp;</td>
      <td align="right"> <?php echo formatNumber($emp->basic+$t+$s-($total+$totalnoncashbenefit)); ?></td>
	  </tr>
	...
	$groupby="";
	$orderby="";
	$where=" where id in(select deductionid from hrm_employeepaiddeductions where employeeid='$emp->id' and month='$month' and year='$year') and statement='yes'";
	...
	if($row->id==4){
	  ...
	  $groupby="";
	  $orderby="";
	  $where=" where employeeid='$emp->id' and loanid in (select loanid from hrm_employeepaiddeductions where employeeid='$emp->id' and month='$month' and year='$year' and year not in(2014))";
	  ...
	  while($rw=mysql_fetch_object($employeeloans->result)){
		...
		$employeepaiddeduction->retrieve($fields,$join,$where,$having,$groupby,$orderby);//echo $employeepaiddeduction->sql."<br/>";
		$employeepaiddeduction=$employeepaiddeduction->fetchObject;
	    ?>
	    <tr valign="top" style="font-size:10px; ">
	      <td ><?php echo substr($rw->name,0,30); ?>:</td>
	      <td>&nbsp;</td>
	      <td align="right"><?php echo formatNumber($rw->principal+$employeepaiddeduction->amount); ?></td>
	      </tr>
	    <?php
	  }
	}
...
// 	  }
	...
	<tr valign="top" style="font-size:10px; ">
	  <td colspan='3' align="center"><?php echo COMPANY_SLOGAN; ?></td>
         ...
         <!--<tr valign="top" style="font-size:10px; line-height:700px; ">
