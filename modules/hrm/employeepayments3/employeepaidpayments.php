...
if($obj->action=="Send Payslip via E-mail"){
	...
	$join=" left join hrm_nationalitys on hrm_employees.nationalityid=hrm_nationalitys.id  left join hrm_countys on hrm_employees.countyid=hrm_countys.id  left join hrm_employeebanks on hrm_employees.employeebankid=hrm_employeebanks.id  left join hrm_bankbranches on hrm_employees.bankbrancheid=hrm_bankbranches.id  left join hrm_assignments on hrm_employees.assignmentid=hrm_assignments.id  left join hrm_grades on hrm_employees.gradeid=hrm_grades.id left join hrm_employees on hrm_employees.id=hrm_employeepayments.employeeid  ";
	$having="";
	$groupby="";
	$orderby="";
		...
		for(i=0;i<elements.length;i++){
			if(elements[i].type=="checkbox" && !isNaN(elements[i].value)  && elements[i].checked==true){
				...
				if(task=="print")
					...
					poptastic("print.php?id="+id+"&month=<?php echo $obj->month; ?>&year=<?php echo $obj->year; ?>",650,400);
			}
		}
	}
}
...
<table style="clear:both;"  class="table" id="tbl" width="100%" border="0" cellspacing="0" cellpadding="2" align="center" >
	...
	<?php
		...
		$where=" where  hrm_employees.id in(select employeeid from hrm_employeepayments where month='$obj->month' and year='$obj->year') and hrm_employeepayments.month='$obj->month' and hrm_employeepayments.year='$obj->year' ".$wh;
	...
	?>
		<tr>
			...
			<td><a href="../employees/addemployees_proc.php?sys=true&id=<?php echo $row->id; ?>" target="_blank"><?php echo $row->employeeid; ?></a></td>
			...
			$now=getDates($obj->year, $obj->month, 01);
				//check deductions that affect all
					...
					$where=" where hrm_employeepaiddeductions.employeeid=$row->id and hrm_employeepaiddeductions.deductionid=$rw->id and hrm_employeepaiddeductions.month='$obj->month' and hrm_employeepaiddeductions.year='$obj->year' and hrm_employeepaiddeductions.reducing='Yes' ";
			...
			while($wr=mysql_fetch_object($loans->result)){
				...
				$orderby="";
				//checks if deduction is still active
				$where=" where hrm_employeepaiddeductions.employeeid=$row->id and hrm_employeepaiddeductions.loanid=$wr->id and hrm_employeepaiddeductions.deductionid in(4) and hrm_employeepaiddeductions.month='$obj->month' and hrm_employeepaiddeductions.year='$obj->year' and hrm_employeepaiddeductions.reducing='Yes' ";
				$employeepaiddeductions->retrieve($fields,$join,$where,$having,$groupby,$orderby);//echo mysql_error();if($row->id==512)echo $employeepaiddeductions->sql."<br/>";
				...
				$orderby="";
				//checks if deduction is still active
				$where=" where hrm_employeepaiddeductions.employeeid=$row->id and hrm_employeepaiddeductions.loanid=$wr->id and hrm_employeepaiddeductions.deductionid in(5) and hrm_employeepaiddeductions.month='$obj->month' and hrm_employeepaiddeductions.year='$obj->year' and hrm_employeepaiddeductions.reducing='Yes' ";
				$employeepaiddeductions->retrieve($fields,$join,$where,$having,$groupby,$orderby);//if($row->id==512 and $wr->id==16)echo $employeepaiddeductions->sql."<br/>";
				if($employeepaiddeductions->affectedRows>0){
					while($rw=mysql_fetch_object($employeepaiddeductions->result)){					
			...
			$netpay=($grosspay-$totalnoncashbenefit)-$totaldeductions;
// 			mysql_query("insert into tmp(employeeid,netpay) values('$row->id','$netpay')");
			?>
...
<tr>
	...
	<td><input type="button" name="action" value="Print Annual Payslip" onclick="Clickheretoprint('print1') "/></td>
</tr>
</table>
</form>
<?php
