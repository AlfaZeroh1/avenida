...
if($obj->action=="Update Status"){
  ...
  if(!empty($obj->statusid)){
    if(!empty($where))
      ...
      $where.=" where ";
    $where.=" hrm_employees.statusid='$obj->statusid' ";
  }
  if(!empty($obj->type)){
    if(!empty($where))
      $where.=" and ";
      ...
      $where.=" where ";
    $where.=" hrm_employees.type='$obj->type' ";
  }
  $employees->retrieve($fields, $join, $where, $having, $groupby, $orderby);
  
  while($rw=mysql_fetch_object($employees->result))
  {
    if(!empty($_POST[$rw->id])){
      $query="update hrm_employees set statusid='$obj->statusid2' where id='$rw->id'";
      mysql_query($query);
    }
  }
}
...
<table class="table">
  ...
  <td align="right">Department : </td>
    ...
    <td><select name="type" class="selectbox">
      ...
      <option value="3" <?php if($obj->type==3){echo"selected";}?>>Weekend Casual</option>
    </select></td>
...
<td><input type="submit" class="btn btn-primary" name="action" value="Filter"/></td>
  ...
  <tr>
    <td>Status:</td>
    <td><select name="statusid2" class="selectbox">
	  <option value="">Select...</option>
	  <?php
	    $employeestatuss=new Employeestatuss();
	    $where="  ";
	...
	<td><input type="submit" class="btn btn-warning" name="action" value="Update Status"/>
    </td>
  </tr>
</table>
