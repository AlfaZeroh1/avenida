...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="1129";//Edit
}
else{
	$auth->roleid="1127";//Add
}
...
if($obj->action=="Save"){
	...
	$error=$employeebanks->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		$employeebanks=$employeebanks->setObject($obj);
		if($employeebanks->add($employeebanks)){
			$error=SUCCESS;
			redirect("addemployeebanks_proc.php?id=".$employeebanks->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
...
if($obj->action=="Update"){
	...
	if(!empty($error)){
		...
		$employeebanks=$employeebanks->setObject($obj);
		if($employeebanks->edit($employeebanks)){
			$error=UPDATESUCCESS;
			redirect("addemployeebanks_proc.php?id=".$employeebanks->id."&error=".$error);
		}
		else{
