...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="9264";//Edit
}
else{
	$auth->roleid="9262";//Add
}
...
if($obj->action=="Save"){
	...
	$error=$employeeleavedays->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		$employeeleavedays=$employeeleavedays->setObject($obj);
		if($employeeleavedays->add($employeeleavedays)){
			$error=SUCCESS;
			redirect("addemployeeleavedays_proc.php?id=".$employeeleavedays->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
...
if($obj->action=="Update"){
	...
	if(!empty($error)){
		...
		$employeeleavedays=$employeeleavedays->setObject($obj);
		if($employeeleavedays->edit($employeeleavedays)){
			$error=UPDATESUCCESS;
			redirect("addemployeeleavedays_proc.php?id=".$employeeleavedays->id."&error=".$error);
		}
		else{
...
$groupby="";
$having="";
$where=" where name='hrm_employeeleavedays' and status=1" ;
...
include "addemployeeleavedays.php";
?>