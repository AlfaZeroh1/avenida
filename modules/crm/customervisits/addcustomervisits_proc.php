...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="8487";//Edit
}
else{
	$auth->roleid="8487";//Add
}
...
if($obj->action=="Save"){
	...
	$error=$customervisits->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		$customervisits=$customervisits->setObject($obj);
		if($customervisits->add($customervisits)){
			$error=SUCCESS;
			redirect("addcustomervisits_proc.php?id=".$customervisits->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
...
if($obj->action=="Update"){
	...
	if(!empty($error)){
		...
		$customervisits=$customervisits->setObject($obj);
		if($customervisits->edit($customervisits)){
			$error=UPDATESUCCESS;
			redirect("addcustomervisits_proc.php?id=".$customervisits->id."&error=".$error);
		}
		else{
