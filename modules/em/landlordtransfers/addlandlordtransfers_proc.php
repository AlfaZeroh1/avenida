...
if($obj->action=="Save"){
	...
	$shplandlordtransfers=$_SESSION['shplandlordtransfers'];
	$error=$landlordtransfers->validates($obj);
	if(!empty($error)){
		$error=$error;
	}
	elseif(empty($shplandlordtransfers)){
		$error="No items in the sale list!";
	}
	else{
		$landlordtransfers=$landlordtransfers->setObject($obj);
		$landlordtransfers->landlordname=$obj->landlordname;
		if($landlordtransfers->add($landlordtransfers,$shplandlordtransfers)){
			$error=SUCCESS;
			//redirect("addlandlordpayments_proc.php?id=".$landlordtransfers->id."&error=".$error);
			$saved="Yes";
		}
		else{
			$error=FAILURE;
...
if($obj->action=="Update"){
	...
	$error=$landlordtransfers->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		...
		$shplandlordtransfers=$_SESSION['shplandlordtransfers'];
		$landlordtransfers->landlordname=$obj->landlordname;
		if($landlordtransfers->edit($landlordtransfers,"",$shplandlordtransfers)){
			$error=UPDATESUCCESS;
			$saved="Yes";
			//redirect("addlandlordpayments_proc.php?id=".$landlordtransfers->id."&error=".$error);
		}
...
if($obj->action2=="Add"){
	...
	;
	$shplandlordtransfers[$it]=array('paymenttermid'=>"$obj->paymenttermid", 'paymenttermname'=>"$paymentterms->name", 'amount'=>"$obj->amount", 'plotid'=>"$obj->plotid", 'plotname'=>"$plots->name", 'month'=>"$obj->month", 'year'=>"$obj->year", 'remarks'=>"$obj->remarks", 'total'=>"$obj->total");

 	$it++;
		$obj->iterator=$it;
 	$_SESSION['shplandlordtransfers']=$shplandlordtransfers;
...
if(!empty($id)){
	...
	$obj=$landlordtransfers->fetchObject;
...
if($obj->action2=="Filter"){
	if(!empty($obj->voucherno)){
		$landlordtransfers = new Landlordtransfers();
		$fields="em_landlordtransfers.id, em_landlordtransfers.documentno, em_landlords.id as landlordid, em_plots.id as plotid, em_plots.name plotname, em_paymentterms.id as paymenttermid, em_paymentterms.name paymenttermname, sys_paymentmodes.id as paymentmodeid, fn_banks.id as bankid, em_landlordtransfers.chequeno, em_landlordtransfers.amount, em_landlordtransfers.paidon, em_landlordtransfers.month, em_landlordtransfers.year, em_landlordtransfers.receivedby, em_landlordtransfers.remarks";
		$join=" left join em_landlords on em_landlordtransfers.landlordid=em_landlords.id  left join em_plots on em_landlordtransfers.plotid=em_plots.id  left join em_paymentterms on em_landlordtransfers.paymenttermid=em_paymentterms.id  left join sys_paymentmodes on em_landlordtransfers.paymentmodeid=sys_paymentmodes.id  left join fn_banks on em_landlordtransfers.bankid=fn_banks.id ";
		$having="";
		$groupby="";
		$orderby="";
		...
		while($row=mysql_fetch_object($res)){
			...
			$ob=$row;
			$shplandlordtransfers[$it]=array('paymenttermid'=>"$row->paymenttermid", 'paymenttermname'=>"$row->paymenttermname", 'amount'=>"$row->amount", 'plotid'=>"$row->plotid", 'plotname'=>"$row->plotname", 'month'=>"$row->month", 'year'=>"$row->year", 'remarks'=>"$row->remarks", 'total'=>"$row->total");
...
if(empty($id) and empty($obj->action)){
	if(empty($_GET['edit']) and empty($obj->retrieve)){
		...
		$defs=mysql_fetch_object(mysql_query("select (max(documentno)+1) as documentno from em_landlordtransfers"));
...
include "addlandlordtransfers.php";
?>