<?php 
require_once("../../../DB.php");
class LandlordtransfersDBO extends DB{
	var $fetchObject;
	var $sql;
	var $id;
	var $result;
	var $affectedRows;
 var $table="em_landlordtransfers";

	function persist($obj){
		$sql="insert into em_landlordtransfers(id,documentno,landlordid,plotid,paymenttermid,paymentmodeid,bankid,chequeno,amount,paidon,month,year,receivedby,remarks,ipaddress,createdby,createdon,lasteditedby,lasteditedon)
	...
	function delete($obj,$where=""){			
		...
		$sql="delete from em_landlordtransfers $where ";
		$this->sql=$sql;
		if(mysql_query($sql,$this->connection))		
				return true;	
	}			
	...
	function retrieve($fields,$join,$where,$having,$groupby,$orderby){			
		$sql="select $fields from em_landlordtransfers $join $where $groupby $having $orderby"; 
 		$this->sql=$sql;
