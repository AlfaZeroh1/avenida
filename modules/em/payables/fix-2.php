...
$fields="distinct documentno, landlordid ";
$where="   ";
$join="";
$having="";
$groupby="  ";
...
while($rw=mysql_fetch_object($landlordpayments->result)){
	...
	$fields="* ";
	$where=" where documentno='$rw->documentno'  and landlordid='$rw->landlordid' ";
	$join="";
	$having="";
	$groupby="  ";
	$orderby="";
	...
	$it=0;
	$shplandlordpayments=array();
	while($row = mysql_fetch_object($landlordpayment->result)){
	  ...
	  $plots = new Plots();
	  $fields=" concat(em_plots.code,' ',em_plots.name) name, em_landlords.id landlordid, concat(em_landlords.llcode,' ',concat(em_landlords.firstname,' ', concat(em_landlords.middlename,' ',em_landlords.lastname))) landlordname ";
	  ...
	  $shplandlordpayments[$it]=array('paymenttermid'=>"$obj->paymenttermid",'landlordname'=>"$plots->landlordname", 'landlordid'=>"$plots->landlordid", 'paymenttermname'=>"$paymentterms->name", 'amount'=>"$obj->amount", 'plotid'=>"$obj->plotid", 'plotname'=>"$plots->name", 'month'=>"$obj->month", 'year'=>"$obj->year", 'remarks'=>"$obj->remarks", 'total'=>"$obj->total");
	...
	$ln = $ln->setObject($obj);
	$ln->add($ln, $shplandlordpayments,true,false);
}
