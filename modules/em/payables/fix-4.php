...
$fields="distinct jvno ";
$where=" where documentno='' and transactionid in(0,16)  ";
$join="";
$having="";
$groupby="  ";
$orderby=" order by jvno ";
$generaljournals->retrieve($fields, $join, $where, $having, $groupby, $orderby,"jk1");
		
while($rw=mysql_fetch_object($generaljournals->result)){

	echo "Journal Vouchers: ".$rw->jvno."\n";

	$gn = new Generaljournals();
	$fields="* ";
	$where=" where jvno='$rw->jvno' and documentno='' and transactionid in(0,16)  ";
	$join="";
	$having="";
	$groupby="  ";
	$orderby="";
	$gn->retrieve($fields, $join, $where, $having, $groupby, $orderby,"jk1");
	
	$it=0;
	$shpgeneraljournals=array();
	while($row = mysql_fetch_object($gn->result)){
	
	  $obj = $row;
	  $obj->jvno='';
	  
	  $shpgeneraljournals[$it]=array('accountid'=>"$obj->accountid", 'accountname'=>"$obj->account", 'memo'=>"$obj->memo", 'remarks'=>"$obj->remarks", 'debit'=>"$obj->debit", 'credit'=>"$obj->credit", 'total'=>"$obj->total",'transactdate'=>"$obj->transactdate");
	...
	$fields="*";
	$where=" where lower(replace(name,' ',''))='journalvouchers'";
	$join="";
	$having="";
	$groupby="";
	$orderby="";
	...
	$generaljournal=$generaljournal->setObject($obj);
	$generaljournal->retrieve=$obj->retrieve;
	$generaljournal->add($generaljournal,$shpgeneraljournals);
}
