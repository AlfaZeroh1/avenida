<title>WiseDigits: Landlordpayables </title>
<?php 
include "../../../head.php";
...
function selectPlots()
{
	...
	var xmlhttp;
	var url="../landlordreceipts/populate.php?id="+id;
...
{ 
	...
	if(ans)
	{
		poptastic("print.php?doc=<?php echo $obj->documentno; ?>&landlordid=<?php echo $obj->landlordid; ?>&invoicedon=<?php echo $obj->invoicedon; ?>&copy=<?php echo $obj->retrieve; ?>",450,940);
	}
}
...
<form action="addlandlordpayables_proc.php" name="landlordpayables" class="forms" method="POST" enctype="multipart/form-data">
...
<?php
  ...
  ?>
	...
	<td><input class="btn" type="submit" name="action2" value="Add"/></td>
		...
		Paid On:<input name="invoicedon" id="invoicedon" class="date_input" size="18" readonly  value="<?php echo $obj->invoicedon; ?>" >
		</div>
...
<table class="table display" width="100%">
	...
	if($_SESSION['shplandlordpayables']){
		...
		$total+=$shplandlordpayables[$i]['amount'];
		?>
		<tr style="font-size:12px; vertical-align:text-top; ">
			<td><?php echo ($i+1); ?></td>
			<td><?php echo $shplandlordpayables[$i]['plotname']; ?> </td>
			<td><?php echo $shplandlordpayables[$i]['paymenttermname']; ?> </td>
			<td align="right"><?php echo formatNumber($shplandlordpayables[$i]['amount']); ?> </td>
			<td><?php echo getMonth($shplandlordpayables[$i]['month']); ?> </td>
			<td><?php echo $shplandlordpayables[$i]['year']; ?> </td>
			<td><?php echo $shplandlordpayables[$i]['remarks']; ?> </td>
			<td><a href='edit.php?i=<?php echo $i; ?>&action=edit&edit=<?php echo $obj->edit; ?>&obj=<?php  echo $ob; ?>&url=addlandlordpayables_proc.php'>Edit</a></td>
			<td><a href='edit.php?i=<?php echo $i; ?>&action=del&edit=<?php echo $obj->edit; ?>&obj=<?php  echo $ob; ?>&url=addlandlordpayables_proc.php'>Del</a></td>
		</tr>
		<?php
		$i++;
		$j--;
...
if($saved=="Yes")
{
    ...
    redirect("addlandlordpayables_proc.php");
  }
