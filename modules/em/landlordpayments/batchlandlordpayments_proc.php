...
if($obj->action=="Update"){
	...
	elseif($closeaccounts->affectedRows>0){
	  $error="Statement is closed!";
		...
		mysql_query("delete from em_landlordpayments where documentno='$obj->documentno'");echo "delete from em_landlordpayments where documentno='$obj->voucherno'";
		mysql_query("delete from fn_generaljournals where documentno='$obj->documentno' and transactionid=3");
...
if($obj->action=="Filter"){
	if(!empty($obj->voucherno)){
		$landlordpayments = new Landlordpayments();
		$fields="em_landlordpayments.id, em_landlordpayments.documentno, concat(em_landlords.llcode,' ',concat(em_landlords.firstname,' ',concat(em_landlords.middlename,' ',em_landlords.lastname))) landlordname, em_landlordpayments.landlordid, em_plots.id as plotid, concat(em_plots.code,' ',em_plots.name) plotname, em_landlordpayments.paymenttermid,em_paymentterms.name paymenttermname, em_landlordpayments.paymentmodeid, em_landlordpayments.bankid, em_landlordpayments.chequeno, em_landlordpayments.amount, em_landlordpayments.paidon, em_landlordpayments.month, em_landlordpayments.year, em_landlordpayments.receivedby, em_landlordpayments.remarks";
		...
		while($row=mysql_fetch_object($res)){
			...
			$ob=$row;
			$shplandlordpayments[$it]=array('landlordid'=>"$row->landlordid",'landlordname'=>"$row->landlordname",'plotid'=>"$row->plotid",'plotname'=>"$row->plotname",'paymenttermid'=>"$row->paymenttermid", 'paymenttermname'=>"$row->paymenttermname", 'amount'=>"$row->amount", 'plotid'=>"$row->plotid", 'plotname'=>"$row->plotname", 'month'=>"$row->month", 'year'=>"$row->year", 'remarks'=>"$row->remarks", 'total'=>"$row->total");
