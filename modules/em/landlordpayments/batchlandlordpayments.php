...
$().ready(function() {
  $("#landlordname").autocomplete({
	source:"../../../modules/server/server/search.php?main=em&module=landlords&field=concat(concat(em_landlords.firstname,' ',em_landlords.middlename),' ',em_landlords.lastname)",
...
<form action="batchlandlordpayments_proc.php" name="landlordpayments" class="forms" method="POST" enctype="multipart/form-data">
<table align='center'>
<?php if(!empty($obj->retrieve)){?>
		<tr>
			<td colspan="4" align="center"><input type="hidden" name="retrieve" value="<?php echo $obj->retrieve; ?>"/>Voucher No:<input type="text" size="4" name="voucherno"/>&nbsp;<input type="submit" name="action" value="Filter"/></td>
		</tr>
		<?php }?>
			<tr>
			<td>
...
<?php
	...
	?>
		...
		<input type="text" name="amount" id="amount" value="<?php echo $obj->amount; ?>"></td>
...
<table class="table display" width="100%">
	...
	if($_SESSION['shplandlordpayments']){
		...
		<tr style="font-size:12px; vertical-align:text-top; ">
			<td><?php echo ($i+1); ?></td>
			<td><?php echo $shplandlordpayments[$i]['landlordname']; ?> </td>
			<td><?php echo $shplandlordpayments[$i]['plotname']; ?> </td>
			<td><?php echo $shplandlordpayments[$i]['paymenttermname']; ?> </td>
			<td><?php echo $shplandlordpayments[$i]['amount']; ?> </td>
			<td><?php echo $shplandlordpayments[$i]['month']; ?> </td>
			<td><?php echo $shplandlordpayments[$i]['year']; ?> </td>
			<td><?php echo $shplandlordpayments[$i]['remarks']; ?> </td>
			<td><a href='edit.php?i=<?php echo $i; ?>&action=edit&edit=<?php echo $obj->edit; ?>&obj=<?php  echo $ob; ?>&url=batchlandlordpayments_proc.php'>Edit</a></td>
			<td><a href='edit.php?i=<?php echo $i; ?>&action=del&edit=<?php echo $obj->edit; ?>&obj=<?php  echo $ob; ?>&url=batchlandlordpayments_proc.php'>Del</a></td>
		</tr>
		<?php
		$i++;
		$j--;
...
if($saved=="Yes")
{
    ...
    redirect("batchlandlordpayments_proc.php");
  }
