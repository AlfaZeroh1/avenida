...
if($obj->action=="Save"){
	...
	elseif(empty($shptenantpayments)){
		...
		$tenantpayments->imprestaccountid=$obj->imprestaccountid;
		$tenantpayments->lplotid=$obj->lplotid;
...
if($obj->action=="Update"){
	...
	if(!empty($error)){
		...
		$tenantpayments->lplotid=$obj->lpotid;
...
if($obj->action2=="Add"){
	...
	$houses = new Houses();
	$fields=" em_houses.*, em_plots.commission, em_plots.commissiontype ";
	$join="";
	$groupby="";
	$having="";
	...
	if($paymentterms->chargemgtfee=="Yes"){
	  $obj->commission=$houses->commission;
	  $obj->commissiontype=$houses->commissiontype;
	}
	
	$obj->total+=$obj->amount;
	$shptenantpayments[$it]=array('paymenttermid'=>"$obj->paymenttermid",'houseid'=>"$obj->houseid", 'housename'=>"$houses->name", 'paymenttermname'=>"$paymentterms->name", 'amount'=>"$obj->amount", 'remarks'=>"$obj->remarks", 'month'=>"$obj->month", 'year'=>"$obj->year", 'commission'=>"$obj->commission", 'commissiontype'=>"$obj->commissiontype", 'total'=>"$obj->total",'createdby'=>"$obj->createdby",'createdon'=>"$obj->createdon");
...
if($obj->action2=="Filter"){
	if(!empty($obj->receiptno)){
		$tenantpayments = new Tenantpayments();
		$fields="em_tenantpayments.id, em_tenantpayments.tenantid, em_tenantpayments.commission, em_tenantpayments.commissiontype, em_tenantpayments.houseid, em_tenantpayments.documentno, em_paymentterms.id as paymenttermid, em_paymentterms.name paymenttermname, sys_paymentmodes.id as paymentmodeid, fn_banks.id as bankid, em_tenantpayments.chequeno, em_tenantpayments.amount, em_tenantpayments.paidon, em_tenantpayments.month, em_tenantpayments.year, em_tenantpayments.paidby, em_tenantpayments.remarks, em_tenantpayments.createdby, em_tenantpayments.createdon";
		...
		while($row=mysql_fetch_object($res)){
			...
			$ob=$row;
			$shptenantpayments[$it]=array('paymenttermid'=>"$ob->paymenttermid",'houseid'=>"$ob->houseid", 'housename'=>"$ob->name", 'paymenttermname'=>"$ob->paymenttermname", 'amount'=>"$ob->amount", 'remarks'=>"$ob->remarks", 'month'=>"$ob->month", 'year'=>"$ob->year", 'total'=>"$ob->total",'createdby'=>"$ob->createdby",'createdon'=>"$ob->createdon",'commission'=>"$obj->commission", 'commissiontype'=>"$obj->commissiontype");
