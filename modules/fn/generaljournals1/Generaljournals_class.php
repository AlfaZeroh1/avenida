...
{				
	...
	function edit($obj,$where="",$shop){
		...
		$total=0;
		while($i<$num){
			$obj->accountid=$shop['accountid'];
			$obj->accountname=$shop['accountname'];
			$obj->memo=$shop['memo'];
			$obj->debit=$shop['debit'];
			$obj->credit=$shop['credit'];
			$obj->memo=$shop['memo'];
			$obj->balance=$shop['balance'];
			$obj->remarks=$shop['remarks'];
			$obj->jvno=$shop['jvno'];
			$obj->transactdate=$shop['transactdate'];
			$obj->transactionid=$shop['transactionid'];
			$obj->class=$shop['class'];
			$obj->documentno=$shop['documentno'];
			$obj->mode=$shop['mode'];
			if($generaljournalsDBO->update($obj,$where)){
				$this->sql=$generaljournalsDBO->sql;
			}
		}
		return true;	
	...
	function validate($obj){
		if(empty($obj->transactionid)){
			$error="Transaction should be provided";
		}
