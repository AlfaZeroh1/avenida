...
$().ready(function() {
  $("#customername").autocomplete({
	source:"../../../modules/server/server/search.php?main=crm&module=customers&field=crm_customers.name&extra=sys_currencys.name&extratitle=currency&join=left join sys_currencys on sys_currencys.id=crm_customers.currencyid",
...
//  womAdd('getPaymentModes("<?php echo $obj->purchasemodeid; ?>")');
 ...
 function setInvoices(id,checked, invrate)
  {	
	  ...
	  var xmlhttp;
	  var url="setInvoices.php?invoiceno="+invoiceno+"&amount="+amount+"&checked="+checked+"&rate="+rate+"&eurorate="+eurorate+"&invrate="+invrate+"&id="+id;
	  xmlhttp=GetXmlHttpObject();
...
<form class="forms" id="theform" action="addcustomerpayments_proc.php" name="customerpayments" method="POST" enctype="multipart/form-data">
	<table width="100%" class="" border="0" align="center" cellpadding="0" cellspacing="0" id="" style="padding:0px;">
 <?php if(!empty($obj->retrieve)){?>
	<tr>
		<td colspan="4" align="center">
		<input type="hidden" name="retrieve" value="<?php echo $obj->retrieve; ?>"/>
		Document No:<input type="text" size="4" name="invoiceno"/>&nbsp;
		<input type="submit" name="action" value="Filter"/></td>
	</tr>
	<?php }?>
			<tr>
				<td><label>Customer:</label></td>
				<td><input type='text' size='32' name='customername' id='customername' value='<?php echo $obj->customername; ?>'>
			...
			<tr>
				...
				<input type="text" readonly size="5" readonly name="currencyname" id="currencyname" value="<?php echo $currencys->name; ?>"/>
					      ...
					      <input type="text" size="5" readonly name="exchangerate" id="exchangerate" value="<?php echo $obj->exchangerate; ?>"/>
					      <input type="text" size="5" readonly name="exchangerate2" id="exchangerate2" value="<?php echo $obj->exchangerate2; ?>"/></td>
			</tr>
			<tr>
				<td><label>Amount:</label></td><td><input type='text' name='amountpaid' id='amountpaid' size='8' value='<?php echo $obj->amountpaid; ?>'/></td>
				<td>&nbsp;</td>
	...
	if($obj->action2=="Retrieve"){
	 ...
	 //get invoices to pay from oldest to newest
	  ...
	  $where=" where fn_generaljournalaccounts.refid='$obj->customerid' and fn_generaljournalaccounts.acctypeid=29 and fn_generaljournals.transactdate>='2015-07-01' and fn_generaljournals.transactdate!='0000-00-00' and fn_generaljournals.balance>0 and fn_generaljournals.debit>0 ";
	  $having="";
	  ...
	  while($rw=mysql_fetch_object($generaljournals->result)){
	    ...
	    if($amount>0){
	      $diff = ($rw->rate-$obj->rate)*$rw->balance;
	 ...
	 $_SESSION['shpinvoices']=$shpinvoices;
	  ...
	  $fields=" fn_generaljournals.id, fn_generaljournals.remarks,fn_generaljournals.transactdate, fn_generaljournals.documentno,fn_generaljournals.rate, fn_generaljournals.memo, fn_generaljournals.reconstatus, fn_generaljournals.balance, fn_generaljournals.debit, fn_generaljournals.credit, fn_generaljournals.transactionid, sys_transactions.name transaction ";
	  $join=" left join fn_generaljournalaccounts on fn_generaljournalaccounts.id=fn_generaljournals.accountid left join sys_transactions on sys_transactions.id=fn_generaljournals.transactionid ";
	  $where=" where fn_generaljournalaccounts.refid='$obj->customerid' and fn_generaljournalaccounts.acctypeid=29 and fn_generaljournals.transactdate>='2015-07-01' and fn_generaljournals.transactdate!='0000-00-00'";
	  $having="";
...
// 	  $obj->amount=$obj->amountpaid;
	  ...
	  if($row->transactionid==20){
	    
	    $query="select sum(credit) credit from fn_generaljournals where transactionid=24 and documentno in (select documentno from pos_returninwards where invoiceno='$row->documentno')";
	    $crds=mysql_fetch_object(mysql_query($query));
	    
	    //get payment
	    $pay = mysql_fetch_object(mysql_query("select sum(amount) amount from fn_customerpaidinvoices where invoiceno='$row->documentno'"));
	  }
	  
	  if($row->balance<=0 and $row->debit>0)
	    $color="green";
	    
// 	  if($row->balance>0 and $amount>0 and $row->debit>0){
// 	  
// 	    $balance=true;    
// 	    
...
// 	  }
	  ...
	  if($row->balance>0 and $row->debit>0 and $obj->amountpaid>0){
	 ...
	 <?php if($obj->unpaid==1 and $row->balance<=0){}else{ ?>
	  <tr style="font-size:12px; vertical-align:text-top; color:<?php echo $color; ?>;">
		  <td><?php echo ($i); ?></td>
		  <td><?php if($row->debit>0 and $row->balance>0){ ?>
		  ...
		  <td><?php echo $row->documentno; ?> 
		  <input type="hidden" name="inv<?php echo $row->id; ?>" id="inv<?php echo $row->id; ?>" value="<?php echo $row->documentno;?>"/></td>
...
<form action="addcustomerpayments_proc.php" name="customerpayments" method="POST" enctype="multipart/form-data">
 ...
 <?php if(!empty($obj->retrieve)){?>
	...
	<tr id="paymentmode">
			...
			<div id="bankdiv" style="float:left;">
				Bank:
				<select name='bankid' id='bankid' class="selectbox">
				<option value="">Select...</option>
				<?php
				$banks=new Banks();
				$fields="*";
			...
			<div id="transactiondiv" style="float:left;">
			  Transaction No:<input type="text" name="transactionno" value="<?php echo $obj->transactionno; ?>"/>
			</div>
			...
			<tr>
			  ...
			  <?php if(!empty($obj->retrieve)){?>
			  <input type="button" class="btn btn-default" onClick="Clickheretoprint();" value="Print" />
			  <?php }else{ ?>
