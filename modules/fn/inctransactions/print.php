...
if(!empty($plotid)){
  ...
  $tenants=$plots->fetchObject;
}

$inctransactions = new Inctransactions();
$fields=" sys_paymentmodes.name paymentmodeid,fn_banks.name bankid,fn_inctransactions.chequeno ";
$join=" left join em_paymentterms on fn_inctransactions.paymenttermid=em_paymentterms.id  left join sys_paymentmodes on fn_inctransactions.paymentmodeid=sys_paymentmodes.id  left join fn_banks on fn_inctransactions.bankid=fn_banks.id ";
$having="";
$groupby="";
$orderby="";
$where=" where fn_inctransactions.documentno='$doc' ";	
$inctransactions->retrieve($fields, $join, $where, $having, $groupby, $orderby);
$tn=$inctransactions->fetchObject;
?>
...
<span style="font-family:'tahoma';font-size:10px;" class="tel"><strong>PIN:</strong> <?php echo $_SESSION['companypin']; ?></span> <span style="font-family:'tahoma';font-size:10px;" class="tel"><strong>VAT:</strong> <?php echo $_SESSION['companyvat']; ?></span><br /><br/>
     <span style="font-family:'tahoma';font-size:10px;">
<span class="name"><strong>Customer/Property: </strong><?php echo $tenants->code; ?></span>
<span class="name"><?php echo $tenants->name; ?></span><br />
...
<table width="100%">
    ...
    <?
    $inctransactions = new Inctransactions();
    $fields="fn_inctransactions.id,  fn_payments.name paymentid, fn_incomes.name incomeid, fn_inctransactions.documentno, em_paymentterms.name as paymenttermid, sys_paymentmodes.name as paymentmodeid, fn_banks.name as bankid, fn_inctransactions.chequeno, fn_inctransactions.total amount, fn_inctransactions.incomedate paidon, fn_inctransactions.month, fn_inctransactions.year, fn_inctransactions.memo";
	$join=" left join fn_payments on fn_inctransactions.paymentid=fn_payments.id  left join fn_incomes on fn_inctransactions.incomeid=fn_incomes.id left join em_paymentterms on fn_inctransactions.paymenttermid=em_paymentterms.id  left join sys_paymentmodes on fn_inctransactions.paymentmodeid=sys_paymentmodes.id  left join fn_banks on fn_inctransactions.bankid=fn_banks.id ";
	$having="";
	$groupby="";
	$orderby="";
	$where=" where fn_inctransactions.documentno='$doc' ";
    ...
    <tr class="<?php echo $some_class; ?>">
      <td style="font-family:'tahoma';font-size:10px;" class="stitle lines"><?php echo $row->paymentid; ?><?php echo $row->incomeid; ?>&nbsp;<?php if(!empty($row->memo)){echo"[$row->memo]";}?></td>
