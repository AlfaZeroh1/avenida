...
{ 
	...
	if(ans)
	{
		poptastic("print.php?documentno=<?php echo $obj->documentno; ?>&supplier=<?php echo $obj->supplierid; ?>&paidon=<?php echo $obj->paidon; ?>&copy=<?php echo $obj->retrieve; ?>&total="+total,450,940);
	}
}
</script>
...
<form class="forms" id="theform" action="addexptransactions_proc.php" name="exptransactions" method="POST" enctype="multipart/form-data">
 ...
 <?php if(!empty($obj->retrieve)){?>
	...
	<tr>
		...
		<td><input type="text" name="amount" id="amount" onchange="calculateTotal();" onblur="calculateTotal();"  size="8" value="<?php echo $obj->amount; ?>"></td>
	...
	<td><input type="submit" name="action2" value="Add"/></td>
		...
		<table align='center' class="table">
			<tr>
			<td>Document No.:</td>
			<td><input type="text" readonly name="documentno" id="documentno"  size="4"  value="<?php echo $obj->documentno; ?>"></td>
			<td>Requisition No.:</td>
			<td><input type="text" readonly name="requisitionno" id="requisitionno"  size="4"  value="<?php echo $obj->requisitionno; ?>"></td>
			<td>Receipt No.:</td>
			<td><input type="text" name="receiptno" id="receiptno"  size="4"  value="<?php echo $obj->receiptno; ?>"></td>
			<td>Expense Date:</td>
			<td><input type="text" name="expensedate" id="expensedate" readonly class="date_input" size="12" readonly  value="<?php echo $obj->expensedate; ?>"></td>
			<td>Remarks:</td>
...
<table style="clear:both" class="table display"  cellpadding="0" align="center" width="100%" cellspacing="0">
	...
	if($_SESSION['shpexptransactions']){
		...
		<tr style="font-size:12px; vertical-align:text-top; ">
			...
			<td><?php echo round($shpexptransactions[$i]['amount'],2); ?> </td>
			<td><?php echo $shpexptransactions[$i]['memo']; ?> </td>
			<td><?php echo round($shpexptransactions[$i]['total'],2); ?> </td>
...
<table align="center" width="100%">
	<tr>
		<td colspan="2" align="center">Total Taxable:<input type="text" size='12' readonly value="<?php echo round(($total-$totaltax),2); ?>"/>&nbsp; Total Tax:<input type="text" size='12' readonly value="<?php echo round($totaltax,2); ?>"/>&nbsp;Total:<input type="text" size='12' readonly value="<?php echo round($total,2); ?>"/></td>
	</tr>
	<?php if(empty($obj->retrieve)){?>
	<tr>
