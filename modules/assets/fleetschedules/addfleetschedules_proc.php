...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="8475";//Edit
}
else{
	$auth->roleid="8475";//Add
}
...
if($obj->action=="Save"){
	...
	if(!empty($error)){
		...
		if($fleetschedules->add($fleetschedules)){
			$error=SUCCESS;
			redirect("addfleetschedules_proc.php?id=".$fleetschedules->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
