...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="7632";//Edit
}
else{
	$auth->roleid="7630";//Add
}
...
if($obj->action=="Save"){
	...
	$error=$fleetfueltypes->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		$fleetfueltypes=$fleetfueltypes->setObject($obj);
		if($fleetfueltypes->add($fleetfueltypes)){
			$error=SUCCESS;
			redirect("addfleetfueltypes_proc.php?id=".$fleetfueltypes->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
...
if($obj->action=="Update"){
	...
	if(!empty($error)){
		...
		$fleetfueltypes=$fleetfueltypes->setObject($obj);
		if($fleetfueltypes->edit($fleetfueltypes)){
			$error=UPDATESUCCESS;
			redirect("addfleetfueltypes_proc.php?id=".$fleetfueltypes->id."&error=".$error);
		}
		else{
...
if(!empty($id)){
	...
	$obj=$fleetfueltypes->fetchObject;
