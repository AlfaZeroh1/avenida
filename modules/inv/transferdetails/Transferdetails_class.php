...
// 			  $ob = $this->setObject($obj);
			  ...
			  if($transferdetailsDBO->persist($obj)){
				  ...
				  if($r->type!="External"){
				    ...
				    $query="select * from inv_compositeitems where itemid2='$obj->itemid'";//echo $query;
				    $rs = mysql_query($query);
				    if(mysql_affected_rows()==1){
				      
				      $r = mysql_fetch_object($rs);
				    
				      if(!empty($r->itemid) and $obj->receipt==1){
					
                        $obj->itemid=$r->itemid;
                        
                        $obj->quantity=$obj->quantity/$r->quantity;
				    ...
				    $branchstocks->transaction="Transfer";
				    
				    if($branchstockss->affectedRows>0){
				      
				      $branchstockss = $branchstockss->fetchObject;
				      $branchstockss->recorddate=$obj->transferedon;
				      $branchstockss->transaction="Transfer";
				      $branchstockss->documentno=$obj->documentno;
				      ...
				      $branchstocks->quantitys=($obj->quantity*-1);
...
// 		}
	...
	function edit($obj,$shop){
		...
		while($i<$num){
			  ...
			  $obj->itemname=$shop[$i]['itemname'];
			  $obj->quantity=$shop[$i]['quantity'];
			  $obj->quantityrec=$shop[$i]['quantity'];
			  $obj->memo=$shop[$i]['memo'];
			  $obj->instalcode=$shop[$i]['instalcode'];
			  $obj->crdcode=$shop[$i]['crdcode'];
			  $obj->id=$shop[$i]['id'];
			  ...
			  if(!empty($obj->id)){
				  ...
				  $query="update inv_transfers t, inv_transferdetails td set td.quantityrec='$obj->quantityrec' where t.id=td.transferid and td.itemid='$obj->itemid' and t.documentno='$obj->documentno'";
				  mysql_query($query);
			  //$ob = $this->setObject($obj);	                          
...
// 				  $br->edit($br);
                ...
                $query="select * from inv_compositeitems where itemid2='$obj->itemid'";//echo $query;
				    ...
				    if(mysql_affected_rows()==1){
				      ...
				      $r = mysql_fetch_object($rs);//print_r($r);
				  ...
				  $branchstocks->transaction="Transfer Receipt";
				  
				  if($branchstockss->affectedRows>0){
				    $branchstockss = $branchstockss->fetchObject;
				    $branchstockss->transaction="Transfer Receipt";
