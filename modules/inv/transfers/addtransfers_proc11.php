...
if($obj->action=="Save"){
	...
	elseif(empty($shptransfers)){
		...
		if($transfers->add($transfers,$shptransfers)){
			$error=SUCCESS;
			$saved="Yes";
		}
		else{
...
if($obj->action=="Update" or $obj->action=="Receive"){
	$transfers=new Transfers();
	$obj->lasteditedby=$_SESSION['userid'];
	$obj->lasteditedon=date("Y-m-d H:i:s");
...
if($obj->action2=="Add"){
	...
	while($rw=mysql_fetch_object($itemdetails->result)){	  
	  $shptransfers[$it]=array('itemid'=>"$obj->itemid",'itemdetailid'=>"$rw->id", 'serialno'=>"$rw->serialno", 'itemname'=>"$items->name", 'quantity'=>"$obj->quantity", 'code'=>"$obj->code", 'costprice'=>"$obj->costprice", 'memo'=>"$obj->memo", 'total'=>"$obj->total");
	  $it++;
	}
		$obj->iterator=$it;
...
if($obj->action=="Filter"){
	...
	//echo "miketheshit";
		...
		while($row=mysql_fetch_object($res)){
			...
			$items->retrieve($fields, $join, $where, $having, $groupby, $orderby);echo mysql_error();echo $items->sql;
			$items=$items->fetchObject;
	  
			$ob=$row;
			$row->total=$row->quantity*$row->costprice;
			
			$shptransfers[$it]=array('itemid'=>"$row->itemid", 'itemname'=>"$items->name", 'quantity'=>"$row->quantity", 'code'=>"$row->code", 'costprice'=>"$row->costprice", 'memo'=>"$row->memo", 'total'=>"$row->total");
