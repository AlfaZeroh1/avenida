...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="8539";//Edit
}
else{
	$auth->roleid="8539";//Add
}
...
if($obj->action=="Save"){
	...
	$error=$projectstocks->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		$projectstocks=$projectstocks->setObject($obj);
		if($projectstocks->add($projectstocks)){
			$error=SUCCESS;
			redirect("addprojectstocks_proc.php?id=".$projectstocks->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
...
if($obj->action=="Update"){
	...
	if(!empty($error)){
		...
		$projectstocks=$projectstocks->setObject($obj);
		if($projectstocks->edit($projectstocks)){
			$error=UPDATESUCCESS;
			redirect("addprojectstocks_proc.php?id=".$projectstocks->id."&error=".$error);
		}
		else{
...
if(!empty($id)){
	$projectstocks=new Projectstocks();
	$where=" where id=$id ";
	$fields="inv_projectstocks.id, inv_projectstocks.projectid, inv_projectstocks.itemid, inv_projectstocks.quantity, inv_projectstocks.ipaddress, inv_projectstocks.createdby, inv_projectstocks.createdon, inv_projectstocks.lasteditedby, inv_projectstocks.lasteditedon";
	$join="";
	$having="";
	$groupby="";
	$orderby="";
	$projectstocks->retrieve($fields,$join,$where,$having,$groupby,$orderby);
	$obj=$projectstocks->fetchObject;
