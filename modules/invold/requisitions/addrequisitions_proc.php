...
if($obj->action=="Filter"){
  if(!empty($obj->invoiceno)){
	...
	while($row=mysql_fetch_object($requisitions->result)){
	  ...
	  $shprequisitions[$it]=array('id'=>"$row->id",'itemid'=>"$row->itemid", 'itemname'=>"$row->itemname", 'quantity'=>"$row->quantity", 'aquantity'=>"$row->quantity", 'tquantity'=>"$aquantity", 'dquantity'=>"$dquantity" , 'reorderlevel'=>"$row->reorderlevel",'maxreorderlevel'=>"$row->maxreorderlevel",'stock'=>"$row->stock",'memo'=>"$row->memo", 'total'=>"$row->total");
  ...
  while($v<$num){
	...
	$quantity=$shop[$v]['dquantity'];
 	...
 	if($items->serialno=="Yes"){
	  ...
	  $fields="*";
	  $where=" where itemid='$items->id' and status=1 and brancheid='".$_SESSION['brancheid']."' ";
	  $join="";
	  $having="";
	  $groupby="";
...
if($obj->action=="ISSUE"){
  ...
  $num = count($shprequisitions);
	...
	$shpissuance[$k]['quantity']=$shprequisitions[$i]['toissue'];
