...
if($action=="edit"){
	...
	$obj->assetid=$shppurchaseorders[$i]['assetid'];
	$obj->assetname=$shppurchaseorders[$i]['assetname'];
	if(!empty($obj->itemid))
	{
	$obj->typeid=2;
	}else{
	...
	$obj->discount = $shppurchaseorders[$i]['discount'];
	$obj->discountamount = $shppurchaseorders[$i]['discountamount'];
