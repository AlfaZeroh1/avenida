...
// 	}
	...
	$shprequisitions[$it]=array('employeeid'=>"$obj->employeeid",'categoryid'=>"$obj->categoryid",'categoryname'=>"$categorys->name",'remarks'=>"$obj->remarks", 'itemid'=>"$obj->itemid", 'expenseid'=>"$obj->expenseid", 'expensename'=>"$expenses->name", 'unitofmeasureid'=>"$obj->unitofmeasureid",'unitofmeasurename'=>"$unitofmeasures->name", 'itemname'=>"$items->name", 'memo'=>"$obj->memo", 'costprice'=>"$obj->costprice", 'quantity'=>"$obj->quantity", 'requiredon'=>"$obj->requiredon", 'total'=>"$obj->total",'available'=>"$obj->available",'package'=>"$items->package");
...
if($obj->action=="Filter"){
	if(!empty($obj->invoiceno)){
		...
		while($row=mysql_fetch_object($res)){
			...
			$query="select sum(pos_orderdetails.quantity) quantity from pos_orders left join pos_orderdetails on pos_orders.id=pos_orderdetails.orderid where itemid='$row->itemid' and orderedon=(select max(orderedon) from proc_purchaseorders left join proc_purchaseorderdetails on proc_purchaseorders.id=proc_purchaseorderdetails.purchaseorderid where proc_purchaseorderdetails.itemid='$row->itemid') ";
