...
$desc=$items->name." ".$sizes->name." (".$obj->quantity.") - ".$j;
if(!empty($downsize)){
  $desc.=" Downsize ";
}
...
a00lt="PNG: <?php echo $str; ?>" title="PNG: <?php echo $str; ?>">
  ...
  $query="insert into post_barcodes(barcode,greenhouseid,itemid,generatedon,ipaddress,createdby, createdon, lasteditedby, lasteditedon) values('$obj->desc','$obj->greenhouseid','$obj->itemid',Now(),'".$_SERVER['REMOTE_ADDR']."','".$_SESSION['userid']."','".date("Y-m-d H:i:s")."','".$_SESSION['userid']."','".date("Y-m-d H:i:s")."')";
  mysql_query($query);
