...
$groupby="";
$orderby="";
$varietys->retrieve($fields,$join,$where,$having,$groupby,$orderby);logging($varietys->sql);
?>
<option value="">Select...</option>
<?php
while($row=mysql_fetch_object($varietys->result)){
  ?>
    <option value="<?php echo $row->id; ?>" <?php if($ob->itemid==$row->id){echo "selected";}?>><?php echo $row->name; ?></option>
  <?php
}
?>