...
function printOrderVAT($obj,$copy,$bool=false){
...
$_SESSION['print']=1;
  ...
  try {
	  ...
	  if(false){
	    $titlet="copy";
	    $titlet=str_pad($titlet,48,'-',STR_PAD_BOTH);
	    $printer -> text($titlet."\n");
	  ...
	  while($row=mysql_fetch_object($orderdetails->result)){
	    ...
	    $item = new item($row->itemname, $row->quantity."X",formatNumber($row->price),formatNumber($row->total));
	    $items[]=$item;
...
// 	  if($obj->vat==1){
	    $titles = new item("TOTAL", "","",formatNumber($total*.84));
	    $printer -> text($titles);
	    $titles = new item("VAT (16%)", "","",formatNumber($total*.16));
	    $printer -> text($titles);
// 	  }
...
//   $printer -> close();
  ...
  $copy++;
  logging($orders->orderno." : ".$copy." => ".$obj->brancheid2." = ".date("H:i:s"));
  
  if($copy<3 and $obj->brancheid2!=30)
    printOrderVAT($obj,$copy,$bool);
