...
function setShop(itemid, warmth){
  var quantity = $("#"+itemid+"-"+warmth).val();
  
  var totals=parseFloat($("#total").text());
  $.post("setorder.php",{itemid:itemid, quantity:quantity, type:1, totals:totals, warmth:warmth}, function(data){
...
//     totals=parseFloat($("#total").text());
    ...
    var totalss=totals-olditemtotal+newitemtotal;
...
function saveForm(itemid,quantity,warmth){     
    ...
    $.post( "addorders_proc.php", { action2: "Add", itemid:itemid, quantity:quantity, memo:$("#memo").val(), type:'1', iterator:$("#iterator").val(), warmth:warmth}, function(data){
...
// 	}else{
	  ...
	  if(err[0]==0){
	    tbl.fnAddData( [	      
		      err[1],		
		      "<input type='text' id='"+err[4]+"-"+err[5]+"' onChange='setShop("+err[4]+",&quot;"+err[5]+"&quot;)' size='4' value="+err[2]+">",
		      err[5],
		      total,
		      ''] );
	      ...
	      $("#"+err[4]+"-"+err[5]).keyboard({ layout: 'qwerty' });
...
<div id="tabs" class="container" style="width:95%;">	
  <ul id="tabss" class="nav nav-pills">
	...
	<li><a href="#myorders" data-toggle="tab" style="font-weight:bold; font-size:20px;">MY ORDERS</a></li>
  </ul>
...
<!--       <input id="text" size="5" class="keyboard" required type="text" placeholder=" Table No..."> -->
      <input type="text" name="tableno" has-error id="tableno" size="5"  value="<?php echo $obj->tableno; ?>">
	  ...
	  ?>
	    <tr>
	      ...
	      <td><input type="text" value="<?php echo $shop[$i]['quantity']; ?>"/></td>
	      <td>&nbsp;</td>
