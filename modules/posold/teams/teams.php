...
if($obj->action=="Build Team"){
  ...
  $error="";
  while($i<count($shop)){
    $query="select b.name, t.teamedon from pos_teams t left join sys_branches b on t.brancheid=b.id where t.brancheid='".$shop[$i]."' and t.shiftid='$ob->shiftid' and t.status=0 and t.teamedon!='$obj->teamedon' order by t.id desc ";
    $res = mysql_query($query);
    if(mysql_affected_rows()>0){
      
      $rw = mysql_fetch_object($res);
      $error.=$rw->name.": ".$rw->teamedon." is still open!";
