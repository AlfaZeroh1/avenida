...
$fields="crm_customers.name, crm_customers.address, crm_customers.code,crm_customers.freight, crm_agents.name agentid, crm_agents.address agentaddress, sys_currencys.name currencyid, sys_currencys.id currency";
...
$invoices = new Invoices();
$fields="pos_invoices.id, sum(quantity) quantity, shippedon, packingno, invoiceno,actualweight,volweight,awbno,dropoffpoint, consignee";
...
<td width="50%" colspan="7">
	...
	<div style="font-size:12px !important;"><strong>FLO ID: </strong><?php echo "3797"; ?></div>
</td>
...
<tr><td colspan="8"><hr></td></tr>
	...
	$i=0;
	    ...
	    <tr>
		  ...
		  <?php }?>
		  <td align="right"><?php echo formatNumber($pack[$i][0]['price']*$pack[$i][0]['quantity']*$pack[$i]['boxes']*(100+$pack[$i][0]['vat'])/100); ?></td>
	</tr>
	<tr>
	<?php
		  $stems=0;
		  ...
		  $s=1;
		  $total+=$pack[$i][0]['price']*$pack[$i][0]['quantity']*$pack[$i]['boxes'];
		  
		  while($s<(count($pack[$i])-1)){
		    $stems+=$pack[$i][$s]['quantity'];
		    $total+=$pack[$i][$s]['price']*$pack[$i][$s]['quantity']*$pack[$i]['boxes'];
		  ...
		  <td align="left"><?php echo formatNumber($pack[$i][$s]['price']); ?></td>
		  <?php if($inv->vatable=="Yes"){?>
		  <td align="right"><?php echo $vat; ?></td>
		  <?php }?>
		  <td align="right"><?php echo formatNumber($pack[$i][$s]['price']*$pack[$i][$s]['quantity']*$pack[$i]['boxes']*(100+$pack[$i][$s]['vat'])/100); ?></td>
		</tr>
<!--  	    <tr><td colspan="8"><hr></td></tr>  -->
	<?
	$s++;
