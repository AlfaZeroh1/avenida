...
<form  id="theform" action="addinvoices_proc.php" name="invoices" method="POST" enctype="multipart/form-data">
 ...
 <?php if(!empty($obj->retrieve)){?>
	<tr>
		...
		<input type="text" name="parent" value="<?php echo $obj->parent; ?>"/>
		<?php
		if(!empty($obj->parent)){
		?>
...
<?php
	...
	?>
		...
		Sold On:<input type="text" name="soldon" id="soldon"  class="date_input" size="16" readonly  value="<?php echo $obj->soldon; ?>">
	...
	if($_SESSION['shpinvoices']){
		...
		if(true){
		  ...
		  <tr style="font-size:12px; vertical-align:text-top; ">
			  <td><?php echo ($i+1); ?></td>
			  <?php if(!empty($obj->retrieve)){?>
			  <td><input type="checkbox" name="<?php echo $shpinvoices[$i]['id']; ?>"/></td>
			  <?php } ?>
			  ...
			  <td><?php echo $shpinvoices[$i]['boxno']; ?> </td>
...
<td><input type="text" name="quantity" id="quantity" size="16" value="<?php echo $obj->quantity; ?>" onchange="calculateTotals();"></td>
	...
	<td><input type="submit" name="action4" value="Add"/></td>
		...
		<table style="clear:both" class="tgrid display" id="myTable" cellpadding="0" align="center" width="100%" cellspacing="0">
	<thead>
	<tr style="font-size:18px; vertical-align:text-top; ">
		<th align="left" >#</th>
		<th align="left">Item  </th>
	...
	if($_SESSION['shpconsumables']){
		...
		if(true){	
		  ...
		  <tr style="font-size:12px; vertical-align:text-top; ">
			  ...
			  <td><?php echo $shpconsumables[$i]['price']; ?> </td>
			  <td><?php echo $shpconsumables[$i]['total']; ?> </td>	
			  <td><a href="edit.php?i=<?php echo $i; ?>&action=edit&edit=<?php echo $obj->edit; ?>">Edit</a></td>
			  <td><a href="edit.php?i=<?php echo $i; ?>&action=del&edit=<?php echo $obj->edit; ?>">Del</a></td>
		  </tr>
		  <?php		  
...
<table align="center" width="100%">
	...
	<tr>
		<td colspan="2" align="center"><input  class="btn btn-primary" type="submit" name="action" id="action" value="<?php echo $obj->action; ?>">&nbsp;<input  class="btn btn-danger" type="submit" name="action" id="action" value="Cancel" onclick="window.top.hidePopWin(true);"/></td><br><br>
	</tr>
	<?php if(!empty($obj->retrieve)){?>
