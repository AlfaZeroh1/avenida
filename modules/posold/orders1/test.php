...
function printOrder($obj,$copy,$bool=false, $bill=false, $brancheid2=""){
...
$_SESSION['print']=1;
  ...
  try {
	  ...
	  if($bill and $copy==1){
	    ...
	    $defs=mysql_fetch_object(mysql_query("select (max(billno)+1) billno from pos_orders"));
	    if($defs->billno == null){
		    $defs->billno=1;
	    }
	    $obj->billno=$defs->billno;
	  
	    $query="select * from pos_orders where orderno in($ordernos)";
	    $s = mysql_query($query);
	    while($r = mysql_fetch_object($s)){
	      
	      $query="update pos_orders set billno='$obj->billno', status=2 where orderno='$r->orderno'";
	      mysql_query($query);
	  ...
	  //get order
	  $orders = new Orders();
	  $fields="pos_orders.*, sys_branchess.name branchename, sys_branchess.printer, sys_branchess.printer2, group_concat(pos_orders.id) id, group_concat(distinct pos_orders.orderno) orderno, sys_branchess.printer2name ";
	  $join=" left join pos_orderdetails on pos_orderdetails.orderid=pos_orders.id  left join sys_branches sys_branchess on sys_branchess.id=pos_orderdetails.brancheid2  ";
	  $having="";
	  $groupby=" ";
	  $orderby=" ";
	  ...
	  if(!$bill and !empty($brancheid2)){
	    $where.=" and pos_orderdetails.brancheid2='$brancheid2' ";
	  }
...
// 	  if(true){
	  ...
	  $ipaddress = "smb:".$ipaddress;logging($ipaddress);
	  ...
	  if($bill)
	    $printer -> text(str_pad("BILL NO:",14,' ').$obj->billno."\n");
	  else
	  ...
	  $fields="sum(pos_orderdetails.quantity) quantity, inv_items.name itemname, pos_orderdetails.price, sum(pos_orderdetails.quantity*pos_orderdetails.price) total, inv_items.warmth war, case when pos_orderdetails.warmth=1 then 'Warm' when pos_orderdetails.warmth=2 then 'Cold' when pos_orderdetails.warmth=3 then 'WET FRY' when pos_orderdetails.warmth=4 then 'DRY FRY' when pos_orderdetails.warmth=5 then 'BOILED' when pos_orderdetails.warmth=6 then 'CHOMA' else '' end warm";
	  ...
	  $printer -> setEmphasis(FALSE);
	  $titles = new item("Catering Levy", "","",formatNumber($total-($taxable+$vat)));
	  $printer -> text($titles);
...
// 	  echo "PRINTED";
	  /* Close printer */
	  $printer -> close();
  } catch(Exception $e) {//logging($e -> getMessage());
...
//   $printer -> close();
  ...
  $copy++;
  logging($orders->orderno." : ".$copy." => ".$obj->brancheid2." = ".date("H:i:s")." == ".$_SESSION['employeename']." == ".$_SESSION['userid']);
  
  if(false){
    if($copy<3 and $obj->brancheid2!=30)
      printOrder($obj,$copy,$bool,$bill);
