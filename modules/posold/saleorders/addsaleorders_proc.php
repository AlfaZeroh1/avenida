...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="4848";//Edit
}
else{
	$auth->roleid="4846";//Add
}
...
if(empty($obj->action)){
	...
	$defs=mysql_fetch_object(mysql_query("select max(documentno)+1 documentno from pos_saleorders"));
...
if($obj->action=="Save"){
	...
	$error=$saleorders->validates($obj);
	if(!empty($error)){
		$error=$error;
	}
	elseif(empty($shpsaleorders)){
		$error="No items in the sale list!";
	}
	else{
		$saleorders=$saleorders->setObject($obj);
		if($saleorders->add($saleorders,$shpsaleorders)){
			$error=SUCCESS;
			$saved="Yes";
		}
		else{
...
if($obj->action=="Update"){
	...
	if(!empty($error)){
		...
		$saleorders=$saleorders->setObject($obj);
		$shpsaleorders=$_SESSION['shpsaleorders'];
		if($saleorders->edit($saleorders,$shpsaleorders)){
			$error=UPDATESUCCESS;
			redirect("addsaleorders_proc.php?id=".$saleorders->id."&error=".$error);
		}
		else{
			$error=UPDATEFAILURE;
		}
...
if($obj->action2=="Add"){
	...
	;
	$shpsaleorders[$it]=array('quantity'=>"$obj->quantity", 'itemid'=>"$obj->itemid", 'total'=>"$obj->total");
