...
if($obj->action=="Save"){
	...
	elseif(empty($shpinwards)){
		...
		if($inwards->add($inwards,$shpinwards,true)){
			$error=SUCCESS;
			$saved="Yes";
		}
		else{
...
if($obj->action=="Update" or $obj->action=="Revoke"){
	$inwards=new Inwards();
	$obj->lasteditedby=$_SESSION['userid'];
	$obj->lasteditedon=date("Y-m-d H:i:s");
...
if($obj->action=="Filter"){
	if(!empty($obj->invoiceno)){
		...
		while($row=mysql_fetch_object($res)){
			...
			$shpinwards[$it]=array('id'=>"$row->id",'quantitys'=>"$row->quantity", 'itemid'=>"$row->itemid", 'itemname'=>"$ob->itemname", 'assetid'=>"$row->assetid", 'assetname'=>"$ob->assetname", 'code'=>"$row->code", 'discount'=>"$row->discount", 'discountamount'=>"$row->discountamount", 'vatclasseid'=>"$row->vatclasseid", 'tax'=>"$row->tax", 'vatamount'=>"$row->vatamount", 'costprice'=>"$row->costprice", 'tradeprice'=>"$row->tradeprice", 'remarks'=>"$row->remarks", 'total'=>"$row->total",'createdby'=>"$obj->createdby",'createdon'=>"$obj->createdon",'categoryid'=>"$row->categoryid", 'categoryname'=>"$row->categoryname");
		...
		if($obj->type=="out"){
		  $obj->inwardno=$obj->documentno;
		  $defs=mysql_fetch_object(mysql_query("select (max(documentno)+1) documentno from proc_inwards where type='$obj->type'"));
...
if($obj->action3=="New Inward"){
  ...
  while($i<$num){
    ...
    assets_categorys.id as assetid, assets_categorys.name assetname, proc_purchaseorderdetails.quantity, proc_purchaseorderdetails.discount, proc_purchaseorderdetails.discountamount, proc_purchaseorderdetails.costprice, proc_purchaseorderdetails.total, proc_purchaseorderdetails.vatclasseid, proc_purchaseorderdetails.tax, proc_purchaseorderdetails.memo, proc_purchaseorders.documentno, proc_suppliers.id as supplierid, proc_purchaseorders.remarks, proc_purchaseorders.orderedon, proc_purchaseorders.file, proc_purchaseorders.createdby, proc_purchaseorders.createdon, proc_purchaseorders.lasteditedby, proc_purchaseorders.lasteditedon, proc_purchaseorders.ipaddress, inv_items.package";
    ...
    while($row=mysql_fetch_object($res)){
		...
		while($x<$quantity){
		  ...
		  $shpinwards[$it]=array('id'=>false,'checked'=>$checked,'purchaseorderdetailid'=>"$row->purchaseorderdetailid",'quantity'=>"1",'quantitys'=>"1", 'itemid'=>"$row->itemid", 'itemname'=>"$items->name",'assetid'=>"$row->assetid", 'assetname'=>"$row->assetname", 'categoryid'=>"$items->categoryid", 'categoryname'=>"$items->category", 'code'=>"$row->code",'discount'=>"$row->discount", 'discountamount'=>"$row->discountamount", 'tax'=>"$row->tax", 'vatclasseid'=>"$row->vatclasseid", 'vatamount'=>"$vatamount", 'costprice'=>"$row->costprice", 'tradeprice'=>"$row->tradeprice", 'remarks'=>"$row->remarks", 'total'=>"$row->total",'createdby'=>"$obj->createdby",'createdon'=>"$obj->createdon",'package'=>"$row->package");
  ...
  $query="select (max(documentno)+1) documentno from proc_inwards where type='$obj->type'";//echo $query;
