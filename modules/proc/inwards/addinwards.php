...
$().ready(function() {
  $("#itemname").autocomplete({
	source:"../../../modules/server/server/search.php?main=inv&module=items&field=name&where=status='Active' and (type!='complex' or type is null)",
...
{ 
	var msg;
	msg="Do you want to print <?php if($obj->type=='in'){?>GRN<?php }else{ ?>RETURN<?php }?>?";
	var ans=confirm(msg);
	if(ans)
	{
...
function changeQuantity(id,value,amount, packages){
  
  value = value*packages;
  amount = amount*packages;
  
  if(value>amount){
    alert("Cannot receive more than ordered!");
  }else{
...
<form  id="theform" action="addinwards_proc.php" name="inwards" method="POST" enctype="multipart/form-data">
 ...
 <?php if(!empty($obj->retrieve)){?>
	<tr>
		...
		<input type="hidden" name="jvno" value="<?php echo $obj->jvno; ?>"/>
		<input type="hidden" name="return" value="<?php echo $obj->return; ?>"/>Document No:<input type="text" size="4" name="invoiceno"/>&nbsp;<input type="submit" name="action" value="Filter"/></td>
	</tr>
	<?php }?>
			<!--<tr>
				<td><label>Project:</label><input type="hidden" name="inwardstatus" value="<?php echo $obj->inwardstatus; ?>"/></td>
			...
			<tr>
			  ...
			  <td><!--<select name="currencyid" id="currencyid" class="selectbox" onchange="getExchangeRate();">
				...
				?>
			      ...
			      <input type="hidden" size='6' readonly name="currencyid" id="currencyid" value="<?php echo $obj->currencyid; ?>"/>
			      <input type="hidden" size='6' readonly name="rate" id="rate" value="<?php echo $obj->rate; ?>"/>
			      <input type="hidden" size='6' readonly name="eurorate" id="eurorate" value="<?php echo $obj->eurorate; ?>"/>
			</tr>
		</table>
	<table width="100%" class="table titems gridd" border="0" align="center" cellpadding="2" cellspacing="0" id="tblSample">
	<tr>
	...
	<tr>
		...
		<input type="text" name="wquantity" id="wquantity" readonly onchange="getPieces();calculateTotal();" onblur="getPieces();calculateTotal();"  size="6" value="<?php echo $obj->wquantity; ?>"></td>
	...
	<td><input type="submit" name="action2" value="Add"/></td>
		...
		<table align='center' class="table">
			<tr>
			<td><?php if($obj->type=="in"){echo "Inward";}else{echo "Return";}?> Note No:</td>
			<td><input type="text" name="documentno" id="documentno" readonly size="8"  value="<?php echo $obj->documentno; ?>"></td>
			<td><?php if($obj->type=="in"){echo "Inward";}else{echo "Return";}?> Date:<input type="date" onChange="getExchangeRate();" name="inwarddate" id="inwarddate" readonly class="date_input" size="12" readonly  value="<?php echo $obj->inwarddate; ?>"></td>
			<td>Remarks :</td>
			<td><textarea name="remarks" ><?php echo $obj->remarks; ?></textarea></td>
			<td>LPO No:</td>
			<td><textarea name="lpono" ><?php echo $obj->lpono; ?></textarea></td>
			<?php if($obj->type=="out"){?>
			<td>GRN No:</td>
			<td><textarea name="inwardno" readonly ><?php echo $obj->inwardno; ?></textarea></td>
			<?php } ?>
			<td>Del No:</td>
			<td><input type="text" size="8" name="deliverynoteno" value="<?php echo $obj->deliverynoteno; ?>"/></td>
			</tr>
		</table>
		<div style="clear:both;">
<table style="clear:both" id="" class="table display" cellpadding="0" align="center" width="100%" cellspacing="0">
	<thead>
	<tr style="font-size:18px; vertical-align:text-top; ">
		...
		<th align="left">Qnt <?php if($obj->type=="in"){?>Received<?php }else{?>Returned<?php } ?>  </th>
		<th align="left">Qnt <?php if($obj->type=="in"){?>Receivable<?php }else{?>Returnable<?php } ?>  </th>
	...
	if($_SESSION['shpinwards']){
		...
		<tr style="font-size:12px; vertical-align:text-top; ">
			<td><?php echo ($i+1); ?></td>
			<td>
			<?php if(!$shpinwards[$i]['checked'] and empty($obj->retrieve)){}else{?>
			<input type="checkbox" id="id<?php echo $i; ?>" onChange="enableQuantity('<?php echo $i; ?>');" <?php if(!$shpinwards[$i]['checked'] and empty($obj->retrieve)){echo"disabled";}?>/>
			<?php } ?>
			</td>
			...
			<td><?php echo getQntPackage($shpinwards[$i]['package'],$shpinwards[$i]['quantitys']); ?> </td>
			<td><?php echo getQntPackage($shpinwards[$i]['package'],$shpinwards[$i]['quantitys']-$shpinwards[$i]['quantity']); ?> </td>
			<td><input type="text" id="quantity<?php echo $i; ?>" size="4" value="<?php echo getQntPackage($shpinwards[$i]['package'],$shpinwards[$i]['quantity']); ?>" readonly onChange="changeQuantity('<?php echo $i; ?>', this.value,'<?php echo getQntPackage($shpinwards[$i]['package'],$shpinwards[$i]['quantity']); ?>','<?php echo $shpinwards[$i]['package']; ?>');"/> </td>
...
<table align="center" width="100%">
	...
	<tr>
		...
		<input type="submit" class="btn btn-primary" name="action" id="action" value="<?php echo $obj->action; ?>">
		
		&nbsp;<input type="submit" name="action" id="action" class="btn btn-warning" value="Cancel" onclick="window.top.hidePopWin(true);"/></td>
	</tr>
	<?php if(!empty($obj->retrieve)){?>
	<tr>
		...
		if($obj->journals!='Yes' and existsRule($auth) and $obj->inwardstatus!=2){ ?>
		<input type="submit" class="btn btn-primary" name="action" id="action" value="Effect Journals"/>
		<input type="submit" class="btn btn-danger" name="action" id="action" value="Revoke"/>
		<?php } ?>
		</td>
	</tr>
	<?php }?>
