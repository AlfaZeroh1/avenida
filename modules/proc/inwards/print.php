...
<td><strong>LPO No:</strong> <?php echo $orders->lpono; ?><br/>
<strong>Delivery No:</strong> <?php echo $orders->deliverynoteno; ?></td>
...
<!-- </div>-->
   ...
   <table width="100%" class="table table-stripped table-bordered">
	...
	<?php
	$inwards = new Inwards();
		$fields="proc_inwards.id, con_projects.name as projectid, concat(assets_categorys.name,': ',proc_inwarddetails.memo) assetname, proc_inwards.documentno, inv_unitofmeasures.name uom, inv_items.id as itemid,inv_items.name itemname,  proc_inwarddetails.quantity, proc_inwarddetails.costprice, proc_inwarddetails.total, proc_inwarddetails.memo, proc_inwards.deliverynoteno, proc_suppliers.id as supplierid, proc_inwards.remarks, proc_inwards.inwarddate, proc_inwards.file, proc_inwards.createdby, proc_inwards.createdon, proc_inwards.lasteditedby, proc_inwards.lasteditedon, proc_inwards.ipaddress, inv_items.package";
		$join=" left join proc_inwarddetails on proc_inwarddetails.inwardid=proc_inwards.id left join inv_items on inv_items.id=proc_inwarddetails.itemid left join con_projects on proc_inwards.projectid=con_projects.id  left join proc_suppliers on proc_inwards.supplierid=proc_suppliers.id left join inv_unitofmeasures on inv_unitofmeasures.id=inv_items.unitofmeasureid left join assets_categorys on assets_categorys.id=proc_inwarddetails.categoryid ";
		$having="";
		$groupby="";
		$orderby="";
		...
		<tr>
			...
			<td align="center"><?php echo $row->itemname; ?><?php echo $row->assetname; ?></td>
  ...
  <td>STORES: __________________________________</td>  <td>Date: _____________________</td>
  
  </tr>
    <tr height="50" style="font-weight:bold;">
  <td>STOCK CONTROLLER: __________________________________</td>  <td>Date: _____________________</td>
  
  </tr>
    <tr height="50" style="font-weight:bold;">
  <td>MANAGER: __________________________________</td>  <td>Date: _____________________</td>
  
  </tr>
    <tr height="50" style="font-weight:bold;">
  <td>SECURITY: __________________________________</td>  <td>Date: _____________________</td>
