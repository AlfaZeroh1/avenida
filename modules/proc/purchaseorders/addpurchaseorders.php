...
// 			
 ...
 womAdd('getExchangeRate(5)');
 womOn();
 </script>
<div class='container'>
<div class="content">
<form  id="theform" action="addpurchaseorders_proc.php" name="purchaseorders" method="POST" enctype="multipart/form-data">
	<table class="table" <?php if($obj->status==2){ ?> style="background-image:url(../../../images/cancel.png); background-repeat:no-repeat;"  <?php } ?> width="100%" border="0" align="center" cellpadding="2" cellspacing="0"  >
...
<?php
	...
	?>
		...
		<td><input type="text" name="quantity" id="quantity" onchange="calculateVATAmount();calculateTotal();" onblur="calculateTotal();"  size="8" value="<?php echo $obj->quantity; ?>"></td>
...
<input type="hidden" name="edit" id="edit" hidden size="0"  value="<?php echo $obj->edit; ?>">
		...
		Order On:<input type="date" name="orderedon" onchange="getExchangeRate();" id="orderedon"  class="date_input" size="8" readonly  value="<?php echo $obj->orderedon; ?>">
		</td><td>
		Requisition No:</td><td><textarea name="requisitionno" ><?php echo $obj->requisitionno; ?></textarea>
		</tr>
                <tr>
			</td>
			</tr>
...
<!-- 			<td><?php echo $shppurchaseorders[$i]['tradeprice']; ?> </td> -->
			...
			<td align="right" id="quantity<?php echo $i; ?>"><?php echo getQntPackage($shppurchaseorders[$i]['package'],$shppurchaseorders[$i]['quantity']); ?> </td>
			<?php if(!empty($obj->retrieve)){?>
...
<table align="center" width="98%">
	...
	<?php
			...
			?>
	<tr>
		<td colspan="2" align="center"><input class="btn btn-primary" type="submit" name="action" id="action" value="<?php echo $obj->action; ?>">&nbsp;<input class="btn btn-danger" type="submit" name="action" id="action" value="Close PO" onclick="return confirm('Are you sure you want to close?')"/></td>
	</tr>
	<?php }if(!empty($obj->retrieve)){?>
