...
// 	}
	...
	$shprequisitions[$it]=array('employeeid'=>"$obj->employeeid",'categoryid'=>"$obj->categoryid",'categoryname'=>"$categorys->name",'remarks'=>"$obj->remarks", 'itemid'=>"$obj->itemid", 'expenseid'=>"$obj->expenseid", 'expensename'=>"$expenses->name", 'unitofmeasureid'=>"$obj->unitofmeasureid",'unitofmeasurename'=>"$unitofmeasures->name", 'itemname'=>"$items->name", 'memo'=>"$obj->memo", 'costprice'=>"$obj->costprice", 'quantity'=>"$obj->quantity", 'requiredon'=>"$obj->requiredon", 'total'=>"$obj->total",'available'=>"$obj->available",'package'=>"$items->package",'consumption'=>"$obj->consumption");
...
if($obj->action=="Raise LPO" or $obj->action=="Cash Purchase"){
  ...
  redirect("../purchaseorders/addpurchaseorders_proc.php?raise=1",true);
}
...
if($obj->action=="Filter"){
	if(!empty($obj->invoiceno)){
		...
		while($row=mysql_fetch_object($res)){
			...
			$query="select case when inv_items.package>0 then sum(pos_orderdetails.quantity)/inv_items.package else sum(pos_orderdetails.quantity) end quantity from pos_orderdetails left join pos_orders on pos_orderdetails.orderid=pos_orders.id left join inv_items on inv_items.id=pos_orderdetails.itemid where pos_orderdetails.itemid='$row->itemid' and pos_orders.orderedon>=date_sub(date(now()), interval 7 day) ";//echo $query;
			$r = mysql_fetch_object(mysql_query($query));
			
			$row->used = $r->quantity;
						
			$query="select * from inv_branchstocks where brancheid='25' and itemid='$row->itemid'";
			$r = mysql_fetch_object(mysql_query($query));
			
			$row->balance=$r->quantity;
			
			$query="select * from inv_stocktrack where brancheid='25' and itemid='$row->itemid' and recorddate<=date_sub(date(now()), interval 7 day) order by id desc limit 1";//echo "$query<br/>";
			$r = mysql_fetch_object(mysql_query($query));
			
			$totals=0;
			$row->opening=$r->remain;
			$totals+=$row->opening;
			
			$query="select sum(pd.quantity) quantity from proc_purchaseorders p left join proc_purchaseorderdetails pd on p.id=pd.purchaseorderid where itemid='$row->itemid' and orderedon<=date_sub(date(now()), interval 7 day) ";
			$r = mysql_fetch_object(mysql_query($query));
			
			$row->additions=$r->quantity;
			$totals+=$row->additions;
			
			$query="select sum(td.quantity) quantity from inv_transfers t left join inv_transferdetails td on t.id=td.transferid where t.brancheid=25 and t.tobrancheid in(select id from sys_branches where type='External') and td.itemid='$row->itemid' and t.transferedon<=date_sub(date(now()), interval 7 day)";
			$r = mysql_fetch_object(mysql_query($query));
			
			$row->transfers = $r->quantity;
			$totals-=$row->transfers;
			
			$consumption = $row->balance-$totals;
			
			$ob=$row;
			$row->total=$row->quantity*$row->costprice;
			$toorder = $row->quantity-$purchaseorderdetails->ordered;
			$shprequisitions[$it]=array('id'=>"$row->id",'remarks'=>"$row->remarks", 'unitofmeasureid'=>"$row->unitofmeasureid",'unitofmeasurename'=>"$row->unitofmeasurename", 'itemid'=>"$row->itemid",'requiredon'=>"$row->requiredon", 'itemname'=>"$row->itemname", 'costprice'=>"$row->costprice", 'categoryid'=>"$row->categoryid", 'categoryname'=>"$row->categoryname", 'expenseid'=>"$row->expenseid", 'expensename'=>"$row->expensename",'memo'=>"$row->memo", 'quantity'=>"$row->quantity",'available'=>"$row->available",'used'=>"$row->used", 'toorder'=>"$toorder", 'total'=>"$row->total",'ordered'=>"$purchaseorderdetails->ordered",'lpono'=>"$purchaseorderdetails->lpono",'delivered'=>"$deliverynotedetails->delivered", 'package'=>"$row->package",'consumption'=>"$row->consumption",'opening'=>"$row->opening",'additions'=>"$row->additions",'transfers'=>"$row->transfers",'totals'=>"$totals",'available'=>"$row->balance",'consumption'=>"$consumption");
