<title>WiseDigits ERP: Packinglists </title>
<?php 
include "../../../head.php";
...
// 	      $('#chatAudio')[0].play();
	      ...
	      $.get("../../post/barcodes/checkBarCodess.php?type=<?php echo $obj->status; ?>",{barcode:str},function(data){
		error=data;
...
//       $('#chatAudio')[0].play();
	...
	$("#employeename").val(data);  
      ...
      document.getElementById("employeeids").value=parseInt(st[0]);
      $("#datecode").val(st[1]); 
      document.getElementById("barcode").focus();
    }
  })
	...
	//return true;	     
	  ...
	  $.get("checkDateCode.php?vbarcode="+vbarcode+"&ebarcode="+ebarcode,function(data){
	    ...
	    if(parseFloat(err[0])==1){
	      ...
	      $.post( "addpackinglists_proc.php", { action2: "Add", barcode:$("#barcode").val(), barcode2:$("#barcode2").val(), sizeid:$("#sizeid").val(), itemid:$("#itemid").val(), quantity:$("#quantity").val(), iterator:$("#iterator").val(), status:$("#status").val(), employeeid:$("#employeeid").val(), employeename:$("#employeename").val(), datecode:$("#datecode").val(),  type:2 } );
...
{  
 ...
 function checkMixedBoxes(st){
//  document.getElementById("itemnam").value="";
 document.getElementById("itemid").value="";
 var dv = document.getElementById('mixed');
  if(st=='Yes'){
...
<hr>
<div class="content">
<form class="forms" action="addpackinglists_proc.php" name="packinglists" method="POST" enctype="multipart/form-data" onsubmit="return checkForm(this,event);">
 ...
 <?php if(!empty($obj->retrieve)){?>
	<tr>
		...
		Packing No:<input type="text" size="4" name="invoiceno" value="<?php echo $obj->invoiceno; ?>"/>
		Box No:<input type="text" size="4" name="box"/>
		&nbsp;<input type="submit" name="action" value="Filter"/></td>
	</tr>
	<?php }?>
			<tr>
				<td><input type="hidden" name="returns" value="<?php echo $obj->returns; ?>"/><input type="hidden" name="invoice" value="<?php echo $obj->invoice; ?>"/><label>Customer:</label></td>
				<td><input type='text' readonly size='30' name='customername' id='customername' value='<?php echo $obj->customername; ?>'>
					<input type="hidden" name='customerid' id='customerid' value='<?php echo $obj->customerid; ?>'><input type="hidden" name='code' id='code' value='<?php echo $obj->code; ?>'></td>
				<td><label>TelNo.:</label></td>
				<td><input type='text' name='tel' id='tel' size='20' readonly value='<?php echo $obj->tel; ?>'/></td>			<tr>
				<td><label>Cust Remarks:</label></td>
				<td><textarea name='remarks' id='remarks' readonly><?php echo $obj->remarks; ?></textarea></td>
				<td><label>Address:</label></td>
				<td><textarea name='address' id='address' ><?php echo $obj->address; ?></textarea>
				<input type="hidden" name="status" value="<?php echo $obj->status; ?>"/>
				</td>			</td>
			</tr>
			<tr>
				<td><label>Box No:</label></td>
<td><input type="text" name="boxno" id="boxno" size="20"  value="<?php echo $obj->boxno; ?>" />
    </td><td>&nbsp;</td>
			</tr>
			<tr>
				<td><label>Mixed Box:</label></td>
<td><input type="radio" name="mixedbox" id="mixedbox" value="Yes" <?php if($obj->mixedbox=='Yes'){echo "checked";}?> onclick="checkMixedBoxes('Yes');" />Yes<br/>
      <input type="radio" name="mixedbox" id="mixedbox" value="No" <?php if($obj->mixedbox=='No'){echo "checked";}?> onclick="checkMixedBoxes('No');"/>No&nbsp;
      <div id="mixed">
      <select name="item" id="item"  class="selectbox">
<option value="">Select...</option>
<?php
	$items=new Items();
	$where=" where pos_items.mixedbox='Yes' ";
	...
	?>
		<option value="<?php echo $rw->id; ?>" <?php if($obj->item==$rw->id){echo "selected";}?>><?php echo initialCap($rw->name);?></option>
	<?php
	}
	?>
</select>
	...
	<tr>
		<td> 
		    &nbsp;
		    Emp: <input type="text" name="barcode2" id="barcode2" value="<?php echo $obj->barcode2; ?>"/>
		    &nbsp;
		    Client: <input type="text" name="barcode" id="barcode" value="<?php echo $obj->barcode; ?>" />
		    
		    &nbsp;<input type='text' readonly size='3' name='datecode'  id='datecode' value='<?php echo $obj->datecode; ?>'>&nbsp;<input type='hidden' readonly size='3' name='employeeids'  id='employeeids' value='<?php echo $obj->employeeids; ?>'><input type='text' readonly size='15' name='employeename'  id='employeename' value='<?php echo $obj->employeename; ?>'>
		</td>
...
<font color='red'>*</font>		<td><input type="text" name="quantity" id="quantity" size="16" value="<?php echo $obj->quantity; ?>"><font color='red'>*</font></td>
	...
	<td><input type="submit" name="action2" value="Add"/></td>
		...
		<table align='center'>
			<tr>
			<td>
		Packing No:<input type="text" name="documentno" id="documentno" readonly size="16"  value="<?php echo $obj->documentno; ?>">
		Order No:<input type="text" readonly name="orderno" id="orderno"  size="16"  value="<?php echo $obj->orderno; ?>">
                Remarks    <textarea name='memo' id='memo' ><?php echo $obj->memo; ?></textarea>
		Packed On:<input type="date" name="packedon" id="packedon" readonly class="date_input" size="16" readonly  value="<?php echo $obj->packedon; ?>">
...
<table class="table display" width="100%">
	...
	if($_SESSION['shppackinglists']){
		...
		<tr style="font-size:12px; vertical-align:text-top; ">
			<td><?php echo ($i+1); ?></td>
			<td><?php echo $shppackinglists[$i]['itemname']; ?> </td>
			<td><?php echo $shppackinglists[$i]['sizename']; ?> </td>
			<td><?php echo $shppackinglists[$i]['quantity']; ?> </td>
			<td><?php echo $shppackinglists[$i]['boxno']; ?> </td>
			<td><?php echo $shppackinglists[$i]['mixedbox']; ?> </td>
			<td><?php echo $shppackinglists[$i]['itemnam']; ?> </td>
			<td><?php echo $shppackinglists[$i]['memo']; ?> </td>
			<td><?php echo $shppackinglists[$i]['employeename']; ?> </td>
			<td><?php echo $shppackinglists[$i]['datecode']; ?> </td>
			<td><?php echo $shppackinglists[$i]['barcode']; ?> </td>
			<td><?php echo $shppackinglists[$i]['total']; ?> </td>
...
<table align="center" width="100%">
	...
	<tr>
		...
		<input  class="btn btn-primary" onclick = "this.style.visibility='hidden', loading.style.visibility='visible'" type="submit" name="action" id="action" value="<?php echo $obj->action; ?>">
		<?php //} ?>
		<?php if($obj->status==1){
		//Authorization.
		...
		if(existsRule($auth)){
		?>
		<input  class="btn btn-primary" type="submit" name="action" id="action" value="Update Invoice">
		<?php }} ?>
		<?php if($obj->status!=1){?><input  class="btn btn-primary" type="submit" name="action" id="action" value="Raise Invoice"><?php } ?>
		<input  class="btn btn-primary" type="submit" name="action" id="action" value="Returns">
...
<?php 
	redirect("addpackinglists_proc.php?retrieve=&next=".$next."&packing=".$packing);
