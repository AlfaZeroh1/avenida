...
<!--     <input type="text" readonly class="date_input" name="toteamedon" id="toteamedon" value="<?php echo $obj->toteamedon; ?>"/> -->
  ...
  <?php
    $query="select * from sys_branches where id='$obj->brancheid'";//echo $query;
    $branches = mysql_fetch_object(mysql_query($query));
    ?>
    <tr style="background-color:#337ab7;color:red;font-size:18px;font-weight:bold;">
        <td colspan='4' align="center"><?php echo strtoupper($branches->name); ?></td>
    </tr>
...
<table style="clear:both;"  class="table table-codensed" id="example" width="100%" >
	...
	<?php
		...
		$teams = new Orders();
		$fields=" distinct sys_branches.name as brancheid, sys_branches.id branche,  pos_shifts.name shift, pos_teams.startedon, pos_teams.endedon, pos_teams.teamedon, pos_teams.remarks, pos_teams.ipaddress, pos_teams.createdby, pos_teams.createdon, pos_teams.lasteditedby, pos_teams.lasteditedon, pos_teams.status, pos_teams.submitted, pos_teams.shiftid shiftids, sum(pos_orderdetails.price*(pos_orderdetails.quantity-(case when pos_orderdetails.voids is null then 0 else pos_orderdetails.voids end))) amount, group_concat(distinct pos_orders.shiftid) shiftid";
		$join=" left join pos_teams on pos_teams.id=pos_orders.shiftid left join sys_branches on pos_orders.brancheid=sys_branches.id  left join pos_shifts on pos_teams.shiftid=pos_shifts.id left join pos_orderdetails on pos_orderdetails.orderid=pos_orders.id ";
		$having="";
		$groupby=" group by  branche, pos_shifts.id, pos_teams.teamedon ";
		$orderby=" order by pos_orders.id desc ";
		$where=" where pos_orderdetails.status!=3 ";
		...
		$teams->retrieve($fields,$join,$where,$having,$groupby,$orderby);//echo $teams->sql;
		$res=$teams->result;
		...
		$query="select sum(pos_orderdetails.price*pos_orderdetails.quantity) amount, sum(pos_orderpayments.amount) paid from pos_orders left join pos_orderpayments on pos_orderpayments.orderid=pos_orders.id left join pos_orderdetails on pos_orderdetails.orderid=pos_orders.id where pos_orders.shiftid in($row->shiftid) and pos_orderdetails.status!=3";
	...
	?>
		<tr style="color:<?php echo $color; ?>;">
			...
			<td>
			  <a href="javascript:;" onclick="showPopWin('../teamdetails/addteamdetails_proc.php?id=<?php echo $rws->id; ?>&brancheid2=<?php echo $obj->brancheid; ?>&employeeid=<?php echo $emp->employeeid; ?>&teamid=<?php echo $obj->teamid; ?>&cashier=1&teamedon=<?php echo $obj->teamedon; ?>',800,630);">Clear</a>
			</td>
			<?php
			}else{
			?>
