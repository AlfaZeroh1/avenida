...
{ 
	...
	if(ans)
	{
		poptastic("printreturninward.php?id="+id+"&doc=<?php echo $obj->documentno; ?>&customerid=<?php echo $obj->customerid; ?>&returninwarddon=<?php echo $obj->soldon; ?>&packingno=<?php echo $obj->packingno; ?>",450,940);
	}
}
...
<form  id="theform" action="addreturninwards_proc.php" name="returninwards" method="POST" enctype="multipart/form-data">
 ...
 <?php if(!empty($obj->retrieve)){?>
	<tr>
		...
		<?php }else{?>
		Document No:<input type="text" size="4" name="returninwardnos"/>&nbsp;
		<?php }?>
	...
	if($_SESSION['shpreturninwards']){
		...
		if(true){
		  ...
		  <tr style="font-size:12px; vertical-align:text-top; ">
			  ...
			  <td><?php echo $shpreturninwards[$i]['quantity']; ?> </td>
			  <td><?php echo $shpreturninwards[$i]['price']; ?> </td>
