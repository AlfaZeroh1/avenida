...
if(empty($obj->action)){
	$defs=mysql_fetch_object(mysql_query("select (max(documentno)+1) documentno from pos_returninwards"));
...
if($obj->action2=="Add"){
	...
	$shpreturninwards[$it]=array('itemid'=>"$obj->itemid", 'itemname'=>"$items->name",'sizeid'=>"$obj->sizeid",'sizename'=>"$sizes->name", 'quantity'=>"$obj->quantity", 'price'=>"$obj->price", 'discount'=>"$obj->discount", 'bonus'=>"$obj->bonus", 'total'=>"$obj->total",'boxno'=>"$obj->boxno",'departmentid'=>"$obj->departmentid");
...
if($obj->action=="Filter"){echo "Here";
	...
	elseif(!empty($obj->parent) and empty($obj->returninwarddate)){
	  $error="Must give Date of Returninward";
	}
	else{
		
		$returninwarddetails = new Returninwarddetails();
		$fields="pos_returninwards.*, pos_returninwarddetails.*";
		$join=" left join pos_returninwards on pos_returninwards.id=pos_returninwarddetails.returninwardid ";
		$having="";
		$groupby="";
		$orderby="";
		if(!empty($obj->parent))
		  $where=" where pos_returninwards.customerid in(select id from crm_customers where customerid='$obj->customerid2' and returnedon='$obj->returnedon')";
		else
		  $where=" where pos_returninwards.documentno='$obj->invoicenos'";
		...
		while($row=mysql_fetch_object($res)){
			...
			$ob=$row;
			$shpreturninwards[$it]=array('id'=>"$ob->id",'itemid'=>"$ob->itemid", 'itemname'=>"$items->name",'itemnam'=>"$item->name",'item'=>"$item->id", 'quantity'=>"$ob->quantity", 'price'=>"$ob->price",'vat'=>"$obj->vat", 'exportprice'=>"$ob->exportprice", 'discount'=>"$ob->discount", 'bonus'=>"$ob->bonus", 'total'=>"$ob->total",'exporttotal'=>"$ob->exportprice*$ob->quantity",'boxno'=>"$obj->boxno");
...
// 		$where=" where pos_returninwardconsumables.returninwardid=$ob->returninwardid ";
// 		$fields="pos_returninwardconsumables.id, pos_returninwardconsumables.itemid, inv_items.name itemname, inv_unitofmeasures.name unitofmeasurename, pos_returninwardconsumables.unitofmeasureid, pos_returninwardconsumables.quantity, pos_returninwardconsumables.price, pos_returninwardconsumables.total, pos_returninwardconsumables.remarks, pos_returninwardconsumables.ipaddress, pos_returninwardconsumables.createdby, pos_returninwardconsumables.createdon, pos_returninwardconsumables.lasteditedby, pos_returninwardconsumables.lasteditedon";
// 		$join=" left join inv_items on inv_items.id=pos_returninwardconsumables.itemid left join inv_unitofmeasures on inv_unitofmeasures.id=pos_returninwardconsumables.unitofmeasureid";
// 		$having="";
// 		$groupby="";
// 		$orderby="";
