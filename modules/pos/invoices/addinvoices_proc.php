...
if($obj->action=="Filter"){
	...
	elseif(!empty($obj->parent) and empty($obj->invoicedate)){
	  $error="Must give Date of Invoice";
		...
		$orderby="";
		if(!empty($obj->parent))
		  $where=" where pos_invoices.customerid in(select id from crm_customers where customerid='$obj->customerid2' and soldon='$obj->invoicedate') or (pos_invoices.customerid='$obj->customerid2'  and soldon='$obj->invoicedate')";
		else
		...
		while($row=mysql_fetch_object($res)){
		    ...
		    $groupby="";
		    $having="";
		    $where=" where pos_items.id='$row->itemid'";
...
// 		  }
  ...
  while($i<$num){     
	...
	$k++;
      ...
      $x=0;
	foreach($shpreturninwards as $key => $value){
	
	  if($value['itemid']==$shpinvoices[$i]['itemid'] and $value['sizeid']==$shpinvoices[$i]['sizeid'] and $value['price']==$shpinvoices[$i]['price']){
	    $shpreturninwards[$x]['quantity']+=$shpinvoices[$i]['quantity'];
	    $shpreturninwards[$x]['total']+=($shpinvoices[$i]['quantity']*$value['price']);	    
	    
	    if(!strpos($shpreturninwards[$x]['boxno'],$shpinvoices[$i]['boxno']))
	      $shpreturninwards[$x]['boxno'].=', '.$shpinvoices[$i]['boxno'];
  ...
  redirect("../returninwards/addreturninwards_proc.php?raise=1&types=".$type);
}
