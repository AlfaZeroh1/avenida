...
function voidOrderItem(id, orderno){
	...
	if(ans)
	{
        var voids=$("#quantity"+id).val();
        $.post("void.php",{id:id,orderno:orderno,voids:voids},function(data){
//             alert(data);
...
<table style="clear:both;" class="table display" width="100%" border="0" cellspacing="0" cellpadding="2" align="center" >
	...
	<?php
		...
		$fields="pos_orderdetails.id, pos_orders.orderno orderno, pos_orders.billno, pos_orders.shiftid, crm_customers.name as customerid, pos_orders.orderedon, pos_orders.remarks, pos_orders.ipaddress, pos_orders.createdby, pos_orders.createdon, pos_orders.lasteditedby, pos_orders.lasteditedon, ((pos_orderdetails.quantity-(case when pos_orderdetails.voids is null then 0 else pos_orderdetails.voids end))*pos_orderdetails.price) total, concat(hrm_employees.pfnum, ' ',concat(hrm_employees.firstname,' ',concat(hrm_employees.middlename,' ',hrm_employees.lastname))) username, sys_branches.name branchename, pos_orders.status, (pos_orders.tableno), sys_branchess.name brancheid2, pos_orders.billno, auth_users.employeeid, inv_items.name itemname, pos_orderdetails.itemid, (pos_orderdetails.quantity-(case when pos_orderdetails.voids is null then 0 else pos_orderdetails.voids end)) quantity, (case when pos_orderdetails.voids is null then 0 else pos_orderdetails.voids end) voids, pos_orderdetails.price, pos_orderdetails.status, pos_orderdetails.voids ";
		...
		if($row->status==3 or ($row->quantity)<=0)
                $color="red";
	...
	?>
		<tr style="color:<?php echo $color; ?>">
			...
			<td><input type="text" <?php if(empty($row->quantity))echo "readonly"; ?> name="quantity<?php echo $row->id; ?>" id="quantity<?php echo $row->id; ?>" value="<?php echo ($row->quantity); ?>"/></td>
...
<!--                     <a href="orderss.php?cancelid=<?php echo $row->id; ?>&orderno=<?php echo $row->orderno; ?>&pop=1">VOID</a>  -->
                    <a href="javascript:;" onClick="voidOrderItem('<?php echo $row->id; ?>','<?php echo $row->orderno; ?>');">VOID</a> 
                <?php 
                }
