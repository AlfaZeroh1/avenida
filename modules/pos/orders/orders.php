...
if(!empty($ob->cancelid)){
  ...
  $query="select pd.*, p.brancheid, p.orderedon, p.orderno from pos_orderdetails pd left join pos_orders p on p.id=pd.orderid where pd.orderid='$ob->cancelid'";
  $res=mysql_query($query);
  while($row=mysql_fetch_object($res)){  
    
    $obj->brancheid=$row->brancheid;
    $obj->quantity=$row->quantity;
    $obj->doc=$row->orderno;
    $obj->orderedon=$row->orderedon;
    ...
    $branchstocks->transaction="Voiding Order";
    
    if($branchstockss->affectedRows>0){
      $branchstockss = $branchstockss->fetchObject;
      
      $branchstockss->transaction="Voiding Order";
...
// 				  $branchstocks->quantity=($obj->quantity*-1);
    ...
    printOrder($obj,4,false,false,$row->brancheid2);
...
<!-- 			<th>&nbsp;</th> -->
	...
	<?php
		...
		$i=0;
		$where=" where pos_orders.status=1 and pos_orderdetails.status!=3 ";
			...
			<!-- <td><input type='checkbox' id="<?php //echo $row->orderno; ?>" onClick="setOrder('<?php //echo $row->orderno; ?>');"/> -->
			 <td>
				<input type="button" 
					...
					onclick="window.open('print_new.php?doc=<?php echo $row->orderno; ?>', 'printWindow', 'width=200,height=150');">
			</td>
...
<a href="javascript:;" class="btn btn-warning" onClick="Clickheretoprint('','PRINT');">PRINT</a>
