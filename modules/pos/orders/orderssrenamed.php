...
// $query="select pos_teams.id, sys_branches.id brancheid, sys_branches.name branchename, pos_teamroles.type teamroletype from pos_teams left join pos_shifts on pos_teams.shiftid=pos_shifts.id left join pos_teamdetails on pos_teamdetails.teamid=pos_teams.id left join sys_branches on sys_branches.id=pos_teams.brancheid left join pos_teamroles on pos_teamroles.id=pos_teamdetails.teamroleid where pos_teams.teamedon='$obj->orderedon' and pos_teamdetails.employeeid='".$_SESSION['employeeid']."'";//echo $query;
...
if(!empty($ob->shiftid)){
  ...
  $query="select pos_teams.* from pos_teamdetails left join pos_teams on pos_teamdetails.teamid=pos_teams.id where pos_teams.teamedon='$obj->orderedon'";
  if($_SESSION['levelid']!=1 and $_SESSION['levelid']!=7)
    $query.=" and pos_teamdetails.employeeid='".$_SESSION['employeeid']."' and pos_teams.status=0";
  if($_SESSION['levelid']==7)
    $query.=" and pos_teams.status=0 ";
  ...
  if(empty($r->id)){
    ...
    $obj->brancheid=$r->brancheid;
    $obj->brancheid2=$r->brancheid;
    
  }
  
  $query="select group_concat(distinct pos_teams.id) id from pos_teamdetails left join pos_teams on pos_teamdetails.teamid=pos_teams.id where pos_teams.teamedon='$r->teamedon'";//echo $query;
  if($_SESSION['levelid']!=1 and $_SESSION['levelid']!=7)
    $query.=" and pos_teamdetails.employeeid='".$_SESSION['employeeid']."' ";
...
<div>
  Shift Date: <input name="orderedon" type="text" size="12" value="<?php echo $obj->orderedon; ?>" class="date_input" readonly/>
  ...
  <input type="checkbox" name="owner" id="owner" value='1' <?php if($obj->owner==1)echo "checked";?>/>My Orders
...
<!-- 			<th>&nbsp;</th> -->
	...
	<?php
		...
		$fields="pos_orders.id, count(*) cnt, group_concat(distinct pos_orders.orderno) orderno, pos_orders.billno, pos_orders.shiftid, crm_customers.name as customerid, pos_orders.orderedon, pos_orders.remarks, pos_orders.ipaddress, pos_orders.createdby, pos_orders.createdon, pos_orders.lasteditedby, pos_orders.lasteditedon, sum(pos_orderdetails.quantity*pos_orderdetails.price) total, concat(hrm_employees.pfnum, ' ',concat(hrm_employees.firstname,' ',concat(hrm_employees.middlename,' ',hrm_employees.lastname))) username, sys_branches.name branchename, pos_orders.status, (pos_orders.tableno), sys_branchess.name brancheid2, pos_orders.billno, auth_users.employeeid, pos_orders.print ";
		...
		$having="";
		$groupby=" group by pos_orders.tableno, pos_orders.billno, auth_users.employeeid ";
...
// 		}
		...
		if(($_SESSION['levelid']!=1 and $_SESSION['levelid']!=5 and $_SESSION['levelid']!=9 and $_SESSION['levelid']!=7)){
		  $where.=" and auth_users.employeeid='".$_SESSION['employeeid']."' ";
		}else{
// 		  $where.=" and pos_orders.status=1 ";
		}
		if(!empty($obj->owner))
		  $where.=" and auth_users.employeeid='".$_SESSION['employeeid']."' ";
		...
		$query="select case when sum(pos_orderpayments.amount) is null then 0 else sum(pos_orderpayments.amount) end paid from pos_orderpayments where billno='$row->billno' and billno>0 ";
	...
	?>
		<tr style="color:<?php echo $color; ?>">
			...
			<td>
			<?php
			  if((($_SESSION['levelid']==4 or $_SESSION['levelid']==5) and $row->print==0) or ($_SESSION['levelid']==7 and $row->print==1) or $_SESSION['levelid']==1){
			?>
			  <input type='checkbox' id="<?php echo $row->orderno; ?>" onClick="setOrder('<?php echo $row->orderno; ?>');"/>
			<?php
			}
			?>
			...
			<td><a class="btn btn-success" onclick="showPopWin('orders.php?tableno=<?php echo $row->tableno; ?>&pop=1&orderedon=<?php echo $obj->orderedon; ?>&orderno=<?php echo $row->orderno; ?>&orderid=<?php echo $row->id; ?>&createdby=<?php echo $row->createdby; ?>&employeeid=<?php echo $row->employeeid; ?>',700,630);" href="javascript:;"><?php echo strtoupper($row->tableno); ?></a></td>
			<td><?php echo strtoupper($row->brancheid2); ?></td>
			<td><?php echo strtoupper($row->branchename); ?></td>
			<td><?php echo ($row->createdon); ?></td>
			<td><?php echo $row->remarks; ?></td>
			<td align="right"><a onclick="showPopWin('../orderpayments/addorderpayments_proc.php?action3=1&orderno=<?php echo $row->orderno; ?>&billno=<?php echo $row->billno; ?>&orderid=<?php echo $row->id; ?>',600,430);" href="javascript:;"><?php echo formatNumber($row->total); ?></a></td>
			<td align="right"><?php echo formatNumber($row->paid); ?></td>
			<td align="right"><?php echo formatNumber($balance); ?></td>
			<td align="right">
			<?php if(!empty($row->billno)){ ?>
			<a onclick="showPopWin('../orderpayments/addorderpayments_proc.php?action3=1&orderno=<?php echo $row->orderno; ?>&billno=<?php echo $row->billno; ?>&orderid=<?php echo $row->id; ?>&total=<?php echo $row->total; ?>',600,430);" href="javascript:;">PAY</a>
			<?php } ?>
			</td>
			<td>
			<?php
			...
			if(existsRule($auth)){
			?>
 			    <a href="orderss.php?cancelid=<?php echo $row->id; ?>">VOID</a> 
			<?php } ?>
			<?php if($row->status==2){?>
			<a href="orderss.php?cancelid=<?php echo $row->id; ?>">CANCEL</a> 
			<?php }if($row->status==3){ ?>
			CANCELLED
			<?php } ?>
			</td>
