...
function delShop(itemid,warmth){
 
 var totals=parseFloat($("#total").text());
//  console.log("ITEM: "+itemid);
  $.post("setorder.php",{itemid:itemid,  type:1, action:2, warmth:warmth},function(data){
 
    var error = data.split("|");//console.log(error);
...
//     totals=parseFloat($("#total").text());
    ...
    var myRow = $("."+itemid).closest('tr'); 
    var rowNo = myRow[0];
    tbl.fnDeleteRow(rowNo);
...
function loadWarmthModal(itemid,quantity,warmth){
  $('#myModal').modal('show');
  if(warmth==1){
    $('#warmthdiv').show();
    $('#boildiv').hide();
	...
	if(parseFloat(err[0])=="7"){
	  ...
	  if(err[0]==0){
	    tbl.fnAddData( [	      
		      ...
		      err[5],
		      total,
		      '<a href="javascript:;" class="'+err[4]+'" onClick="delShop(&quot;'+err[4]+'&quot;,&quot;'+err[6]+'&quot;);"><img src="../trash.png" alt="delete" title="delete" /></a>'] );
...
<!--       <input id="text" size="5" class="keyboard" required type="text" placeholder=" Table No..."> -->
      <input type="text" name="tableno" has-error id="num" size="5"  value="<?php echo $obj->tableno; ?>">
	  ...
	  <tfoot>
        ...
        <div class="panel-footer" style="text-align:center;height:70px;">
