...
{ 
	$.post("addorders_proc.php",{action:action,type:1, combined:1,vat:vat});
}
...
// 		  $where.=" and pos_orders.status=1 ";
		...
		$query="select case when sum(pos_orderpayments.amount) is null then 0 else sum(pos_orderpayments.amount) end paid from pos_orderpayments where billno='$row->billno' ";
	...
	?>
		<tr style="color:<?php echo $color; ?>">
			...
			<a onclick="showPopWin('../orderpayments/addorderpayments_proc.php?action3=1&orderno=<?php echo $row->orderno; ?>&billno=<?php echo $row->billno; ?>&orderid=<?php echo $row->id; ?>',600,430);" href="javascript:;">PAY</a>
			<?php } ?>
			</td>
			<td>
			<?php
