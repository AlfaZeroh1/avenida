...
function checkQuantity($obj){
  ...
  $query="select * from inv_branchstocks where brancheid='$obj->brancheid2' and itemid='$obj->itemid'";logging($query);
  $r = mysql_fetch_object(mysql_query($query));
  logging("$obj->quantity>$r->quantity");
  if($obj->quantity>$r->quantity)
    return false;
  else
    return true;
