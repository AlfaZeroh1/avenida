...
// Fetch order (similar logic to your original script)
$orders = new Orders();
...
// Fetch company header from sys_config (ids 1,2,9 in your original)
$config = new Config();
...
// helper format (use your lib's formatNumber if available)
function fmt($n) {
    return number_format($n, 2);
}

// building small metadata
$servedBy = ""; 
...
<!-- Optional Bootstrap for layout on screen (not required for printing) -->
...
<style>
/* Thermal receipt styling for 72mm width */
:root {
  --receipt-width-mm: 72mm;
...
.receipt {
  width: calc(var(--receipt-width-mm) - 6mm); /* small padding */
  margin: 0 auto;
  padding: 6px 3mm;
...
@media print {
  ...
  .receipt { box-shadow: none; margin: 0; padding: 0; width: auto; }
}
</style>
</head>
<body>
...
<div class="receipt" id="receipt">
  ...
  <div class="totals">
    <?php
      // VAT example like your original: total*0.84 and VAT 16% of full total
      $vat_base = $total * 0.84;
  ...
  <div style="margin-top:6px; border-top:1px dashed #000; padding-top:6px;">
    <?php
      // footer from sys_config receiptfootnote if exists
      $q = "SELECT value FROM sys_config WHERE name='receiptfootnote' LIMIT 1";
      $r = mysql_query($q);
      if ($r && mysql_num_rows($r)) {
        $rw = mysql_fetch_object($r);
        echo '<div class="footer">' . htmlspecialchars($rw->value) . '</div>';
      }
    ?>
...
<div class="print-controls no-print">
  ...
  &nbsp;
  <button onclick="downloadPdf();" class="btn btn-secondary">Save as PDF</button>
</div>
...
// Optional: helper to save as PDF via print to PDF (browser dependent)
function downloadPdf(){
  window.print();
}

// optional: auto print on load (set to true if you want)
var autoPrint = false;
if (autoPrint) {
