...
// 	  $printer -> text(str_pad("TABLE NO:",14,' ').$orders->tableno."\n");
	  ...
	  $titles = new item("TOTAL", "","",formatNumber($total/1.16));
	  $printer -> text($titles);
	  $titles = new item("VAT (16%)", "","",formatNumber($total*.16/1.16));
	  $printer -> text($titles);
	  ...
	  $value=$rw->value;
	  $value=str_pad($value,48,' ',STR_PAD_BOTH);
