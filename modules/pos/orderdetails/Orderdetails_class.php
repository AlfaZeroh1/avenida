	...
	function add($obj,$shop){
		...
		while($i<$num){
			...
			if($orderdetailsDBO->persist($ob)){		
				...
				/*if($obj->brancheid2==30){
                    //logging("HERE");
				}else{*///logging("HEREXXXX");
                    //$query="select * from inv_compositeitems where itemid2='$obj->itemid' or itemid='$obj->itemid' ";//echo $query;
					$query="select * from inv_compositeitems where itemid='$obj->itemid' ";//echo $query;
                    $rs = mysql_query($query);
                    if(mysql_affected_rows()>0){
                    while($r = mysql_fetch_object($rs)){
                    
                        //check if constituent items belong to more than 
                        if(!empty($r->brancheid))
                            $obj->brancheid2=$r->brancheid;
                    
                        if(!empty($r->itemid2) and !empty($r->reducing)){
                        ...
                        $obj->brancheid=$obj->brancheid2;
                        
                        $this->addBranchStock($obj);
