...
{				
	...
	function add($obj,$shop){
		...
		while($i<$num){
			...
			$ob->brancheid=$obj->brancheid2;
			
			if($orderdetailsDBO->persist($ob)){		
				
				$query="select * from inv_compositeitems where itemid2='$obj->itemid'";
				...
				if(mysql_affected_rows()>0){
				  ...
				  while($r = mysql_fetch_object($rs)){
				    ...
				    if(!empty($r->brancheid))
				      $ob->brancheid2=$r->brancheid;
				  
				    if(!empty($r->itemid)){
				      
				      $ob->itemid=$r->itemid;
				      $ob->quantity=$obj->quantity/$r->quantity;
				      
				    }
				  
				    $ob->brancheid=$obj->brancheid2;
				    
				    $this->addBranchStock($ob);
