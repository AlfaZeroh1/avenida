...
// 	  logging(print_r($shporders));
	  ...
	  if($x>-1){
	    ...
	    $shporders[$it]=array('id'=>"$ob->id",'itemid'=>"$obj->itemid",'sizeid'=>"$obj->sizeid",'sizename'=>"$sizes->name", 'itemname'=>"$items->name", 'quantity'=>"$obj->quantity",'price'=>"$items->retailprice", 'memo'=>"$obj->memo",'warmth'=>"$obj->warmth");
	  }
	  $it++;
		  $obj->iterator=$it;
	  ...
	  echo "$find|".$items->name."|".$obj->quantity."|".$items->retailprice."|".$items->id."|".$items->warmth;
