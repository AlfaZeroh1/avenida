<title>WiseDigits: Purchases </title>
<?php 
include "../../../head.php";
...
$().ready(function() {
  ...
  $("#suppliername").autocomplete({
	source:"../../../modules/server/server/search.php?main=motor&module=suppliers&field=name",
	focus: function(event, ui) {
		event.preventDefault();
		$(this).val(ui.item.label);
      	},
...
<div class='main'>
<form  id="theform" action="addpurchases_proc.php" name="purchases" method="POST" enctype="multipart/form-data">
 ...
 <?php if(!empty($obj->retrieve)){?>
	...
	<?php }?>
			...
			<tr>
				<td><label>Company Tel:</label></td>
				<td><input type='text' name='tel' id='tel' size='15' readonly value='<?php echo $obj->tel; ?>'/></td>				<td><label>Remarks:</label></td>
	...
	<td><input type="submit" name="action2" value="Add"/></td>
		...
		Bought On:<input type="date" name="boughton" id="boughton"  class="date_input" size="8" readonly  value="<?php echo $obj->boughton; ?>">
		Memo:<textarea name="memo" ><?php echo $obj->memo; ?></textarea>
		Purchase Mode:<select name='mode' >
