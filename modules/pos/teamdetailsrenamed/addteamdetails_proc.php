...
if($obj->action2=="ADD"){
  ...
  if(empty($obj->paymentmodeid)){
    ...
    $teampayments->employeename=$obj->employeename;
...
if(!empty($id)){
	...
	if($ob->cashier==1)
	  $where=" where pos_teamdetails.id in($id) and pos_orders.brancheid='$ob->brancheid2'  and pos_teamdetails.employeeid='$ob->employeeid' ";
	else
	  $where=" where pos_teamdetails.id in($id) and pos_orders.brancheid='$ob->brancheid2'  ";
...
// 	$rw->paid = $obj->paid;
	...
	if(empty($ob->cashier)){
	  ...
	  $query="select concat(hrm_employees.firstname, ' ',concat(hrm_employees.middlename,' ',hrm_employees.lastname)) employeename, pos_teamdetails.employeeid from pos_teamdetails left join hrm_employees on hrm_employees.id=pos_teamdetails.employeeid where pos_teamdetails.teamid in($obj->teamid) and pos_teamdetails.teamroleid=4";//echo $query;
	  $r = mysql_fetch_object(mysql_query($query));
	  $obj->employeename=$r->employeename;
	  $obj->employeeid=$r->employeeid;
