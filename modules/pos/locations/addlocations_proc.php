<?php 
session_start();
require_once("../../../DB.php");
require_once("../../../lib.php");
require_once("Locations_class.php");
require_once("../../auth/rules/Rules_class.php");


if(empty($_SESSION['userid'])){;
	redirect("../../auth/users/login.php");
}

//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="2170";//Edit
}
else{
	$auth->roleid="2168";//Add
}
$auth->levelid=$_SESSION['level'];
auth($auth);


//connect to db
$db=new DB();
$obj=(object)$_POST;
$ob=(object)$_GET;

$mode=$_GET['mode'];
if(!empty($mode)){
	$obj->mode=$mode;
}
$id=$_GET['id'];
$error=$_GET['error'];
if(!empty($_GET['retrieve'])){
	$obj->retrieve=$_GET['retrieve'];
}
	
	
if($obj->action=="Save"){
	$locations=new Locations();
	$obj->createdby=$_SESSION['userid'];
	$obj->createdon=date("Y-m-d H:i:s");
	$obj->lasteditedby=$_SESSION['userid'];
	$obj->lasteditedon=date("Y-m-d H:i:s");
	$obj->ipaddress=$_SERVER['REMOTE_ADDR'];
	$error=$locations->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		$locations=$locations->setObject($obj);
		if($locations->add($locations)){
			$error=SUCCESS;
			redirect("addlocations_proc.php?id=".$locations->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
	}
}
	
if($obj->action=="Update"){
	$locations=new Locations();
	$obj->lasteditedby=$_SESSION['userid'];
	$obj->lasteditedon=date("Y-m-d H:i:s");

	$error=$locations->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		$locations=$locations->setObject($obj);
		if($locations->edit($locations)){
			$error=UPDATESUCCESS;
			redirect("addlocations_proc.php?id=".$locations->id."&error=".$error);
		}
		else{
			$error=UPDATEFAILURE;
		}
	}
}
if(empty($obj->action)){
}

if(!empty($id)){
	$locations=new Locations();
	$where=" where id=$id ";
	$fields="pos_locations.id, pos_locations.name, pos_locations.description, pos_locations.createdby, pos_locations.createdon, pos_locations.lasteditedby, pos_locations.lasteditedon, pos_locations.ipaddress";
	$join="";
	$having="";
	$groupby="";
	$orderby="";
	$locations->retrieve($fields,$join,$where,$having,$groupby,$orderby);
	$obj=$locations->fetchObject;

	//for autocompletes
}
if(empty($id) and empty($obj->action)){
	if(empty($_GET['edit'])){
		$obj->action="Save";
	}
	else{
		$obj=$_SESSION['obj'];
	}
}	
elseif(!empty($id) and empty($obj->action)){
	$obj->action="Update";
}
	
	
$page_title="Locations ";
include "addlocations.php";
?>