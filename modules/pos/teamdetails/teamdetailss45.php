...
<table style="clear:both;"  class="table table-codensed" id="example" width="100%" >
	...
	<?php
		$i=0;
		if(!empty($_SESSION['shift'])){
		  $query="select group_concat(id) teamid from pos_teams left join pos_teamdetails on pos_teams.id=pos_teamdetails.teamid where pos_teamdetails.employeeid='".$_SESSION['employeeid']."' and pos_teams.status=0 order by pos_teams.id desc ";
		...
		$fields=" group_concat(distinct pos_orders.id) orderid, hrm_employees.id employeeid, concat(hrm_employees.firstname, ' ',concat(hrm_employees.middlename,' ',hrm_employees.lastname)) name,  hrm_assignments.name assignmentid, auth_users.id userid,   sum((pos_orderdetails.quantity-(case when pos_orderdetails.voids is null then 0 else pos_orderdetails.voids end))*pos_orderdetails.price) amount, pos_teams.id teamid, pos_teams.brancheid, sum(pos_orders.paid) paid ";
		...
		$orderby=" ";
		$where=" where pos_teams.shiftid='$shifts->id' ";
		
		$where.=" and pos_teams.teamedon='$r->teamedon' and pos_orders.brancheid='$obj->brancheid' ";
...
// 		  continue;
	...
	?>
		<tr style="color:<?php echo $color; ?>;">
			...
			<td><a href="../orders/orders.php?shiftid=<?php echo $obj->teamid; ?>&employeeid=<?php echo $row->employeeid; ?>&brancheid2=<?php echo $obj->brancheid; ?>&teamedon=<?php echo $obj->teamedon; ?>"><?php echo initialCap($row->name); ?></a></td>
			<?php
			//Authorization.
			$auth->roleid="7486";//View
			$auth->levelid=$_SESSION['level'];
