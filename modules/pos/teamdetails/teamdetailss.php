...
function clickHerePrint(itemdetailid, teamid, createdby, type){
  
  var brancheid = $("#brancheid").val();
  poptastic("print.php?itemdetailid="+itemdetailid+"&brancheid="+brancheid+"&teamid="+teamid+"&createdby="+createdby+"&type="+type);
...
<table style="clear:both;"  class="table table-codensed" id="example" width="100%" >
	...
	<?php
		...
		$orderby=" ";
		$where=" where pos_orders.status=1 and pos_teams.shiftid='$obj->shiftid' and pos_orderdetails.status!=3 ";
	...
	?>
		<tr style="color:<?php echo $color; ?>;">
			...
			<td>
			  ...
			  <a href="javascript:;" onClick="clickHerePrint('<?php echo $rw->id; ?>','<?php echo $row->teamid; ?>','<?php echo $row->brancheid; ?>',1);">Print</a>
			  <a href="javascript:;" onClick="clickHerePrint('<?php echo $rw->id; ?>','<?php echo $row->teamid; ?>','<?php echo $row->brancheid; ?>',2);">Print Summary</a>
