...
<table style="clear:both" class="table display"  cellpadding="0" align="center" width="100%" cellspacing="0">
	...
	if($_SESSION['shppurchases']){
		...
		<tr style="font-size:12px; vertical-align:text-top; ">
			...
			<td><select name="vatclasseid<?php echo $i; ?>" class="selectbox" onchange="getVatClasss(this.value,'<?php echo $i; ?>','<?php echo $shppurchases[$i]['costprice']; ?>','<?php echo $shppurchases[$i]['quantity']; ?>','<?php echo $shppurchases[$i]['total']; ?>','<?php echo $shppurchases[$i]['vatamount']; ?>');">
			<option value="">Select...</option>
			<?php
			$vatclasses = new Vatclasses();
			$fields="*";
