...
{				
	...
	function add($obj){
		...
		if($branchstocksDBO->persist($obj)){
			  ...
			  $obj->transaction="Purchase on ".date("Y-m-d H:i:s");
			  $obj->remain=$obj->available+$obj->quantity;
	...
	function edit($obj,$where=""){
		...
		if($branchstocksDBO->update($obj,$where)){
		           ...
		           //insert into stock track  
			  $stocktrack=new Stocktrack();
			  $branchstock = mysql_fetch_object(mysql_query("select quantity from inv_branchstocks where itemid='$obj->id' and brancheid='$obj->brancheid'"));
			  $obj->available=$branchstock->quantity;
			  $obj->transaction="Stock take on ".date("Y-m-d H:i:s");
			  $obj->remain=$obj->available+$obj->quantity;
			  $obj->createdby=$_SESSION['userid'];
			  $obj->createdon=date("Y-m-d H:i:s");
			  $obj->lasteditedby=$_SESSION['userid'];
