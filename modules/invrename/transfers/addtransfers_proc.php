...
// 	      $branches->retrieve($fields, $join, $where, $having, $groupby, $orderby);//echo $branches->sql;
// 	      $branches=$branches->fetchObject;
// 	      
// 	    if(empty($shptransfers[$i]['instalcode']) and $shptransfers[$i]['installationcode']=='Yes' and $branches->type!='Store' and $branches->type!='Transit')
// 	    {
// 	    $error="Every Item Must be Given an installation code";
// 	    }
// 	    $i++;
// 	    }
	if(!empty($error)){
	...
	elseif(empty($shptransfers)){
		...
		if($transfers->add($transfers,$shptransfers,$bool=false)){
                        ...
                        $query="update inv_requisitions set status=1 where documentno='$obj->requisitionno'";
...
if($obj->action=="Receive"){
	...
	if(!empty($error)){
		...
		if($transfers->edit($transfers,$shptransfers)){
                         $query="update inv_transfers set status=1 where documentno='$obj->documentno'";
                        $ress=mysql_query($query);
...
if($obj->action2=="Add"){
	...
	$shptransfers[$it]=array('itemid'=>"$obj->itemid",'itemdetailid'=>"$obj->itemdetailid", 'serialno'=>"$obj->serialno", 'itemname'=>"$items->name", 'quantity'=>"1", 'code'=>"$obj->code", 'costprice'=>"$obj->costprice", 'memo'=>"$obj->memo", 'costprice'=>"$obj->costprice", 'installationcode'=>"$items->installationcode", 'instalcode'=>"$itemdetails->instalcode", 'version'=>"$obj->version");
	$it++;
	}
		$obj->iterator=$it;
...
//         echo $obj->invoiceno;
	...
	//echo "miketheshit";
		...
		while($row=mysql_fetch_object($res)){
			...
			$shptransfers[$it]=array('itemid'=>"$row->itemid",'itemdetailid'=>"$row->itemdetailid", 'status'=>"$row->status",'quantityrec'=>"$row->quantity",'serialno'=>"$row->serialno",'remaining'=>"$remaining", 'itemname'=>"$items->name", 'quantity'=>"$row->quantity", 'code'=>"$row->code", 'costprice'=>"$row->costprice", 'memo'=>"$row->memo",'crdcode'=>"$row->crdcode",'instalcode'=>"$row->instalcode", 'total'=>"$row->total",'crdcode'=>"$creditcodes->creditcode");
