...
$().ready(function() {
  ...
  $("#itemname").autocomplete({
	source:"../../../modules/server/server/search.php?main=inv&module=items&field=inv_items.name&where=status='Active' and (type!='complex' or type is null)",
	focus: function(event, ui) {
		event.preventDefault();
		$(this).val(ui.item.label);
      	},
...
function setTotal(id,val,dquantity,stock)
{ 
try{	
	if(val>stock && false){
	  $("#dquantity"+id).val(0);
	  alert("Cannot dispath more than is available!");
	  return 0;
	}else{
...
try{	
	...
	var url="getReorderLevels.php?itemid="+item+"&brancheid="+$("#brancheid").val();//alert(url);
	xmlhttp.open("GET",url,true);
	xmlhttp.send();
	}catch(e){alert(e);}
...
<!-- 		<th align='left'>Total</th> -->
	...
	if(!empty(count($_SESSION['shprequisitions']))){
		...
		$color="";
// 		if($shprequisitions[$i]['stock']<=0)
// 		  $color="red";
		?>
		<tr style="font-size:12px; vertical-align:text-top; color:<?php echo $color; ?>">
			<td><?php echo ($i+1); ?></td>
			<?php if(!empty($obj->action)){ ?>
			<td><input type="checkbox" name="<?php echo $shprequisitions[$i]['id']; ?>" <?php if($shprequisitions[$i]['tquantity']<=0 ){echo"disabled";}?>/></td>
			<?php } ?>
			...
			<?php if($obj->retrieve==1){ ?>
			<td><input type="text" id="dquantity<?php echo $i; ?>" size="4" onchange="setTotal('<?php echo $i; ?>',this.value, '<?php echo $shprequisitions[$i]['dquantity'];?>', '<?php echo $shprequisitions[$i]['stock'];?>');" value="<?php echo $shprequisitions[$i]['dquantity']; ?>"/></td>
			<?php } ?>
...
<table align="center" width="100%">
	...
	<tr>
		...
		<?php
		if($obj->status==0 and empty($ob->approve) and empty($obj->retrieve)){
		?>
...
<!-- 		<input type="submit" class="btn btn-info" name="action" id="action" value="Submit for Approval" /> -->
		...
		?>
		<input type="button" class="btn btn-warning" name="action" id="action" value="Print" onclick="Clickheretoprint();"/>
		<?php
// 		if($obj->status==2 and !empty($ob->action)){
		  ?>
		  <?php if(!empty($obj->dispatch)){ ?>
		  <input type="submit" class="btn btn-info" name="action" id="action" value="Dispatch" />
		  <?php } ?>
		  <?php if(!empty($obj->issue)){ ?>
...
// 		}
	...
	<tr>
		<td colspan="2" align="center">
		<?php if($issuance->affectedRows==0 and $requisitions->status==0 and $requisitions->createdby==$_SESSION['userid']){ ?><input type="submit" class="btn btn-danger" name="action" id="action" value="Revoke" /><?php } if($requisitions->status==1){ ?><input type="submit" name="action" id="action" class="btn btn-primary" value="ISSUE" /><?php } ?>
		<input class="btn btn-default" type="button" name="action" id="action" value="Print" onclick="Clickheretoprint();"/></td>
	</tr>-->
	<?php }?>
<?php if(!empty($obj->id)){?>
<?php }?>
