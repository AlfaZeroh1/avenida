...
// 		document.getElementById("stock").value=ui.item.stock;
...
// 		document.getElementById("quantity").value=1;
 ...
 function getStock(itemid){
  $.get("getStocks.php",{itemid:itemid,brancheid:$("#brancheid").val()},function(data){
    $("#stock").val(data);
  });
 }
</script>
...
<?php
	...
	<tr>
		...
		<input type="text" name="stock" id="stock" readonly onchange="calculateTotal();" onblur="calculateTotal();"  size="8" value="<?php echo $obj->stock; ?>"></td>
...
<table style="clear:both" class="table table-condensed table-striped table-bordered" id="tbl" cellpadding="0" align="center" width="100%" cellspacing="0">
	...
	if($num>0){
		...
		<tr <?php if($shptransfers[$i]['status']==1){ ?> style="color:green;" <?php } ?> >
			...
			<?php if($obj->receive==1 ){?>
			<td>
			<?php if($shptransfers[$i]['quantity']>$shptransfers[$i]['quantityrec']){ ?>
			<input type="checkbox" id="id<?php echo $i; ?>" <?php if($shptransfers[$i]['status']==1 and $obj->receive!=1){ ?> disabled <?php } ?> onChange="enableQuantity('<?php echo $i; ?>');" />
			<?php } ?>
			</td>
			<?php } ?>
