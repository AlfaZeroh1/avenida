...
<script type="text/javascript" charset="utf-8">
 <?php $_SESSION['aColumns']=array('inv_items.id','inv_items.code','inv_items.name', 'inv_departments.name departmentid', 'inv_items.retailprice','inv_items.costprice','inv_items.size','inv_unitofmeasures.name unitofmeasureid','inv_items.quantity','inv_items.package','inv_items.status','inv_items.reducing','sys_currencys.name currencyid','1','inv_items.value','(inv_items.value*inv_items.quantity) as stock','inv_items2.name as itemname','inv_items.type');?>
 <?php $_SESSION['sColumns']=array('id','code','name','departmentid','categoryid','costprice','size','unitofmeasureid','quantity','package','status','reducing','retailprice','1','value','stock','itemname','type');?>
 ...
 $(document).ready(function() {
 	...
 	$('#tbl').dataTable( {
		...
		"fnRowCallback": function( nRow, aaData, iDisplayIndex ) {
			...
			if(aaData[17]=="complex")
			  $('td:eq(2)', nRow).html("<a href='javascript:;' onclick='showPopWin(&quot;../compositeitems/compositeitems.php?itemid="+aaData[0]+"&quot;, 900, 600);'>"+aaData[2]+"</a>");
			else
