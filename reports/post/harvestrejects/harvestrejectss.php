...
$rptgroup='';
$track=0;
if(!empty($obj->grrejecttypeid) or !empty($obj->grsizeid) or !empty($obj->gritemid) or !empty($obj->grgreenhouseid)  or !empty($obj->grgradedon) or !empty($obj->grreportedon) or !empty($obj->gremployeeid) or !empty($obj->grstatus) or !empty($obj->grcreatedby) or !empty($obj->grcreatedon) ){
	$obj->shrejecttypeid='';
	$obj->shsizeid='';
	$obj->shitemid='';
	$obj->shgreenhouseid='';
...
// 		}
	...
	if(!empty($obj->shgreenhouseid)  or empty($obj->action)){
		...
		$rptjoin.=" left join prod_greenhouses on prod_greenhouses.id=post_harvestrejects.greenhouseid ";
		$k++;
		}
...
if(!empty($obj->reduce)){
	if($track>0)
		$rptwhere.="and";
		if($obj->reduce=="reduc")
		  $rptwhere.=" post_harvestrejects.reduce=''";
		else
...
if(!empty($obj->greenhouseid)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" post_harvestrejects.greenhouseid='$obj->greenhouseid'";
		$join=" left join prod_greenhouses on post_harvestrejects.id=prod_greenhouses.harvestrejectid ";
...
$sizes=new Sizes();
  ...
  while($rw=mysql_fetch_object($sizes->result)){
    $cols="case when sum(case when sizeid=$rw->id then post_harvestrejects.quantity end) is null then '' else sum(case when sizeid=$rw->id then post_harvestrejects.quantity end) end'$rw->name'";
  ...
  if(!empty($obj->shquantity)  or empty($obj->action)){
		array_push($sColumns, 'quantity');
		array_push($aColumns, "sum(post_harvestrejects.quantity) quantity");
		$k++;
		}
?>
...
<form  action="harvestrejectss.php" method="post" name="harvestrejects" >
	...
	<tr>
		...
		<table class="tgrid gridd" border="0" align="right">
			...
			<select name='createdby' class='selectbox'>
				...
				$where=" where auth_users.id in(select createdby from post_harvestrejects) ";
			...
			<tr>
			  ...
			  <select name="reduce" class="selectbox">
			    <option value="reduc" <?php if($obj->reduce=="reduc"){echo"selected";}?>>Packing Hall</option>
		...
		<table class="tgrid gridd" width="100%" border="0" align="left">
			...
			<tr>
				...
				<td><input type='checkbox' name='grgreenhouseid' value='1' <?php if(isset($_POST['grgreenhouseid']) ){echo"checked";}?>>&nbsp;Greenhouse</td>
		</table>
		</td>
		</tr>
		<tr>
		...
		<table class="tgrid gridd" width="100%" border="0" align="left">
			...
			<tr>
				<td><input type='checkbox' name='shgreenhouseid' value='1' <?php if(isset($_POST['shgreenhouseid'])  or empty($obj->action)){echo"checked";}?>>&nbsp;Greenhouse</td>
		</table>
		</td>
	</tr>
	<tr>
