...
// if(!empty($obj->status) and $obj->status=="checkedin")
//   $rptwhere=" post_graded.status='checkedin'";
// else
//   $rptwhere=" post_graded.status!='checkedin'";
...
//processing columns to show
	...
	if(!empty($obj->shdownsize) ){
		array_push($sColumns, 'downsize');
		array_push($aColumns, "post_graded.downsize");
		$k++;
		}
...
// 		}
	...
	if(!empty($obj->shcreatedby)  or empty($obj->action)){
		...
		$rptjoin.=" left join auth_users on auth_users.id=post_graded.createdby";
	}
	...
	if(!empty($obj->shcolourid)  or empty($obj->action)){
		...
		array_push($aColumns, "pos_colours.name as colourid");
		$k++;
		$join=" left join pos_items on post_graded.itemid=pos_items.id ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
		}
		$join=" left join pos_colours on pos_colours.id=pos_items.colourid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
		}
		}
	...
	if(!empty($obj->shteamid) ){
		...
		array_push($aColumns, "post_teams.name as teamid");
		$k++;
	...
	if(!empty($obj->shipaddress) ){
		...
		array_push($aColumns, "post_graded.ipaddress");
		$k++;
		}
...
if(!empty($obj->itemid)){
	if($track>0)
		...
		$join=" left join pos_items on post_graded.id=pos_items.gradeid ";
...
if(!empty($obj->colourid)){
	if($track>0)
		$rptwhere.="and";
	$rptwhere.=" post_graded.colourid='$obj->colourid' ";	
	$track++;
}
	...
	<tr>
		...
		<table class="tgrid gridd" border="0" align="right">
			...
			<tr>
			  ...
			  <td><select name="status">
				<option value="">Select...</option>
				<option value="checkedin" <?php if($obj->status=="checkedin"){echo"selected";}?>>Scanning In</option>
		...
		<table class="tgrid gridd" width="100%" border="0" align="left">
			...
			<tr>
				...
				<td><input type='checkbox' name='gritemid' value='1' <?php if(isset($_POST['gritemid']) ){echo"checked";}?>>&nbsp;Variety</td>
			<tr>
			...
			<tr>
		        ...
		        <tr> 
		                ...
		                <td><input type='checkbox' name='grdatecode' value='1' <?php if(isset($_POST['grdatecode'])){echo"checked";}?>>&nbsp;Date Code</td>
		</table>
		</td>
		</tr>
		<tr>
		...
		<table class="tgrid gridd" width="100%" border="0" align="left">
			...
			<tr>
				...
				<td><input type='checkbox' name='shitemid' value='1' <?php if(isset($_POST['shitemid'])  or empty($obj->action)){echo"checked";}?>>&nbsp;Variety</td>
			<tr>
			...
			<tr>
				<td><input type='checkbox' name='shdownsize' value='1' <?php if(isset($_POST['shdownsize'])  or empty($obj->action)){echo"checked";}?>>&nbsp;Down Size</td>
			...
			<tr>
			        ...
			        <td><input type='checkbox' name='shdatecode' value='1' <?php if(isset($_POST['shdatecode'])  or empty($obj->action)){echo"checked";}?>>&nbsp;Date Code</td>
