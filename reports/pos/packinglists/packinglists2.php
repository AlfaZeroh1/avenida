...
$rptgroup='';
$track=0;
if(!empty($obj->grdocumentno) or !empty($obj->grorderno) or !empty($obj->grcustomerid) or !empty($obj->grpackedon) or !empty($obj->grfleetid) or !empty($obj->gremployeeid) or !empty($obj->grcreatedby) or !empty($obj->grcreatedon) or !empty($obj->gritemid) or !empty($obj->grboxno) ){
	$obj->shdocumentno='';
	$obj->shorderno='';
	$obj->shcustomerid='';
	$obj->shpackedon='';
...
//processing columns to show
	...
	if(!empty($obj->shfleetid)  or empty($obj->action)){
		...
		array_push($aColumns, "assets_fleets.assetid as fleetid");
...
if(!empty($obj->shitemid)  or empty($obj->action)){
		...
		array_push($aColumns, "pos_packinglistdetails.itemid");
		$k++;
		...
		$join=" left join  pos_packinglistdetails on pos_packinglists.id=pos_packinglistdetails.itemid ";
