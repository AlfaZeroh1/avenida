...
require_once("../../../modules/pos/orderdetails/Orderdetails_class.php");
...
//processing columns to show
	if(!empty($obj->shitemid)  or empty($obj->action)){
		...
		$rptjoin.=" left join pos_items on pos_items.id=pos_orderdetails.itemid ";
		$k++;
		}
	...
	if(!empty($obj->shmemo)  or empty($obj->action)){
		...
		array_push($aColumns, "pos_orderdetails.memo");
		$k++;
		}

	if(!empty($obj->shcreatedby)  or empty($obj->action)){
		array_push($sColumns, 'createdby');
		array_push($aColumns, "pos_orderdetails.createdby");
		$k++;
		}

	if(!empty($obj->shcreatedon)  or empty($obj->action)){
		array_push($sColumns, 'createdon');
		array_push($aColumns, "pos_orderdetails.createdon");
		$k++;
		}

	if(!empty($obj->shipaddress) ){
		array_push($sColumns, 'ipaddress');
		array_push($aColumns, "pos_orderdetails.ipaddress");
		$k++;
		}
...
if(!empty($obj->itemid)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_orderdetails.itemid='$obj->itemid'";
		$join=" left join pos_items on pos_orderdetails.id=pos_items.orderdetailid ";
...
if(!empty($obj->createdby)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_orderdetails.createdby='$obj->createdby'";
	$track++;
}
...
if(!empty($obj->fromcreatedon)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_orderdetails.createdon>='$obj->fromcreatedon'";
	$track++;
}
...
if(!empty($obj->tocreatedon)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_orderdetails.createdon<='$obj->tocreatedon'";
	$track++;
}
...
<script type="text/javascript" charset="utf-8">
 ...
 <?php $_SESSION['sTable']="pos_orderdetails";?>
 <?php $_SESSION['sOrder']="";?>
 ...
 $(document).ready(function() {
 	...
 	$('#tbl').dataTable( {
		...
		"bJQueryUI": true,
		"bRetrieve":true,
		"sAjaxSource": "../../../modules/server/server/processing.php?sTable=pos_orderdetails",
...
<form  action="orderdetails.php" method="post" name="orderdetails" >
	...
	<tr>
		...
		<table class="tgrid gridd" border="0" align="right">
			...
			<select name='createdby' class='selectbox'>
				...
				$users = new Users();
				$fields="*";
				$where="where id in(select createdby from pos_orderdetails)";
				$join="   ";
				$having="";
				$groupby="";
				$orderby="";
