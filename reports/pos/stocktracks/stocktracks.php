...
require_once("../../../modules/pos/stocktracks/Stocktracks_class.php");
...
<form  action="stocktracks.php" method="post" name="stocktracks">
<table width="100%" border="0" align="center">
	<tr>
		...
		<table class="tgrid gridd" width="100%" border="0" align="left">
			...
			<tr>
				<td><input type='checkbox' name='shrecorddate' value='1' <?php if(isset($_POST['shrecorddate'])  or empty($obj->action)){echo"checked";}?>>&nbsp;Date</td>
...
<table style="clear:both;"  class="tgrid display" id="example" width="100%" border="0" cellspacing="0" cellpadding="2" align="center" >
	<thead>
		<tr>
			<th>#</th>
			<?php if($obj->shrecorddate==1  or empty($obj->action)){ ?>
				<th>Record Date </th>
			<?php } ?>
	...
	<?php
		...
		$fields="pos_stocktracks.id, pos_items.name as itemid, pos_stocktracks.tid, pos_stocktracks.documentno, pos_stocktracks.batchno, pos_stocktracks.quantity, pos_stocktracks.costprice, pos_stocktracks.value, pos_stocktracks.discount, pos_stocktracks.tradeprice, pos_stocktracks.retailprice, pos_stocktracks.tax, pos_stocktracks.expirydate, pos_stocktracks.recorddate, pos_stocktracks.status, pos_stocktracks.remain, pos_stocktracks.transaction, pos_stocktracks.createdby, pos_stocktracks.createdon, pos_stocktracks.lasteditedby, pos_stocktracks.lasteditedon, pos_stocktracks.ipaddress".$fds.$fd;
