...
//processing columns to show
	...
	if(!empty($obj->shitemid)   or empty($obj->action)){
		...
		array_push($aColumns, "concat(inv_items.name,' ',(case when pos_orderdetails.warmth=1 then 'Warm' when pos_orderdetails.warmth=2 then 'Cold' when pos_orderdetails.warmth=3 then 'WETFRY' when pos_orderdetails.warmth=4 then 'DRYFRY' when pos_orderdetails.warmth=5 then 'BOILED' when pos_orderdetails.warmth=6 then 'CHOMA' else '' end)) itemid");
		$k++;
		$join=" left join pos_orderdetails on pos_orders.id=pos_orderdetails.orderid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
	...
	if(!empty($obj->shcreatedby)  or empty($obj->action)){
		...
		array_push($aColumns, "case when concat(hrm_employees.firstname,' ',concat(hrm_employees.middlename,' ',hrm_employees.lastname)) is null then auth_users.username else concat(hrm_employees.firstname,' ',concat(hrm_employees.middlename,' ',hrm_employees.lastname)) end createdby");
		
		$join=" left join auth_users on auth_users.id=pos_orders.createdby ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
		}
...
if(empty($obj->type))
  ...
  $rptwhere=" (pos_orders.status=3 or pos_orderdetails.voids>0) ";
//processing filters
...
if(!empty($obj->brancheid2)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_orderdetails.brancheid2='$obj->brancheid2'";
	$join=" left join pos_orderdetails on pos_orders.id=pos_orderdetails.orderid ";
	if(!strpos($rptjoin,trim($join))){
		$rptjoin.=$join;
	}
...
if(!empty($obj->fromcreatedon)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" date(pos_orders.createdon)>='$obj->fromcreatedon'";
	$track++;
}
...
if(!empty($obj->tocreatedon)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" date(pos_orders.createdon)<='$obj->tocreatedon'";
	$track++;
}
...
if(!empty($obj->shquantity)   or empty($obj->action) ){
		...
		if(empty($obj->type)){
            if(!empty($rptgroup)){
                array_push($aColumns, "sum(pos_orderdetails.quantity-(case when pos_orderdetails.voids is null then 0 else pos_orderdetails.voids end)) as quantity");
            }else{
            array_push($aColumns, "(pos_orderdetails.quantity-(case when pos_orderdetails.voids is null then 0 else pos_orderdetails.voids end)) as quantity");
            }
		}else{
            if(!empty($rptgroup)){
                array_push($aColumns, "sum(pos_orderdetails.voids) as quantity");
            }else{
            array_push($aColumns, "(pos_orderdetails.voids) as quantity");
            }
		}
		$k++;
...
if(!empty($obj->shprice)  or empty($obj->action) ){
		...
		if(!empty($rptgroup)){
		  array_push($aColumns, "sum((pos_orderdetails.quantity-(case when pos_orderdetails.voids is null then 0 else pos_orderdetails.voids end))*pos_orderdetails.price) as price");
		}else{
		  array_push($aColumns, "(pos_orderdetails.quantity-(case when pos_orderdetails.voids is null then 0 else pos_orderdetails.voids end))*pos_orderdetails.price as price");
		}
