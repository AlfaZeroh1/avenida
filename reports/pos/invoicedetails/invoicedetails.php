...
//processing columns to show
	if(!empty($obj->shinvoiceid)  or empty($obj->action)){
		array_push($sColumns, 'invoiceid');
		array_push($aColumns, "pos_invoices.name as invoiceid");
		$rptjoin.=" left join pos_invoices on pos_invoices.id=pos_invoicedetails.invoiceid ";
		$k++;
		}
	...
	if(!empty($obj->shitemid)  or empty($obj->action)){
		...
		$rptjoin.=" left join pos_items on pos_items.id=pos_invoicedetails.itemid ";
		$k++;
		}
	...
	if(!empty($obj->shdiscount)  or empty($obj->action)){
		...
		array_push($aColumns, "pos_invoicedetails.discount");
		$k++;
		}
	...
	if(!empty($obj->shcreatedby)  or empty($obj->action)){
		...
		array_push($aColumns, "pos_invoicedetails.createdby");
		$k++;
		}

	if(!empty($obj->shcreatedon)  or empty($obj->action)){
		array_push($sColumns, 'createdon');
		array_push($aColumns, "pos_invoicedetails.createdon");
		$k++;
		}
...
if(!empty($obj->itemid)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_invoicedetails.itemid='$obj->itemid'";
		$join=" left join pos_items on pos_invoicedetails.id=pos_items.invoicedetailid ";
...
if(!empty($obj->createdby)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_invoicedetails.createdby='$obj->createdby'";
	$track++;
}
...
if(!empty($obj->fromcreatedon)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_invoicedetails.createdon>='$obj->fromcreatedon'";
	$track++;
}
...
if(!empty($obj->tocreatedon)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_invoicedetails.createdon<='$obj->tocreatedon'";
	$track++;
}
...
<form  action="invoicedetails.php" method="post" name="invoicedetails" >
	...
	<tr>
		...
		<table class="tgrid gridd" border="0" align="right">
			...
			<select name='createdby' class='selectbox'>
				...
				$users = new Users();
				$fields="*";
				$where="where id in(select createdby from pos_invoicedetails)";
				$join="   ";
				$having="";
				$groupby="";
				$orderby="";
		...
		<table class="tgrid gridd" width="100%" border="0" align="left">
			...
			<tr>
				<td><input type='checkbox' name='shinvoiceid' value='1' <?php if(isset($_POST['shinvoiceid'])  or empty($obj->action)){echo"checked";}?>>&nbsp;Invoice</td>
...
<table style="clear:both;"  class="table" id="tbl" width="100%" border="0" cellspacing="0" cellpadding="2" align="center" >
	<thead>
		<tr>
			<th>#</th>
			<?php if($obj->shinvoiceid==1  or empty($obj->action)){ ?>
				<th>Invoice </th>
			<?php } ?>
