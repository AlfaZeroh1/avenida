...
$sizes=new Sizes();
  ...
  while($rw=mysql_fetch_object($sizes->result)){
    $cols=" case when sum(case when sizeid=$rw->id then pos_itemstocks.quantity end) <0 then 0 else sum(case when sizeid=$rw->id then pos_itemstocks.quantity end) end '$rw->name'";
  ...
  if(!empty($obj->shquantity)  or empty($obj->action)){
		array_push($sColumns, 'quantity');
		array_push($aColumns, "case when sum(pos_itemstocks.quantity)<0 then 0 else sum(pos_itemstocks.quantity) end quantity");
		$k++;
		}
?>
...
<table width="100%" border="0" align="center">
	<tr>
		...
		<table class="tgrid gridd" border="0" align="right">
			...
			<tr>
			<td><input type='checkbox' name='outofstock' value='1' <?php if(isset($_POST['outofstock']) ){echo"checked";}?>>&nbsp;Out of Stock</td>
			</tr>
		</table>
		</td>
		<td>
