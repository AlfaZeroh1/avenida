...
require_once("../../../modules/pos/sales/Sales_class.php");
require_once("../../../modules/pos/saledetails/Saledetails_class.php");
require_once("../../../modules/auth/users/Users_class.php");
...
$rptgroup='';
$track=0;
if(!empty($obj->gritemid) or !empty($obj->grdocumentno)or !empty($obj->gragentid) or !empty($obj->grsoldon) or !empty($obj->grcreatedby) or !empty($obj->grcreatedon) or !empty($obj->grmode) or !empty($obj->grsaleid) ){
	$obj->shitemid='';
	$obj->shdocumentno='';
...
//processing columns to show
	...
	if(!empty($obj->shquantity)  or empty($obj->action)){
		...
		if(!empty($rptgroup))
		  array_push($aColumns, "sum(pos_saledetails.quantity) quantity");
		else
		  array_push($aColumns, "pos_saledetails.quantity");
		$k++;
		$join=" left join pos_saledetails on pos_sales.id=pos_saledetails.saleid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
	...
	if(!empty($obj->shcostprice) ){
		...
		array_push($aColumns, "pos_saledetails.costprice");
		$k++;
		$join=" left join pos_saledetails on pos_sales.id=pos_saledetails.saleid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
	...
	if(!empty($obj->shtradeprice) ){
		...
		array_push($aColumns, "pos_saledetails.tradeprice");
		$k++;
		$join=" left join pos_saledetails on pos_sales.id=pos_saledetails.saleid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
	...
	if(!empty($obj->shretailprice)  or empty($obj->action)){
		...
		array_push($aColumns, "pos_saledetails.retailprice");
		$k++;
		$join=" left join pos_saledetails on pos_sales.id=pos_saledetails.saleid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
	...
	if(!empty($obj->shtax) ){
		...
		array_push($aColumns, "pos_saledetails.tax");
		$k++;
		$join=" left join pos_saledetails on pos_sales.id=pos_saledetails.saleid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
	...
	if(!empty($obj->bonus) ){
		...
		array_push($aColumns, "pos_saledetails.bonus");
		$k++;
		$join=" left join pos_saledetails on pos_sales.id=pos_saledetails.saleid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
	...
	if(!empty($obj->shtotal)  or empty($obj->action)){
		array_push($sColumns, 'total');
		 if(!empty($rptgroup))
		  array_push($aColumns, "case when sum(pos_saledetails.total)>0 then sum(pos_saledetails.total) else 0 end total");
		else
		  array_push($aColumns, "case when pos_saledetails.total>0 then pos_saledetails.total else 0 end total");
		$k++;
		$join=" left join pos_saledetails on pos_sales.id=pos_saledetails.saleid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
	...
	if(!empty($obj->shmemo)  or empty($obj->action)){
		...
		if(!empty($rptgroup))
		  array_push($aColumns, "memo");
		else
		  array_push($aColumns, "pos_saledetails.memo");
		$k++;
		$join=" left join pos_saledetails on pos_sales.id=pos_saledetails.saleid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
	...
	if(!empty($obj->shcreatedby)  or empty($obj->action)){
		...
		$rptjoin.=" left join auth_users on auth_users.id=pos_sales.createdby ";
		$k++;
		}
...
<table width="100%" border="0" align="center">
	<tr>
		...
		<table class="tgrid gridd" width="100%" border="0" align="left">
			...
			<tr>
			<td><input type='checkbox' name='grmemo' value='1' <?php if(isset($_POST['grmemo']) ){echo"checked";}?>>&nbsp;Customer</td>
			<tr>
		...
		<table class="tgrid gridd" width="100%" border="0" align="left">
			...
			<tr>
			        <td><input type='checkbox' name='shmemo' value='1' <?php if(isset($_POST['shmemo']) ){echo"checked";}?>>&nbsp;Customer</td>
			<tr>
