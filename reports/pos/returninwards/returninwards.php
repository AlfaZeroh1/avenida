...
$rptgroup='';
$track=0;
if(!empty($obj->grcustomerid) or !empty($obj->grdocumentno) or !empty($obj->grpackingno) or !empty($obj->gritemid) or !empty($obj->grreturnedon) or !empty($obj->grcreatedby) or !empty($obj->grcreatedon) or !empty($obj->grsizeid) or !empty($obj->grcurrencyid) or !empty($obj->grvatable) or !empty($obj->grinvoiceno) ){
	$obj->shcustomerid='';
	$obj->shdocumentno='';
	$obj->shpackingno='';
	$obj->shitemid='';
	...
	$obj->shtotal=1;
	$obj->shquantity=1;
	$obj->grexchangerate=1;
	$obj->grexchangerate2=1;
...
//processing columns to show
	...
	if(!empty($obj->shitemid)  or empty($obj->action)){
		...
		array_push($aColumns, " case when pos_items.name is null then inv_items.name else pos_items.name end itemid");
		$k++;
		...
		$join=" left join pos_returninwardconsumables on pos_returninwards.id=pos_returninwardconsumables.returninwardid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
		}
		$join=" left join  inv_items on inv_items.id=pos_returninwardconsumables.itemid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
		}
		}
	...
	if(!empty($obj->shquantity)  or empty($obj->action)){
		...
		if(!empty($rptgroup))
		  array_push($aColumns, "sum(pos_returninwarddetails.quantity) quantity");
		else
		  array_push($aColumns, "pos_returninwarddetails.quantity");
		$k++;
		$join=" left join pos_returninwarddetails on pos_returninwards.id=pos_returninwarddetails.returninwardid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
	...
	if(!empty($obj->shprice)  or empty($obj->action)){
		...
		array_push($aColumns, "pos_returninwarddetails.price");
		$k++;
		$join=" left join pos_returninwarddetails on pos_returninwards.id=pos_returninwarddetails.returninwardid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
	...
	if(!empty($obj->shtotal)  or empty($obj->action)){
		array_push($sColumns, 'total');
		 if(!empty($rptgroup))
		  array_push($aColumns, "round( case when sum(pos_returninwarddetails.quantity*pos_returninwarddetails.price) is null then 0 else sum(pos_returninwarddetails.quantity*pos_returninwarddetails.price) end +(select case when sum(pos_returninwardconsumables.total) is null then 0 else sum(pos_returninwardconsumables.total) end from pos_returninwardconsumables where returninwardid=pos_returninwards.id),2) total");
		else
		  array_push($aColumns, "round(pos_returninwarddetails.total,2)");
		$k++;
		$join=" left join pos_returninwarddetails on pos_returninwards.id=pos_returninwarddetails.returninwardid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
	...
	<tr>
		...
		<table class="tgrid gridd" width="100%" border="0" align="left">
			...
			<tr>
				...
				<td><input type='checkbox' name='shvat' value='1' <?php if(isset($_POST['shvat']) ){echo"checked";}?>>&nbsp;Vat</td>
			<tr>
