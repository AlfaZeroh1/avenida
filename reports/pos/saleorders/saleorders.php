...
require_once("../../../modules/pos/saleorders/Saleorders_class.php");
...
//processing columns to show
	if(!empty($obj->shitemid)  or empty($obj->action)){
		...
		$rptjoin.=" left join inv_items on inv_items.id=pos_saleorders.itemid ";
	}

	if(!empty($obj->shdocumentno)  or empty($obj->action)){
		array_push($sColumns, 'documentno');
		array_push($aColumns, "pos_saleorders.documentno");
	}

	if(!empty($obj->shcustomerid) ){
		array_push($sColumns, 'customerid');
		array_push($aColumns, "crm_customers.name as customerid");
		$rptjoin.=" left join crm_customers on crm_customers.id=pos_saleorders.customerid ";
	}

	if(!empty($obj->shagentid) ){
		array_push($sColumns, 'agentid');
		array_push($aColumns, "crm_agents.name as agentid");
		$rptjoin.=" left join crm_agents on crm_agents.id=pos_saleorders.agentid ";
	}
...
if(!empty($obj->itemid)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_saleorders.itemid='$obj->itemid'";
	$track++;
}
...
if(!empty($obj->documentno)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_saleorders.documentno='$obj->documentno'";
	$track++;
}
...
if(!empty($obj->customerid)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_saleorders.customerid='$obj->customerid'";
	$track++;
}
...
if(!empty($obj->agentid)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_saleorders.agentid='$obj->agentid'";
	$track++;
}
...
if(!empty($obj->employeeid)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_saleorders.employeeid='$obj->employeeid'";
	$track++;
}
...
if(!empty($obj->fromquantity)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_saleorders.quantity>='$obj->fromquantity'";
	$track++;
}
...
if(!empty($obj->toquantity)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_saleorders.quantity<='$obj->toquantity'";
	$track++;
}
...
if(!empty($obj->quantity)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_saleorders.quantity='$obj->quantity'";
	$track++;
}

if(!empty($obj->fromprofit)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_saleorders.profit>='$obj->fromprofit'";
	$track++;
}

if(!empty($obj->toprofit)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_saleorders.profit<='$obj->toprofit'";
	$track++;
}
...
if(!empty($obj->profit)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_saleorders.profit='$obj->profit'";
	$track++;
}
...
if(!empty($obj->fromsoldon)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_saleorders.soldon>='$obj->fromsoldon'";
	$track++;
}
...
if(!empty($obj->tosoldon)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_saleorders.soldon<='$obj->tosoldon'";
	$track++;
}
...
if(!empty($obj->fromexpirydate)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_saleorders.expirydate>='$obj->fromexpirydate'";
	$track++;
}
...
if(!empty($obj->toexpirydate)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_saleorders.expirydate<='$obj->toexpirydate'";
	$track++;
}
...
if(!empty($obj->fromcreatedon)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_saleorders.createdon>='$obj->fromcreatedon'";
	$track++;
}
...
if(!empty($obj->tocreatedon)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_saleorders.createdon<='$obj->tocreatedon'";
	$track++;
}
...
if(!empty($obj->createdby)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" pos_saleorders.createdby='$obj->createdby'";
	$track++;
}
...
<script type="text/javascript" charset="utf-8">
 ...
 <?php $_SESSION['sTable']="pos_saleorders";?>
 <?php $_SESSION['sOrder']="";?>
 ...
 $(document).ready(function() {
 	...
 	$('#tbl').dataTable( {
		...
		"bJQueryUI": true,
		"bRetrieve":true,
		"sAjaxSource": "../../../modules/server/server/processing.php?sTable=pos_saleorders",
...
<form  action="saleorders.php" method="post" name="saleorders" class='forms'>
<table width="100%" border="0" align="center">
	<tr>
		...
		<table class="tgrid gridd" border="0" align="right">
			...
			<tr>
				<td>Profit</td>
				<td><strong>From:</strong><input type='text' id='fromprofit' size='from20' name='fromprofit' value='<?php echo $obj->fromprofit;?>'/>
								<br/><strong>To:</strong><input type='text' id='toprofit' size='to20' name='toprofit' value='<?php echo $obj->toprofit;?>'></td>
			</tr>
			<tr>
				<td>Sold On</td>
		...
		<table class="tgrid gridd" width="100%" border="0" align="left">
			...
			<tr>
				<td><input type='checkbox' name='grexpirydate' value='1' <?php if(isset($_POST['grexpirydate']) ){echo"checked";}?>>&nbsp;Expiry Date</td>
		...
		<table class="tgrid gridd" width="100%" border="0" align="left">
			...
			<tr>
				<td><input type='checkbox' name='shcustomerid' value='1' <?php if(isset($_POST['shcustomerid']) ){echo"checked";}?>>&nbsp;Customer</td>
