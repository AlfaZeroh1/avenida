...
if(empty($obj->action)){
	$obj->frominvoicedon=date('Y-m-d');
	$obj->toinvoicedon=date('Y-m-d');
}
...
//processing columns to show
	...
	if(!empty($obj->shamount)  or empty($obj->action)){
		...
		if(empty($rptgroup))
		  ...
		  array_push($aColumns, "sum(hos_payables.amount) amount");
		$k++;
...
if(!empty($obj->patientid)){
	if($track>0)
		...
		$join=" left join hos_patients on hos_payables.id=hos_patients.payableid ";
...
if(!empty($obj->transactionid)){
	if($track>0)
		...
		$join=" left join sys_transactions on hos_payables.id=sys_transactions.payableid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
		}
	$track++;
...
if(!empty($obj->departmentid)){
	if($track>0)
		...
		$join=" left join hos_departments on hos_payables.id=hos_departments.payableid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
		}
	$track++;
	...
	<tr>
		...
		<table class="tgrid gridd" border="0" align="right">
			...
			<tr>
				<td>Pay Status</td>
				<td>
				<select name='paid' class='selectbox'>
				<option value="">Select...</option>
				<option value='Yes' <?php if($obj->gender=='Yes'){echo"selected";}?>>Yes</option>
				<option value='No' <?php if($obj->gender=='No'){echo"selected";}?>>No</option>
		</select></td>
