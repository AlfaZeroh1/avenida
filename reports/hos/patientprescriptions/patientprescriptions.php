...
if(!empty($obj->itemid)){
	if($track>0)
		...
		$join=" left join inv_items on hos_patientprescriptions.id=inv_items.patientprescriptionid ";
...
if(!empty($obj->patienttreatmentid)){
	if($track>0)
		...
		$join=" left join hos_patienttreatments on hos_patientprescriptions.id=hos_patienttreatments.patientprescriptionid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
		}
	$track++;
