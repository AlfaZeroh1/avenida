...
if(empty($obj->action)){
	$obj->fromspraydate=date('Y-m-d');
	$obj->tospraydate=date('Y-m-d');
}
...
$rptgroup='';
$track=0;
if(!empty($obj->grareaid) or !empty($obj->grvarietyid) or !empty($obj->grchemicalid) or !empty($obj->grnozzleid) or !empty($obj->grspraymethodid) or !empty($obj->grcreatedby) or !empty($obj->grcreatedon) or !empty($obj->grspraydate) ){
	$obj->shareaid='';
	$obj->shvarietyid='';
	$obj->shchemicalid='';
	$obj->shingredients='';
...
if(!empty($obj->areaid)){
	if($track>0)
		...
		$join=" left join prod_areas on prod_sprayprogrammes.id=prod_areas.sprayprogrammeid ";
...
if(!empty($obj->blockid)){
	if($track>0)
		...
		$join=" left join prod_blocks on prod_sprayprogrammes.id=prod_blocks.sprayprogrammeid ";
...
if(!empty($obj->nozzleid)){
	if($track>0)
		...
		$join=" left join prod_nozzles on prod_sprayprogrammes.id=prod_nozzles.sprayprogrammeid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
		}
	$track++;
...
if(!empty($obj->spraymethodid)){
	if($track>0)
		...
		$join=" left join prod_spraymethods on prod_sprayprogrammes.id=prod_spraymethods.sprayprogrammeid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
		}
	$track++;
