...
$rptgroup='';
$track=0;
if(!empty($obj->grname) or !empty($obj->grcolourid) or !empty($obj->grtypeid) or !empty($obj->shduration) or !empty($obj->shquantity) or !empty($obj->shremarks)){
	$obj->shtypeid='';
	$obj->shcolourid='';
	$obj->shduration='';
	$obj->shquantity='';
...
// // 	if($track>0) TableToolsInit.sSwfPath = "../../../media/swf/ZeroClipboard.swf";
// 	$rptgroup.=", ";
// // 	else
// 	$rptgroup.=" group by ";
// 
	...
	<thead>
		<tr>
			...
			<?php if($obj->shremarks==1  or empty($obj->action)){ ?>
				<th>Remarks </th>
			<?php } ?>
			<?php if($obj->shvariety==1  or empty($obj->action)){ ?>
				<th>Variety </th>
			<?php } ?>
		</tr>
	</thead>
