...
if(empty($obj->action)){
	$obj->fromcheckedon=date('Y-m-d');
	$obj->tocheckedon=date('Y-m-d');
}
...
//processing columns to show
	...
	if(!empty($obj->shcreatedon)  or empty($obj->action)){
		...
		array_push($aColumns, "prod_qualitychecks.createdon");
		$k++;
	}
	...
	<tr>
		...
		<table class="tgrid gridd" border="0" align="right">
			...
			<select name='createdby' class='selectbox'>
				...
				$where=" where auth_users.id in(select createdby from prod_qualitychecks) ";
