...
$rptgroup='';
$track=0;
if(!empty($obj->grgreenhouseid) or !empty($obj->grvarietyid) or !empty($obj->gremployeeid) or !empty($obj->grplantedon) or !empty($obj->grcreatedon) or !empty($obj->grcreatedby) or !empty($obj->grbreederid) ){
	$obj->shgreenhouseid='';
	$obj->shvarietyid='';
	$obj->shheadsize='';
	$obj->shemployeeid='';
...
//processing columns to show
	if(!empty($obj->shgreenhouseid)  or empty($obj->action)){
		...
		$rptjoin.=" left join prod_greenhouses on prod_greenhouses.id=prod_greenhousevarietys.greenhouseid ";
		$k++;
		}
	...
	if(!empty($obj->shvarietyid)  or empty($obj->action)){
		...
		$rptjoin.=" left join prod_varietys on prod_varietys.id=prod_greenhousevarietys.varietyid ";
		$k++;
		}
	...
	if(!empty($obj->shcreatedby)  or empty($obj->action)){
		...
		$rptjoin.=" left join auth_users on auth_users.id=prod_greenhousevarietys.createdby ";
		$k++;
		}
	...
	if(!empty($obj->shbreederid)  or empty($obj->action)){
		...
		$rptjoin.=" left join prod_breeders on prod_breeders.id=prod_greenhousevarietys.breederid ";
		$k++;
		}

	if(!empty($obj->shplants) ){
		array_push($sColumns, 'plants');
		array_push($aColumns, "prod_greenhousevarietys.plants");
		$k++;
		}
...
if(!empty($obj->frombreederid)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" prod_greenhousevarietys.breederid>='$obj->frombreederid'";
	$track++;
}

if(!empty($obj->tobreederid)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" prod_greenhousevarietys.breederid<='$obj->tobreederid'";
		$join=" left join prod_breeders on prod_breeders.greenhousevarietyid=prod_greenhousevarietys.id ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
		}
	$track++;
	...
	<tr>
		...
		<table class="tgrid gridd" border="0" align="right">
			...
			<select name='createdby' class='selectbox'>
				...
				$where=" where auth_users.id in(select createdby from prod_greenhousevarietys) ";
		...
		<table class="tgrid gridd" width="100%" border="0" align="left">
			...
			<tr>
				<td><input type='checkbox' name='shplants' value='1' <?php if(isset($_POST['shplants']) ){echo"checked";}?>>&nbsp;Plants</td>
		</table>
		</td>
	</tr>
	<tr>
