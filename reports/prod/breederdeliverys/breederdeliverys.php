...
//processing columns to show
	...
	if(!empty($obj->shipaddress)  or empty($obj->action)){
		...
		array_push($aColumns, "prod_breederdeliverys.ipaddress");
		$k++;
	}
	...
	if(!empty($obj->shcreatedby)  or empty($obj->action)){
		...
		$rptjoin.=" left join auth_users on auth_users.id=prod_breederdeliverys.createdby ";
		$k++;
		}
	
	if(!empty($obj->shcreatedon)  or empty($obj->action)){
		array_push($sColumns, 'createdon');
		array_push($aColumns, "prod_breederdeliverys.createdon");
		$k++;
	}
...
if(!empty($obj->todeliveredon)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" prod_breederdeliverys.ipaddress<='$obj->ipaddress'";
	$track++;
}
...
if(!empty($obj->week)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" prod_breederdeliverys.createdby='$obj->createdby'";
	$track++;
}
...
if(!empty($obj->planted)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" prod_breederdeliverys.createdon='$obj->createdon'";
	$track++;
}
...
<form  action="breederdeliverys.php" method="post" name="breederdeliverys" class=''>
<table width="100%" border="0" align="center">
	<tr>
		...
		<tr>
		<td>
		<table id="tbl" width="100%" border="0" align="left">
			<tr>
