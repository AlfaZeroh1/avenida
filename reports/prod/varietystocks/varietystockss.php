...
$sizes=new Sizes();
  ...
  while($rw=mysql_fetch_object($sizes->result)){
   $cols=" case when sum(case when sizeid=$rw->id then prod_varietystocks.quantity end) is null then '' else sum(case when sizeid=$rw->id then prod_varietystocks.quantity end) end '$rw->name'";
 ...
 $(document).ready(function() {
 	...
 	$('#tbl').dataTable( {
		...
		"fnRowCallback": function( nRow, aaData, iDisplayIndex ) {
			...
			for(var i=1; i<num; i++){
				if(i==1)
				  $('td:eq('+i+')', nRow).html("<a href='varietystocks.php?varietyid='"+aaData[i]+"'>"+aaData[i]+"</a>");
				else
	...
	<tr>
		...
		<table class="tgrid gridd" border="0" align="right">
			...
			<select name='createdby' class='selectbox'>
				...
				$where=" where auth_users.id in(select createdby from prod_varietystocks) ";
