...
if(!empty($obj->sharea)){
		...
		array_push($aColumns, "sum(prod_greenhousevarietys.area) area");
		
		$join=" left join prod_greenhousevarietys on prod_greenhousevarietys.varietyid=prod_harvests.varietyid and prod_greenhousevarietys.greenhouseid=prod_harvests.greenhouseid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
		}
		$k++;
 ...
 $(document).ready(function() {
 	...
 	var tbl = $('#tbl').dataTable( {
		...
		"fnRowCallback": function( nRow, aaData, iDisplayIndex ) {
			...
			for(var i=1; i<num; i++){
				if(aaData[1]=='' || aaData[2]=='' || aaData[3]=='' || isNull(aaData[1]) || isNull(aaData[2]) || isNull(aaData[3])){
				...
				else if(aaData[4]=='' || isNull(aaData[4])){
		...
		"fnFooterCallback": function ( nRow, aaData, iStart, iEnd, aiDisplay ) {
			...
			for(var i=0; i<aaData.length; i++){
			  ...
			  for(var j=<?php echo $mnt; ?>; j<aaData[i].length; j++){
			    ...
			    //if(j%2==0 && j<="<?php //echo $end; ?>"){
			      if(aaData[i][j]=='')
				aaData[i][j]=0;			      
				
				if(i==0)
				  total[j]=0;
				  <?php if(!empty($obj->shperc)){?>
				  if(j%2!=0 && j<="<?php echo $end; ?>"){
				  <?php }?>
				    <?php if(!empty($obj->grgreenhouseid)){?> 
				    if((aaData[i][1]!='' && aaData[i][2]!='' && aaData[i][3]!='' && aaData[i][4]!='') )
				      total[j] = changeNum(total[j])+changeNum(aaData[i][j]);	//alert(parseFloat(aaData[i][j]));	
				    <?php }else{?>
				      total[j] = changeNum(total[j])+changeNum(aaData[i][j]);
				    <?php }?>
				  <?php if(!empty($obj->shperc)){?>
				  }
				  <?php }?>
