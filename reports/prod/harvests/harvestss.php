...
<?php
 $sizes=new Sizes();
  ...
  while($rw=mysql_fetch_object($sizes->result)){
    $cols=" case when sum(case when sizeid=$rw->id then prod_harvests.quantity end) is null then '' else sum(case when sizeid=$rw->id then prod_harvests.quantity end) end '$rw->name'";
...
if(!empty($obj->shbudgets) ){
		...
		array_push($aColumns, "(select sum(prod_productionbudgets.quantity) from prod_productionbudgets left join prod_greenhousevarietys on prod_greenhousevarietys.id=prod_productionbudgets.greenhousevarietyid where month='$obj->month' and year='$obj->year' and varietyid=prod_harvests.varietyid) budgets"); 
		$k++;
		
		array_push($sColumns, 'percbudgets');
		array_push($aColumns, "concat(round((sum(prod_harvests.quantity)/(select sum(prod_productionbudgets.quantity) from prod_productionbudgets left join prod_greenhousevarietys on prod_greenhousevarietys.id=prod_productionbudgets.greenhousevarietyid where month='$obj->month' and year='$obj->year' and varietyid=prod_harvests.varietyid))*100,2),'','%') percbudgets");
		$k++;
		}
if(!empty($obj->sharea) ){
 ...
 $(document).ready(function() {
 	...
 	var tbl = $('#tbl').dataTable( {
		...
		"fnRowCallback": function( nRow, aaData, iDisplayIndex ) {
			...
			var r;
			for(var i=1; i<num; i++){
				if((aaData[1]==null || aaData[2]==null || aaData[3]==null) || (aaData[1]=='' || aaData[2]=='' || aaData[3]=='') ){//alert("");
				...
				else if(aaData[4]==null || aaData[4]==''){
				  $('td:eq(0)', nRow).html('').addClass("hide");
				  $('td:eq('+i+')', nRow).html('<strong>'+aaData[i]+'</strong>').addClass("hide");
				 <?php if(!empty($obj->grgreenhouseid) and !empty($obj->grvarietyid)) { ?>
				 tbl.fnDeleteRow(iDisplayIndex);
				 <?php } ?>
				}
				else{
				$('td:eq(0)', nRow).html(iDisplayIndex+1);
		...
		"fnFooterCallback": function ( nRow, aaData, iStart, iEnd, aiDisplay ) {
			...
			for(var i=0; i<aaData.length; i++){
			  ...
			  for(var j=<?php echo $mnt; ?>; j<aaData[i].length; j++){
				...
				if((aaData[i][1]!=null && aaData[i][2]!=null && aaData[i][3]!=null && aaData[i][4]!=null) && (aaData[i][1]!='' && aaData[i][2]!='' && aaData[i][3]!='' && aaData[i][4]!='') )
...
<table width="100%" border="0" align="center">
	<tr>
		...
		<table class="tgrid gridd" border="0" align="right">
			...
			<tr>
				<td>Quantity</td>
				<td><input type='text' id='quantity' size='16' name='quantity' value='<?php echo $obj->fromquantity;?>'></td>
			</tr>
			<tr>
				<td>Harvest Date</td>
