...
//processing columns to show
	...
	if(!empty($obj->shpaymenttermid)  or empty($obj->action)){
		array_push($sColumns, 'paymenttermid');
		array_push($aColumns, "em_paymentterms.name as paymenttermid");
		$rptjoin.=" left join em_paymentterms on em_paymentterms.id=fn_inctransactions.paymenttermid ";
		$k++;
		}
	...
	if(!empty($obj->shcustomerid)  or empty($obj->action)){
		...
		$rptjoin.=" left join crm_customers on crm_customers.id=fn_inctransactions.customerid ";
		$k++;
		}
	...
	if(!empty($obj->shquantity)  or empty($obj->action)){
		...
		if(!empty($rptgroup)){
			array_push($aColumns, "sum(fn_inctransactions.quantity) quantity");
		}else{
		array_push($aColumns, "fn_inctransactions.quantity");
		}
	...
	if(!empty($obj->shtotal)  or empty($obj->action)){
		...
		array_push($aColumns, "fn_inctransactions.total");
		$k++;
		}
	...
	if(!empty($obj->shmonth)  or empty($obj->action)){
		...
		array_push($aColumns, "fn_inctransactions.month");
		$k++;
		}

	if(!empty($obj->shyear)  or empty($obj->action)){
		array_push($sColumns, 'year');
		array_push($aColumns, "fn_inctransactions.year");
		$k++;
		}
	...
	if(!empty($obj->shbankid)  or empty($obj->action)){
		...
		$rptjoin.=" left join fn_banks on fn_banks.id=fn_inctransactions.bankid ";
		$k++;
		}
...
if(!empty($obj->paymenttermid)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" fn_inctransactions.paymenttermid='$obj->paymenttermid'";
		$join=" left join em_paymentterms on fn_inctransactions.id=em_paymentterms.inctransactionid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
		}
	$track++;
...
if(!empty($obj->customerid)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" fn_inctransactions.customerid='$obj->customerid'";
		$join=" left join crm_customers on fn_inctransactions.id=crm_customers.inctransactionid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
		}
	$track++;
...
if(!empty($obj->month)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" fn_inctransactions.month='$obj->month'";
	$track++;
}
...
if(!empty($obj->year)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" fn_inctransactions.year='$obj->year'";
	$track++;
}
...
if(!empty($obj->bankid)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" fn_inctransactions.bankid='$obj->bankid'";
		$join=" left join fn_banks on fn_inctransactions.id=fn_banks.inctransactionid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
		}
	$track++;
