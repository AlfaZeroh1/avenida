...
<table style="clear:both;"  class="table" id="tbl" width="100%" border="0" cellspacing="0" cellpadding="2" align="center" >
	...
	<?php
		...
		$fields="fn_generaljournalaccounts.id as id, fn_generaljournalaccounts.acctypeid, sys_currencys.name currencyid, fn_generaljournalaccounts.name as accountid, sum(fn_generaljournals.debit) debit, sum(fn_generaljournals.credit) credit, case when sys_acctypes.balance='dr' then (sum(fn_generaljournals.debit)-sum(fn_generaljournals.credit)) else (sum(fn_generaljournals.credit)-sum(fn_generaljournals.debit)) end balance ";
		...
		$join.=" left join sys_acctypes on sys_acctypes.id=fn_generaljournalaccounts.acctypeid left join sys_currencys on sys_currencys.id=fn_generaljournalaccounts.currencyid ";	
...
// 			$where.=" (fn_generaljournalaccounts.categoryid='' or fn_generaljournalaccounts.categoryid is null) ";
		...
		$orderby="";
		$generaljournals->retrieve($fields,$join,$where,$having,$groupby,$orderby);echo mysql_error();//echo $generaljournals->sql;
		...
		if(!empty($_GET['acctypeid'])){
		  $query="select sum(debit) debit from fn_generaljournals where accountid in(select id from fn_generaljournalaccounts where categoryid='$row->id')";
		  $db=mysql_fetch_object(mysql_query($query));
		  
		  $row->debit=$row->debit+$db->debit;
		  
		  $query="select sum(credit) credit from fn_generaljournals where accountid in(select id from fn_generaljournalaccounts where categoryid='$row->id')";
		  $db=mysql_fetch_object(mysql_query($query));
	...
	?>
		<tr>
			...
			if($generaljournalaccounts->affectedRows>0){
			  ?>
			  <td><a href="generaljournals.php?categoryid=<?php echo $row->id; ?>&class=<?php echo $obj->class; ?>" target="_blank"><?php echo initialCap($row->accountid); ?></a></td>
			  <?
			}else{
			?>
