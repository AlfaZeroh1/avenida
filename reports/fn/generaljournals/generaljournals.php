...
<table style="clear:both;"  class="table" id="tbl" width="100%" border="0" cellspacing="0" cellpadding="2" align="center" >
	...
	<?php
		...
		$query = "select * from fn_accounttypes";
		...
		$query1="select group_concat(id) id from sys_acctypes where accounttypeid='$row->id'";
		...
		if($row->id==1 and $row->id==2){
		  ...
		  $query="select sum(debit*(case when '$obj->currencyid=5' then fn_generaljournals.rate when '$obj->currencyid=1' then fn_generaljournals.eurorate else 1 end)) debit, sum(debitorig) debitorig from fn_generaljournals $where and accountid in(select id from fn_generaljournalaccounts where acctypeid in($row2->id)) ";
		  $db=mysql_fetch_object(mysql_query($query));
		  
		  $row->debit=$row->debit+$db->debit;
		  $row->debitorig=$row->debitorig+$db->debitorig;
		  
		  $query="select sum(credit*(case when '$obj->currencyid=5' then fn_generaljournals.rate when '$obj->currencyid=1' then fn_generaljournals.eurorate else 1 end)) credit, sum(creditorig) creditorig from fn_generaljournals $where and accountid in(select id from fn_generaljournalaccounts where acctypeid in($row2->id)) ";
		  $db=mysql_fetch_object(mysql_query($query));
	...
	?>
		<tr>
			<td><?php echo $i; ?></td>
			<td><a href="generaljournalsaccounttype.php?accounttypeid=<?php echo $row->id; ?>&grp=1&class=<?php echo $obj->class; ?>" target="_blank"><?php echo initialCap($row->name); ?></a></td>
