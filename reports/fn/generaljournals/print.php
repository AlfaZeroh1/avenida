...
$obj->categoryid = $_GET['categoryid'];
$obj->currencyid = $_GET['currencyid'];
$obj->accounttypeid = $_GET['accounttypeid'];
...
$groupby="";
$orderby="";
$where = " where crm_customers.id='$generaljournalaccounts->refid' ";
...
$groupby="";
$orderby="";
$where = " where proc_suppliers.id='$generaljournalaccounts->refid' ";
...
?>
           ...
           <h3>
           <div align="right" style="margin-right:240px;" >As At <?php echo date("Y-m-d H:i:s"); ?></div>
           </h3>
           </span>
            </div>
</td>
...
<div id="company" style="text-align:left">  
            ...
            Email: <?php echo $_SESSION['companyemail']; ?>
            </strong>
            </p>
</div>
</td>
...
<div class="fnt">
			      ...
			      <?php if(!empty($ob->invno)){ ?>
			      <th width="" align="center"><span style="text-decoration:underline;">Invoice No.</span></th>
			      <?php }if(!empty($ob->shippingno)){ ?>
			      <th width="" align="center"><span style="text-decoration:underline;">Shipping No.</span></th>
			      <?php }if(!empty($ob->delno)){ ?>
			      <th width="" align="center"><span style="text-decoration:underline;">Del No.</span></th>
			      <?php }if(!empty($ob->pono)){ ?>
			      <th width="" align="center"><span style="text-decoration:underline;">PO No.</span></th>
			      <?php } ?>
			      ...
			      <th width="" align="center"><span style="text-decoration:underline;">Memo</span></th>
			      <th width="" align="center"><span style="text-decoration:underline;"></span></th>
		...
		if(!empty($obj->categoryid))
		{
		     $where=" where (fn_generaljournals.accountid='$obj->categoryid' or accountid in (select id from fn_generaljournalaccounts where categoryid='$obj->categoryid')) ";
		}else{
		     $where=" where fn_generaljournals.accountid='$obj->accountid' ";
		}
		
		if($obj->accounttypeid==1 and $obj->accounttypeid==2){
		  $where.=" and transactdate>='$obj->fromtransactdate' and transactdate<='$obj->totransactdate' and fn_generaljournals.transactdate!='0000-00-00' ";
		}else{
		  $where.=" and transactdate<'$obj->fromtransactdate' and fn_generaljournals.transactdate!='0000-00-00' ";
		}
		$groupby=" ";
		$orderby="";
		...
		<tr>
			...
			<td align="center"></td>
			<?php } ?>
			<td colspan="2" align="center">Bal C/D</td>
		...
		$i=0;
		$generaljournals=new Generaljournals ();
		$fields="fn_generaljournals.id, fn_generaljournals.daccountid, fn_generaljournals.tid, fn_generaljournals.documentno, fn_generaljournals.chequeno,  sys_currencys.name currencyid, fn_generaljournals.mode, sys_transactions.name , fn_generaljournals.transactionid, fn_generaljournals.remarks, fn_generaljournals.memo, fn_generaljournals.transactdate, case when $obj->currencyid=1 then sum(fn_generaljournals.debit*fn_generaljournals.eurorate) when $obj->currencyid=5 then sum(fn_generaljournals.debit*fn_generaljournals.rate) else sum(fn_generaljournals.debit) end  debit, case when $obj->currencyid=1 then sum(fn_generaljournals.credit*fn_generaljournals.eurorate) when $obj->currencyid=5 then sum(fn_generaljournals.credit*fn_generaljournals.rate) else sum(fn_generaljournals.credit) end credit";
		$join=" left join sys_transactions on sys_transactions.id=fn_generaljournals.transactionid left join sys_currencys on sys_currencys.id=fn_generaljournals.currencyid ";
		$having="";
		if(!empty($obj->categoryid))
		{
		...
		$groupby= " group by jvno,documentno,transactionid ";
		...
		while($row=mysql_fetch_object($res)){
			...
			if($row->transactionid==28){
			  $query="select group_concat(invoiceno) nos from fn_supplierpaidinvoices left join fn_supplierpayments on fn_supplierpaidinvoices.supplierpaymentid=fn_supplierpayments.id where fn_supplierpayments.documentno='$row->documentno' ";
			  $invoices = mysql_fetch_object(mysql_query($query));
			}
		?> 
		<tr>
			...
			<?php } ?>
			<td colspan="2" align="left"><?php echo $row->remarks; ?><?php if(!empty($row->chequeno)){echo "&nbsp;CHQ: ".$row->chequeno;}?></td>
			<td colspan="2" align="left"><?php echo $row->memo; ?>&nbsp;<?php if(!empty($invoices->nos)){echo "&nbsp;INV: ".$invoices->nos; }?></td>
			<td align="right"><?php if(!empty($row->debit)){echo formatNumber($row->debit);} ?></td>
			<td align="right"><?php if(!empty($row->credit)){echo formatNumber($row->credit);} ?></td>
			<td align="right" style="font-weight: bold; "><?php echo formatNumber($bal); ?></td>
		</tr>
   ...
   <tfoot>
	...
	<?php // } ?>
	 <?php
		...
		$where.= " and fn_generaljournals.transactdate <= '$obj->totransactdate' and (DATEDIFF('$obj->totransactdate',transactdate)+$customers->creditdays)<0 and fn_generaljournals.transactdate>='2015-07-01' and fn_generaljournals.transactdate!='0000-00-00' ";
		$groupby= "";
		$orderby="";
		...
		$where.= " and fn_generaljournals.transactdate <= '$obj->totransactdate' and (DATEDIFF('$obj->totransactdate',transactdate)+$customers->creditdays)>0 and (DATEDIFF('$obj->totransactdate',transactdate)+$customers->creditdays)<=30 and fn_generaljournals.transactdate>='2015-07-01' and fn_generaljournals.transactdate!='0000-00-00' ";
		$groupby= "";
		$orderby="";
		...
		$where.= " and fn_generaljournals.transactdate <= '$obj->totransactdate' and (DATEDIFF('$obj->totransactdate',transactdate)+$customers->creditdays)>30 and (DATEDIFF('$obj->totransactdate',transactdate)+$customers->creditdays)<=60 and fn_generaljournals.transactdate>='2015-07-01' and fn_generaljournals.transactdate!='0000-00-00' ";
		$groupby= "";
		$orderby="";
		...
		$where.= " and fn_generaljournals.transactdate <= '$obj->totransactdate' and (DATEDIFF('$obj->totransactdate',transactdate)+$customers->creditdays)>60 and (DATEDIFF('$obj->totransactdate',transactdate)+$customers->creditdays)<=90  and fn_generaljournals.transactdate>='2015-07-01' and fn_generaljournals.transactdate!='0000-00-00' ";
		$groupby= "";
		$orderby="";
		...
		$where.= " and fn_generaljournals.transactdate <= '$obj->totransactdate' and (DATEDIFF('$obj->totransactdate',transactdate)+$customers->creditdays)>90 and fn_generaljournals.transactdate>='2015-07-01' and fn_generaljournals.transactdate!='0000-00-00' ";
		$groupby= "";
		$orderby="";
