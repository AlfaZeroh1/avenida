...
<?php }?>
<form  action="generaljournalsacctype.php" class="forms" method="post" name="generaljournals">
<table>
...
<table style="clear:both;"  class="table" id="tbl" width="100%" border="0" cellspacing="0" cellpadding="2" align="center" >
	...
	<?php
		...
		$query = "select * from fn_generaljournalaccounts where acctypeid='$obj->acctypeid' and (categoryid is null or categoryid=0 or categoryid='')";
		...
		//accountid=8, give us total of 
		  ...
		  //get the grand children of 8, children of 326 and get their totals
		   $query="select sum(debit*(case when '$obj->currencyid=5' then fn_generaljournals.rate when '$obj->currencyid=1' then fn_generaljournals.eurorate else 1 end)) debit, sum(debitorig) debitorig from fn_generaljournals left join fn_generaljournalaccounts on fn_generaljournalaccounts.id=fn_generaljournals.accountid $where and categoryid in(select id from fn_generaljournalaccounts where categoryid in($row->id)) ";
		  $db=mysql_fetch_object(mysql_query($query));
		  
		  $row->debit=$row->debit+$db->debit;
		  $row->debitorig=$row->debitorig+$db->debitorig;
		  
		  $query="select sum(credit*(case when '$obj->currencyid=5' then fn_generaljournals.rate when '$obj->currencyid=1' then fn_generaljournals.eurorate else 1 end)) credit, sum(creditorig) creditorig from fn_generaljournals left join fn_generaljournalaccounts on fn_generaljournalaccounts.id=fn_generaljournals.accountid $where and categoryid in(select id from fn_generaljournalaccounts where categoryid in($row->id)) ";
		  $db=mysql_fetch_object(mysql_query($query));
	...
	?>
		<tr>
			...
			?>
			<td><a href="generaljournalscategory.php?categoryid=<?php echo $row->id; ?>&accounttypeid=<?php echo $obj->accounttypeid; ?>&tb=true&grp=1&class=<?php echo $obj->class; ?>" target="_blank"><?php echo initialCap($row->name); ?></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<a href="account.php?id=<?php echo $row->id; ?>&accounttypeid=<?php echo $obj->accounttypeid; ?>&tb=true&class=<?php echo $obj->class; ?>" target="_blank"><img src="p.jpeg"></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<a href="account.php?categoryid=<?php echo $row->id; ?>&accounttypeid=<?php echo $obj->accounttypeid; ?>&tb=true&class=<?php echo $obj->class; ?>" target="_blank"><img src="pk1.jpeg"></a></td>
			<?php
			}else{
			?>
			<td><a href="account.php?id=<?php echo $row->id; ?>&tb=true&accounttypeid=<?php echo $obj->accounttypeid; ?>&class=<?php echo $obj->class; ?>" target="_blank"><?php echo initialCap($row->name); ?></a></td>
			<?php
			}
			?>
