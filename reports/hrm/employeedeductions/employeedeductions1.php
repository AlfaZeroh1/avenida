...
//processing columns to show
	...
	if($obj->deductionid==7){
	  array_push($sColumns,"arrears");
	  array_push($aColumns, "(select sum(amount) from hrm_employeearrears where employeeid=hrm_employeedeductions.employeeid and month='$obj->frommonth' and year='$obj->fromyear') arrears");
	  $k++;
	}
	...
	if(!empty($obj->shdeductiontypeid)  or empty($obj->action)){
		...
		$rptjoin.=" left join hrm_deductiontypes on hrm_deductiontypes.id=hrm_employeedeductions.deductiontypeid ";
		$k++;
		}
...
if(!empty($obj->deductiontypeid)){
	if($track>0)
		...
		$join=" left join hrm_deductiontypes on hrm_employeedeductions.id=hrm_deductiontypes.employeedeductionid ";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
		}
	$track++;
