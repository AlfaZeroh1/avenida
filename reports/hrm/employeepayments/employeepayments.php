...
$rptgroup='';
$track=0;
if(!empty($obj->gremployeeid) or !empty($obj->grassignmentid) or !empty($obj->grpaymentmodeid) or !empty($obj->gremployeebankid) or !empty($obj->grbankbrancheid) or !empty($obj->gryear) or !empty($obj->grmonth) or !empty($obj->grpaidon) or !empty($obj->grcreatedon) or !empty($obj->grcreatedby) or !empty($obj->grdepartmentid)){
	$obj->shemployeeid='';
	$obj->shassignmentid='';
	$obj->shpaymentmodeid='';
	$obj->shbankid='';
...
//processing columns to show
	...
	if(!empty($obj->shbasic) or empty($obj->action)){
		...
		if(!empty($rptgroup))
		   array_push($aColumns, "case when sum(hrm_employeepayments.basic) is null then 0 else round(sum(hrm_employeepayments.basic),0) end basic");
		else
		  array_push($aColumns, "case when hrm_employeepayments.basic is null then 0 else round((hrm_employeepayments.basic),0) end basic");
		$k++;
	...
	if(!empty($obj->shdepartmentid)  or empty($obj->action)){
		...
		$join=" left join hrm_employees on hrm_employees.id=hrm_employeepayments.employeeid ";
