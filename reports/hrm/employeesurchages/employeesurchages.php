...
//processing columns to show
	...
	if(!empty($obj->shchangedon)  or empty($obj->action)){
		array_push($sColumns, 'chargedon');
		array_push($aColumns, "hrm_employeesurchages.chargedon");
	}
	...
	if(!empty($obj->shcreatedby)  or empty($obj->action)){
		...
		$rptjoin.=" left join auth_users on auth_users.id=hrm_employeesurchages.createdby ";
	}
...
<form  action="employeesurchages.php" method="post" name="employeesurchages" class=''>
  ...
  ?>
        ...
        <option value="12" <?php if($obj->tomonth==12){echo"selected";}?>>December</option>
			...
			<select name='createdby' class='selectbox'>
				...
				$where=" where auth_users.id in(select createdby from hrm_employeesurchages) ";
