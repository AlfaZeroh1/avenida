...
//processing columns to show
	...
	if(!empty($obj->shcurrencyid)  or empty($obj->action)){
		...
		$rptjoin.=" left join sys_currencys on sys_currencys.id=inv_items.currencyid ";
	}
	...
	if(!empty($obj->shcostprice)  or empty($obj->action)){
		...
		array_push($sColumns, 'lcostprice');
		array_push($aColumns, " (select round(costprice,2) costprice from proc_purchaseorderdetails where itemid=inv_items.id having costprice<>inv_items.costprice order by id desc limit 1) as lcostprice");
	}
...
if(!empty($obj->currencyid)){
	if($track>0)
		$rptwhere.="and";
		$rptwhere.=" inv_items.currencyid='$obj->currencyid'";
	$track++;
}
		...
		<table class="tgrid gridd" width="100%" border="0" align="left">
			...
			<tr>
				...
				<td><input type='checkbox' name='shipaddress' value='1' <?php if(isset($_POST['shipaddress']) ){echo"checked";}?>>&nbsp;IP Address</td>
                        <tr>
                                <td><input type='checkbox' name='shcurrencyid' value='1' <?php if(isset($_POST['shdepartmentid'])  or empty($obj->action)){echo"checked";}?>>&nbsp;Currency</td>
		</table>
		</td>
	</tr>
	<tr>
