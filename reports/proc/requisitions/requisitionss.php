...
//processing columns to show
	...
	if(!empty($obj->shlpos)  or empty($obj->action)){
		array_push($sColumns, 'lquantity');
		array_push($aColumns, "(select sum(proc_purchaseorderdetails.quantity) from proc_purchaseorderdetails left join proc_purchaseorders on proc_purchaseorderdetails.purchaseorderid=proc_purchaseorders.id where proc_requisitions.documentno in(proc_purchaseorders.requisitionno) and proc_requisitiondetails.itemid=proc_purchaseorderdetails.itemid) lquantity");
	}
	
	if(!empty($obj->shgrns)  or empty($obj->action)){
		array_push($sColumns, 'gquantity');
		array_push($aColumns, "(select sum(proc_inwarddetails.quantity) from proc_inwarddetails left join proc_inwards on proc_inwarddetails.inwardid=proc_inwards.id where proc_purchaseorders.documentno in(proc_inwards.lpono) and proc_inwarddetails.itemid=proc_purchaseorderdetails.itemid) gquantity");
	}
	...
	if(!empty($obj->shcreatedby)  or empty($obj->action)){
		...
		$rptjoin.=" left join auth_users on auth_users.id=proc_requisitions.createdby ";
		$k++;
		}
...
<form  action="requisitionss.php" method="post" name="requisitions" >
<table width="100%" border="0" align="center">
	<tr>
		...
		<table class="tgrid gridd" width="100%" border="0" align="left">
			...
			<tr>
				<td><input type='checkbox' name='shlpos' value='1' <?php if(isset($_POST['shlpos']) ){echo"checked";}?>>&nbsp;LPO</td>
			...
			<tr>
				<td><input type='checkbox' name='shgrns' value='1' <?php if(isset($_POST['shgrns']) ){echo"checked";}?>>&nbsp;GRN</td>
		</table>
		</td>
	</tr>
	<tr>
