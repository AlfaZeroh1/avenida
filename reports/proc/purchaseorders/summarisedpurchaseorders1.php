...
if(empty($obj->action)){
	$obj->fromrequesteddate=date('Y-m-d');
	$obj->torequesteddate=date('Y-m-d');
}
...
// array_push($aColumns, " 1 as approved");
	...
	if(!empty($obj->shsupplier)  or empty($obj->action)){
		...
		array_push($aColumns, "(select case when group_concat(distinct proc_purchaseorders.supplierid) is null then '' else group_concat(distinct proc_suppliers.name) end supplier from proc_purchaseorders where proc_purchaseorders.requisitionno=proc_requisitions.documentno and proc_purchaseorders.id in(select purchaseorderid from proc_purchaseorderdetails where proc_purchaseorderdetails.itemid=proc_requisitiondetails.itemid)) as supplier");
		$k++;
		$join=" left join proc_purchaseorders on proc_requisitions.documentno=proc_purchaseorders.requisitionno";
		if(!strpos($rptjoin,trim($join))){
			$rptjoin.=$join;
		}
...
<form  action="summarisedpurchaseorders1.php" method="post" name="summarisedpurchaseorders" >
...
<table style="clear:both;"  class="table" id="tbl" width="100%" height="200px" border="0" cellspacing="0" cellpadding="2" align="center" >
	<thead>
		<tr>            
		      <th>#</th>
		      <th>Item</th>
